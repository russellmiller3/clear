...
const CANONICAL_DISPATCH = new Map([
  ...
  ['enforce_that', (ctx) => {
    // Canonical message form (2026-05-03, back-compat removed): scan
    // backward for STRING preceded by ':' preceded by `or_fail_with_msg`.
    // The leading comma between expression and marker is conventional but
    // tolerated as optional. Bare `enforce that X` (no message) keeps
    // working with a default refusal — the no-message case has its own
    // tests and is not deprecated.
...
CANONICAL_DISPATCH.set('ask', (ctx) => {
  ...
  //
  // Single canonical form. If you need "approve first N runs, then auto-fire,"
  // write it as a visible conditional in source over an explicit counter
  // table you own:
  //
  //   count = look up first records in OpenNotepadApprovals where ...
  //   if count is less than 3:
  //     ask user to confirm 'Open Notepad?'
  //     save { ... } as new OpenNotepadApproval
  //   else:
  //     run command 'notepad.exe'
  //
  // The previous `with graduation after N runs` sugar was removed
  // 2026-05-14 as a PHILOSOPHY §1:1 cleanup — that sugar packaged an
  // implicit runtime state machine (counter table emit + scope-key build
  // + counter check + audit-row emit), violating "no super commands".
  if (ctx.tokens.length >= 4 && second === 'user' &&
      (ctx.tokens[2].canonical === 'to_connector' || ctx.tokens[2].value === 'to') &&
      ctx.tokens[3].value === 'confirm') {
    // Reject any trailing `with graduation` — the old sugar is gone.
    for (let k = 4; k < ctx.tokens.length - 1; k++) {
      ...
      if (isWith && isGrad) {
        ctx.errors.push({
          line: ctx.line,
          message: "`with graduation after N runs` was removed 2026-05-14 (PHILOSOPHY §1:1 cleanup). Write the gate logic as a visible conditional in source over an explicit counter table you declare yourself. Example: `count = look up first records in Approvals where action_key is 'X'; if count is less than 3: ask user to confirm 'X' ... else: ...`.",
        });
        return ctx.i + 1;
      }
    }

    const messageExpr = parseExpression(ctx.tokens, 4, ctx.line);
    if (messageExpr.error || !messageExpr.node) {
      ctx.errors.push({ line: ctx.line, message: "ask user to confirm needs a message. Example: ask user to confirm 'Proceed?'" });
      return ctx.i + 1;
    }

    const node = { type: NodeType.HUMAN_CONFIRM, message: messageExpr.node, line: ctx.line };
    ctx.body.push(node);
    return ctx.i + 1;
  }
