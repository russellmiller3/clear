...
- **When threading a new field through a per-target ctx, edit BOTH the variable build AND the ctx object literal — they're often 30+ lines apart.** Cycle 5 added `ctx.tablePolicies` (a per-table access-rule map). I correctly populated the variable at the schema-collection loop in `compileToJSBackend` (line ~14663) but forgot to add `tablePolicies` to the ctx object literal at line 14695. The compiler kept seeing `ctx.tablePolicies = undefined` even though the populated map was sitting two functions away. The lookup branch's wrap silently became a no-op. Caught only because a debug `console.error` showed `hasTablePolicies: false` at the call site. Five minutes wasted hunting for a cascade-or-routing problem that turned out to be a one-token omission. **Rule:** when adding a new ctx field used by a downstream node-handler, grep `mode:\s*['"]backend['"]` (or whatever the ctx-build search pattern is) BEFORE the first compile run, and verify every match has the new field. The Cloudflare ctx, the Node-JS ctx, the BrowserServer ctx, and the Python ctx all need parallel updates.
- **Substring collisions on common English fragments are silent killers in the Python emit.** A docstring inside the inlined `_DB.update` Python class said *"Convention 2 lets the compiled output enforce per-row policies."* The compiler test at line 598 was `expect(result.python.includes('let')).toBe(false)`. The word "lets" matched. Test failed with `true !== false` and ZERO context about which line had "let" — I had to write a probe script to find the offset. Reworded to "Convention 2 makes ..." and the test passed. **Rule:** never use the words `let`, `const`, `var`, `function`, `return`, `=>`, `;` (the JS artifact list the Python tests check) in any string the Python compiler embeds — comments, docstrings, error messages, anything. Even fragments inside larger words count. The "auth substring collision" gotcha (CHANGELOG 2026-04-07) is the same pattern with a different victim.
- **The runtime auto-add precedent (tenant_id → user_id) is the cleanest way to ship per-row security plumbing without making every app declare a column.** Russell's queue spec said "add user_id (number) back to the per-user tables across all 8 templates" — that would have been ~17 edits across 12 files plus a per-app maintenance burden forever. Instead, mirror the existing tenant_id auto-add at `runtime/db.js:219` and the parallel backfill at line 237. One commit, zero per-app changes, scales to every future app. **Rule:** before doing a sweep of N templates to add a uniform field, check whether the runtime can add the column once. If the field is security plumbing (tenant_id, user_id, _version), invisible-to-author is the right design. If the field is domain-specific (status, created_at), per-app declaration is right.
- **Cycle 2b's "fixture sweep" is bigger than the headline suggests.** The queue said "~300 test fixtures." The actual count is 335 source strings inside `compileProgram(...)` calls in clear.test.js that declare a table without an access rule. A line-based rewrite script can't trivially handle inline-in-JS strings (per the existing "rewrite-enforce-that-msg.mjs" gotcha) — substitution risks adding rules to fixtures that intentionally test the warning-shape. Best done as a focused branch with dry-run + spot-check before applying.

---

## Session 2026-05-04 (also): cramped layout was duplicate-CSS-rule fight, not a cache bug

Russell rebuilt Studio + the Marcus apps and STILL saw cramped stat cards stretched across the full content area. My initial diagnosis (HANDOFF.md "browser cache, suspect 1") was wrong — the new cap rule wasn't actually winning over the old stretch rule. Took 90 minutes to find because the obvious diagnostic (browser cache, stale Marcus app server, broken shortcut) was wrong.

### Gotchas-as-rules

- **Two CSS rules at the same selector specificity is a coin-flip even with source order.** The compiler emitted TWO definitions of `.clear-stat-strip` — the old design-system-v3 base map at line 16576 with `grid-template-columns: repeat(auto-fit, minmax(180px, 1fr))` (stretches), and my new rule in `BUTTON_PEARL_CSS` at line 16954 with `minmax(220px, 320px)` (caps). Same specificity (one class). Source order says LATER wins — but with the rules in different `<style>` blocks merged through Tailwind's pipeline, the cascade was unpredictable. Cards rendered stretched even though the cap rule came after textually. Fix: rewrite the BASE rule directly. Single source of truth, no override needed. **Rule:** when a CSS override "should win by source order" but doesn't actually fire visually, stop fighting the cascade and edit the original rule. Don't add a higher-specificity selector unless you genuinely need both rules to coexist.
- **`preview_inspect` beats `preview_screenshot` for verifying CSS caps.** The screenshot tool kept timing out (preview_screenshot 30s timeouts on the Marcus app even though `preview_eval` returned immediately). `preview_inspect` on `.clear-stat-card` instantly returned `width: 320px` (CSS) and `boundingBox.width: 278` (rendered) — caught the fix working without ever rendering an image. **Rule:** for verifying spacing / dimensions / colors, default to `preview_inspect` and `preview_eval`. Save screenshots for "does this still LOOK like a dashboard" eyeball checks, not for confirming a max-width fired.
- **The wrong CSS class on a form-field rule is silent.** My first form-field cap targeted `.clear-form-field` — a class the compiler never emits. The rule loaded, parsed, applied to zero elements. Inputs stayed at 1223px. The fix was finding the class DaisyUI actually emits (`fieldset.fieldset`) via `preview_eval` listing the actual classNames on the form fields. **Rule:** before writing a CSS rule that targets compiled output, run a `preview_eval` query that lists the actual classes / tags on the elements you want to style. Don't assume the class name from the compiler-side intent — verify it from the rendered DOM.
- **Backticks in template-literal CSS comments break the build silently if the linter/Node parser sees them as template-literal terminators.** I had `Targets DaisyUI's \`fieldset\` class (the actual emitted class) plus \`clear-form-field\` for any explicitly-tagged fields` inside `BUTTON_PEARL_CSS = \`...\``. The backticks closed/reopened the template literal mid-comment, and `node` errored with `SyntaxError: Unexpected identifier 'fieldset'`. **Rule:** in template-literal CSS, never use backticks for emphasis even in /* */ comments. Use single quotes or just plain words.
- **Disabled-button hover guard needs ALL the variant combos.** A bare `.btn:disabled:hover` rule won't beat `.btn-primary:hover` because the more-specific selector wins. The fix lists every combo: `.btn-primary.btn-disabled:hover`, `.btn-primary:disabled:hover`, `.btn-primary[aria-disabled="true"]:hover`, repeat for each of `.btn-error`, `.btn-outline`, `.btn-ghost`, plus `.toolbar-btn.primary` for Studio's toolbar. Tedious but each combo is one specificity tier higher than the variant's hover state. **Rule:** when blocking an animated hover state on a disabled element, enumerate every variant × every "disabled" form (`:disabled`, `[aria-disabled]`, `.disabled`, `.is-disabled`).
...
- **A line-based rewrite script can't see source strings embedded inside JS string literals.** `scripts/rewrite-enforce-that-msg.mjs` walks every line that starts with `enforce that`. That handles standalone `.clear` files perfectly. But it misses inline-in-JS forms like `compileProgram("...\n  enforce that X or 'msg'\n  ...")` — the line in the `.js` file starts with `const result = ...`, not `enforce that`. Result: 20 occurrences in `clear.test.js` were untouched even after a clean run. Solution: keep the parser back-compat path active until a JS-string-aware rewrite pass strips them, then remove the fallback in a follow-up commit. **Rule:** any bulk-rewrite script for embedded-syntax patterns has TWO targets — standalone source files (line-scoped is fine) and inline strings (needs language-aware parsing). Plan for both before declaring scope.
- **The "no back-compat" rule has an honest two-commit shape when the rewrite is partial.** Russell's rule is "no back-compat once the new form ships." But shipping a strict-only parser with 20 known callers still using the old form means 20 instant test failures and a red main. The honest path is one commit that adds new + keeps old working + rewrites every safe-to-rewrite occurrence, then a follow-up commit that handles the harder occurrences and strips back-compat. Both commits land before the change is "done"; the back-compat existence between them is bounded and documented. The alternative — strict ship with broken tests — violates the deeper rule (don't leave Russell with broken state).
- **Parser changes that move from one form to another need the no-message case explicitly preserved.** My first parser draft made the message MANDATORY because the new form has a clear marker. That broke `enforce that X` (no message — was always supported, with a default refusal). The new parser handler now tries the new form first, falls back to old form, and if neither matches just leaves `guardMessage = null` so the default kicks in downstream. **Rule:** when refactoring a parser handler, list every shape the OLD handler accepted before writing the new one. "Make the new shape the only one" is a different decision than "make the new shape the only one with a message."
...
- **`testUtils.it()` is sync — it does NOT await async test functions.** Reading `it('...', async () => { ... })` and seeing ✅ in the output means NOTHING about whether the async work actually succeeded. The function gets called, returns a Promise, and `it()` immediately prints ✅ and increments `passed`. If the async assertion fails, you get an unhandled rejection AFTER the runner already counted the test as passing. My runtime-witness wiring printed three ✅ marks while the spawned servers were silently failing to boot — only the post-test crash made it visible. **Rule:** never use `it()` for tests with real awaits (spawn, fetch, sleep, anything I/O-bound). For async tests use top-level await OUTSIDE the describe/it harness, or write the test to drive its assertions synchronously. The existing `clear.test.js` has hundreds of `async () =>` test functions; most happen to work because their internal awaits are no-ops over sync calls (e.g. `compileProgram` is sync), but any test that does real I/O is silently broken until its assertion fails synchronously.
- **Port-zero is the right pattern for "find a free port."** Open `net.createServer().listen(0)`, read the OS-assigned port from `.address().port`, close the listener, hand the port to the spawned subprocess via `PORT` env var. There's a tiny race between `close` and `listen` on the same port, but the OS doesn't recycle that fast in practice. Beats picking from a hardcoded range (collisions in CI), beats spawning on port 0 directly (the compiled server's "Server running on port X" log shows `0` and you can't address it).
- **Spawning a CommonJS-shaped file inside an ESM-scoped repo needs BOTH directory-level fixes.** The compiled JS uses `require()`. Repo `package.json` says `"type": "module"`. If you write the tempfile as `.js` to repo root, node parses it as ESM and `require` is undefined. If you rename to `.cjs`, the tempfile loads as CJS but the FIRST `require('./clear-runtime/db')` walks node into a `.js` file under an ESM-scoped directory and fails the same way. Fix: `.cjs` extension on the tempfile AND a sibling `package.json` with `{"type": "commonjs"}` inside `clear-runtime/`. Both are load-bearing; neither alone is enough. The learnings rule on ESM/CJS scope already documents the second half — Russell had documented the trap, the runtime helpers just didn't have the file yet.
- **A "trust but verify" harness is the only honest way to ship a structural-proof claim.** When the prover says PROVED on the basis "the compiler emits a runtime guard," that's a delegation, not a verification. The harness measures the delegation: compile the source, run the compiled app, fire violating inputs, count rejections. If 100% of violators reject AND each rejection carries the rule name, the prover's claim is corroborated. If even one slips through, the prover was lying. Build this for every claim that delegates trust to another component — the alternative is shipping a credibility story you can't defend to the CTO who asks "how do you know?"

## Session 2026-05-02 evening: prover gap closure (equality folding, empty bodies, impure expressions)

The business-rules eval surfaced four prover verdicts that were quietly wrong. Each one was the kind of bug that doesn't crash anything — the prover happily reports the wrong answer with full confidence — which is the worst class of bug for a system whose pitch is "trust our verdict."

### Gotchas-as-rules

- **Multi-word operator phrases need explicit synonym entries even when each component is a known synonym.** `is` and `equal to` were both already synonyms for `is`/`==`. But the tokenizer's longest-match-greedy walks left to right token-by-token; given `is equal to`, it matches `is` first (1 token), then sees `equal to` (2 tokens) and matches it as a *separate* operator. The result was binary garbage: `5 is equal to 7` parsed as `5 == (variable_ref 'equal to')` and the right side was treated as a free variable. Fix: add `'is equal to'` and `'is not equal to'` as 3-word synonyms — the longest-match greedy then picks the 3-token sequence and emits a single operator. **Rule:** when adding any operator synonym, also add the form prefixed with the canonical word, otherwise the user's most-natural phrasing produces a tokenization that looks correct but smuggles a free variable into the parse tree.
- **Impurity walkers must descend into single-node expression fields, not just body arrays.** The prover's purity check walked `n.body`, `n.thenBody`, and `n.elseBody` arrays but missed `n.expression` — which is where assignments wrap their RHS. A rule like `scored = ask claude '…'` produces `{ type: 'assign', expression: { type: 'ask_ai' } }`, so the impure node hides one level deep inside a non-array field. Without descending into `expression`, the rule slipped past the impurity check and got marked as structurally PROVED — exactly the wrong verdict for a rule that calls AI. **Rule:** any AST walker that classifies node trees needs to descend into BOTH array-shaped containers (`body`, `thenBody`, `elseBody`, `actions`, `cards`) AND single-node fields (`expression`, `condition`, `target`). Forgetting either silently wrong-verdicts the cases that matter most.
- **Parser-level errors and prover-level verdicts have different audiences — don't double-emit.** The parser used to hard-error when a rule had an empty body. The prover already returned UNVERIFIABLE with a clear reason ("rule body is empty") for the exact same case. The parser error caused the proof bundle to bail out entirely (`status: 'parse_error', rules: []`), so the prover's well-crafted verdict never reached the user. The validator also catches empty bodies for compile-time enforcement. Three layers all trying to be authoritative produced one broken pipeline. **Rule:** parser errors should fire only for things the proof bundle and validator cannot recover from. Anything semantically meaningful (empty rule, no guard, missing refusal path) belongs to the prover and the validator — let the parser stay permissive so downstream layers can attribute verdicts.
- **Eval harnesses are how you discover that the prover's verdict was wrong.** Unit tests of the simplifier said "5 == 7 folds to false." Unit tests of `proveRule` said "guards that simplify to false → DISPROVED." Both passed. But the integration — does `enforce that 5 is equal to 7 or 'msg'` end up DISPROVED in the bundle? — failed silently because the tokenizer split the operator. Only the eval harness, which exercises the full pipeline from source string to verdict, caught the gap. **Rule:** any system where the user-facing claim is "we got this right" needs an eval harness that runs end-to-end inputs through the entire pipeline, not just unit tests of components. The four wrong verdicts here all passed every component-level test before this session.

## Session 2026-05-02: concurrency Phase 1 (read-modify-write detector)

The first half of the regulated-tier "no race conditions" pitch landed as a static validator pass plus two new endpoint modifiers (`safe to retry`, `with optimistic lock`). Phase 1 detects every read-modify-write pattern in plain sight; Phase 2 will wire the runtime that prevents the race. The honest sentence after Phase 1 is "we flag every endpoint where a race can happen," not "we prove no races." Selling Phase 1 as prevention is a customer trust problem; selling it as detection is honest and still differentiating.

### Gotchas-as-rules

- **Validator warnings are a mix of strings and objects, and existing tests use `w.includes(...)` directly on them.** Pushing a structured object (`{line, severity, code, message}`) from a new validator pass crashes any test whose negative-control endpoint accidentally trips that new pass. Existing tests assume warnings are strings; honoring that assumption (string with embedded `[CODE]` prefix for searchability) keeps the test surface stable. Object-shaped warnings stay legitimate elsewhere — the rule is "match the surrounding convention; don't unilaterally restructure the warning array shape from a new validator." The `validator.js` warning array's mixed-shape history is a real constraint, not an accident.
- **Endpoint body-line modifiers are the cheapest way to add per-endpoint metadata.** `requires login` is the template: a tiny canonical-dispatch handler emits a marker node into the endpoint body, and the validator/compiler walk for it. Two new modifiers (`safe to retry`, `with optimistic lock`) shipped in ~20 lines of parser code each because the pattern is generalizable. When you need a per-endpoint declaration that isn't a CRUD verb, write it as a body-line modifier rather than a header keyword — the parser doesn't have to change shape, and the surface reads like English.
- **Read-modify-write detection works without dataflow analysis.** The pattern is local: lookup → ASSIGN with possessive name (or FIELD_CHANGE) → save-as-update of the same variable. No alias tracking, no flow-sensitive analysis, no symbolic execution — three statement-shape checks against the endpoint body in order. The trade is that mutations behind a function call slip through Phase 1; that's the right scope choice for a static detector that has to ship in one session and stay false-positive-free across all 8 templates.
- **Multi-word synonyms beat single-word ones because the tokenizer is greedy.** `with optimistic lock` (3 words) beats single-word `with`; `safe to retry` (3 words) beats single-word `retry`. The tokenizer's `MULTI_WORD_SYNONYMS` array is sorted longest-first, so any new multi-word phrase that contains an existing synonym still works. The collision check is "does my phrase appear inside an existing pattern?" not "does my phrase share a word?" — and the answer for both phrases here was no.
- **Phase-end doc cascade survives the long arc; commit-end cascade does not.** When a session ships multiple commits all in service of one epic (test → parser → validator → docs), running the doc sweep at the end of the EPIC keeps phase context fresh. Running it after every commit fragments the cascade across surfaces; running it at session end loses the why-it-matters framing the user-facing docs need. Phase-end is the right granularity.
