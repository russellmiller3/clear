...
export const LANGUAGE_PRIMITIVE_SPECS = Object.freeze([
  {
    ...
    description: 'Prevent double-processing when two approvers click approve on the same pending request; the second submit checks status before changing it twice',
    feature_tags: ['concurrency', 'approval', 'optimistic_lock', 'double_processing', 'status_transition', 'two_approvers', 'same_pending_request', 'second_submit', 'stale_approval', 'click_approve'],
    source: [
      'when user updates deal at /api/deals/:id/approve:',
      '  requires login',
      '  // Use when two approvers click approve on the same pending request; the second submit must not change it twice.',
  ...
  },
  {
    ...
    is_primitive: 1,
    pattern_set: 'language',
    title: 'Approval routing by amount threshold',
    description: 'Route approval work to a manager below a threshold and to a VP at or above the threshold',
    feature_tags: ['routing', 'approval', 'queue', 'amount_threshold', 'manager', 'vp'],
    source: [
      ...
      '',
      'queue for request:',
      "  reviewer is request's approval_tier",
      '  actions: approve, reject',
    ].join('\n'),
  },
  {
    template_name: 'clear-language::row_action::approval-approve-reject-actions',
    parent_template_name: 'clear-language-primitives',
    pattern_kind: 'row_action',
    is_primitive: 1,
    pattern_set: 'language',
    title: 'Approval queue approve and reject row actions',
    description: 'Add per-row approve and reject actions to an approval queue table',
    feature_tags: ['row_action', 'button_action', 'approval', 'approve', 'reject', 'endpoint'],
    source: [
      'display pending as table showing title, requester, amount, status with actions:',
      "  'Approve' is primary",
      '    // data-action: updates the selected request to approved and refreshes pending requests.',
      "    change selected_request's status from 'pending' to 'approved'",
      '    update selected_request at /api/requests/:id/approve',
      "  'Reject' is danger",
      '    // data-action: updates the selected request to rejected and refreshes pending requests.',
      "    change selected_request's status from 'pending' to 'rejected'",
      '    update selected_request at /api/requests/:id/reject',
    ].join('\n'),
  },
  {
    template_name: 'clear-language::workflow::booking-customer-availability-overlap',
    parent_template_name: 'clear-language-primitives',
    pattern_kind: 'workflow',
    is_primitive: 1,
    pattern_set: 'language',
    title: 'Room booking with customers, availability, and overlap guard',
    description: 'Model rooms, customers, and bookings; search available rooms; reject overlapping bookings before save; allow cancellation through a backend action',
    feature_tags: ['booking', 'room', 'customer', 'availability', 'available_rooms', 'overlap', 'double_booking', 'cancel', 'workflow'],
    source: [
      ...
      '',
      'when user requests data from /api/rooms/available:',
      '  requires login',
      "  start_time = incoming's start_time",
      "  end_time = incoming's end_time",
      '  rooms = get all Rooms',
      '  send back rooms',
      '',
      'when user sends booking to /api/bookings:',
      '  requires login',
      "  existing_booking = look up Booking where room_id is booking's room_id",
      "  if existing_booking's start_time is before booking's end_time and existing_booking's end_time is after booking's start_time:",
      "    fail with error message 'Room is unavailable'",
      '  saved = save booking as new Booking',
      '  send back saved',
      '',
      'when user updates booking at /api/bookings/:id/cancel:',
      '  requires login',
      "  change booking's status from 'confirmed' to 'cancelled'",
