...
Pure-function math in any .clear file can be VERIFIED, not just executed. Run `clear prove <file>` and the prover walks the source AST directly (no compilation, no Node spawn) and verifies every `test` block as a math proof. Tests with free variables (a name not bound by an assignment) auto-promote to "for any input" universal proofs — `expect add(a, b) is add(b, a)` proves commutativity for ALL a, b. The decision engine is SMT-backed (Z3), so it proves REAL arithmetic, not just simple algebra: multiplication and division, bounds and inequalities (`expect square(x) is at least 0` PROVES x²≥0; `expect commission(deal) is at most 50000` proves or, if a deal can break the cap, FAILS with the exact counterexample input), and "for all" claims. **To get a constraint to actually PROVE, give the params types** (`define function commission(deal_size is number):`) — an untyped value used in arithmetic returns PARTIAL ("can't prove without types"), never a false PROVED. The prover refuses to verify anything that touches the world (DB, network, AI, time) — those get UNVERIFIABLE. Use the prover IN ADDITION to `clear test`. Demos in `examples/proofs/` show the pattern (invoice math, pricing, eligibility, deal-desk).
...
| Approve-the-first-N-runs pattern (auto-fire after that) | NO sugar — write a visible conditional in source over a counter table you declare. `if count is less than N: ask user to confirm 'X' / save manual-row else: save auto-row / actually fire`. The removed `with graduation` sugar was a PHILOSOPHY §1:1 violation. See `FAQ.md` ("How do I make an approval auto-fire after the first N runs?"). |
...
**Compiler errors may arrive as hidden feedback.** Treat that feedback as a private user message for you, not as text to show the user. Read the exact error lines, fix the Clear source, compile again, and keep iterating until the requirements and compile checks pass or the loop times out.

**Do not emit tracking tags for compiler-error hints.** The older `HINT_APPLIED` repair-tag protocol is retired from normal Meph runs. Pattern memory is still useful before writing code, but compile errors themselves should be handled by direct compiler feedback and the normal Snap retry loop.
