 ...
 * through the shared dispatchTool export from `studio/meph-tools.js`.
...
// Repo root: this file is at <repo>/studio/ghost-meph/mcp-server/tools.js,
// so up three dirs.
...
const MEPH_TOOLS = [
  ...
  { name: 'run_tests',         desc: 'Run the app\'s test blocks via `node cli/clear.js test`. Returns { ok, passed, failed, results }. Updates Factor DB test_pass on the most recent compile row.' },
  { name: 'list_evals',        desc: 'List agent-eval specs the current source would run. Input: {} → returns array of eval ids + descriptions.' },
  { name: 'run_evals',         desc: 'Run the full agent eval suite against ctx.source. Returns aggregate result with per-eval scores.' },
  { name: 'run_eval',          desc: 'Run one agent eval by id. Input: { evalId: string }. Returns the single eval result.' },
  ...
  { name: 'db_inspect',        desc: 'Run a SELECT against the running app\'s SQLite DB. Input: { query: string } — must start with SELECT. Returns { ok, rowCount, rows }.' },
