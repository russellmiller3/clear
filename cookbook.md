...
|------|-------|--------------|---------------|
| `confirm-before-start.mjs` | SessionStart | Tells Claude to read HANDOFF.md, summarize the top next move in 3-5 plain-English bullets, then STOP and wait for the user to type "g" (one-letter green light) or another instruction. No auto-pilot opening. | The auto-pilot session opener (build queue → start working immediately) sometimes started on the wrong thing because the user hadn't confirmed priorities. With confirm-before-start, the user can read the summary in 10 seconds and either type "g" to proceed or redirect. Costs one round-trip; saves wrong-direction sessions. Added 2026-05-03 after the user asked "all i should have to type is g." |
