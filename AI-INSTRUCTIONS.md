...
Clear supports full TDD for pure functions. Always write the failing test before writing the function.

**Pattern: red → green → refactor**

```clear
# Step 1: write the test (it fails — function doesn't exist yet)
test 'tax calculation':
  set result to calculate_tax(100, 0.08)
  expect result is 8

# Step 2: write the function (test goes green)
define function calculate_tax(price, rate):
  send back price * rate

# Step 3: add edge cases
test 'tax on zero price':
  set result to calculate_tax(0, 0.08)
  expect result is 0

test 'tax rounds to two decimals':
  set result to calculate_tax(19.99, 0.07)
  expect result is less than 2
```

**Rules:**
- `send back` inside `define function` compiles to a plain `return` — not HTTP. Safe to call from test blocks.
- Call the function directly in the test: `set result to my_function(arg1, arg2)`, then `expect result is X`.
- Function parameters are positional. No named args.
- You can define a function named `sum`, `max`, `min`, etc. — user-defined functions shadow built-in aliases. `sum(a, b)` calls your function if one is defined.
- **Do not define functions named `length`, `keys`, `values` or other JS built-ins** — those can cause unexpected shadowing in JavaScript output.

**When to use function TDD vs endpoint TDD:**

| Use function TDD | Use endpoint TDD |
|-----------------|-----------------|
| Pure calculation (tax, discount, scoring) | HTTP behavior (status codes, auth, response shape) |
| String manipulation or formatting | Database reads/writes |
| Business logic that doesn't touch DB | Multi-step workflows |
| Algorithm you want to test in isolation | Anything requiring `req`/`res` context |
...
| `first item of rows` / `last record of rows` | `first of rows` / `last of rows` | Canonical form is short. The compiler also accepts selector noun phrases from the collection synonym table for natural repairs. |
