...
| `CRUD` | `save X as User` / `look up all records in Users table` / `delete from Users where ...` | In-memory DB or SQL. **`look up all` / `get all` caps results at 50 by default.** Use `look up every` / `get every` to return all rows. When the target table has an `RLS_POLICY` with a `creator` or `row_role` subject, every CRUD operation auto-injects the ownership check (compile-time emit). |
