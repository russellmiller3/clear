...
| `GRAMMAR_MATCH_CALL` | `set result to match input against 'concepts'` — runs the runtime-grammar matcher against any text expression. Accepts a literal string, variable, or any other expression that evaluates to text. Use anywhere an assignment RHS is valid, including inside `test 'name':` blocks for behavioral parity tests. | Compiles to `result = _grammarMatch('concepts', input)` (JS) / `result = _grammar_match('concepts', input)` (Python). Returns `{ kind: 'matched' \| 'no_match' \| 'partial' \| 'ambiguous', frame, slotValues, missingSlots, ambiguousMatches }`. Wired Cycle 1.10 (2026-05-13 night, commit 7d6a91a). |
