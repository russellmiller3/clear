...
| `LIVE_BLOCK` | `live:` + indented body — OPTIONAL explicit effect fence (2026-05-06: was originally planned to become mandatory in "Phase B-2," but that plan is dropped). Body holds calls that talk to the world (`ask claude`, `call API`, `subscribe to`, timers). The compiler NEVER requires effects to be wrapped — the prover walks the AST and classifies purity automatically. `live:` is a pure reader/audit marker for regulated apps that want a visual fence. Most apps write effects inline like every other language. | Body emits inline with a `// live: block — explicit effect fence` comment marker. Zero runtime semantics — pure documentation for the human reader. |
...
| `USE` | `import tables.clear` / `import helpers.clear as helpers` / `import double, triple from helpers.clear` / `import npm 'stripe' as stripe` / `import 'lib' from './lib.js'` | Module import. Canonical: `import` (with `include` as a silent alias). `use` was retired from the import grammar on 2026-05-13 and is reserved for future configuration syntax (e.g. `use postgres for the database`). Shapes: inline-all (default for path form), namespaced (`as <alias>`), selective (`NAME, NAME from PATH`), npm package, external-JS escape. |
...
| `DISPLAY` | `display x as dollars called 'Label'` / `display x as table showing a, b with delete` / `display x as chat showing role, content` / `display records as record browser` | `<output>` or `<table>` with action buttons, chat bubble component, or Lenat-style explorer/timeline pane (`capability explorer`, `record browser`, `trace timeline`) |
...
| `STAT_CARD` | `stat card 'Pending Count':` + `value EXPR`, optional `delta 'TEXT'`, `sparkline [1,2,3]` or `sparkline energy_logs taking 'level'`, `icon 'inbox'` | Polished KPI card with label, value, delta, optional sparkline and Lucide icon. Data-driven form (`sparkline VAR taking 'FIELD'`) emits an SVG placeholder filled client-side from `_state[VAR]` on load and every `_recompute` tick |
...
| `GUARD` | `enforce that stock > 0, or fail with error message: 'Out of stock'` | Conditional 400 response |
...
| Node Type | Syntax | Notes |
|-----------|--------|-------|
| `DATA_SHAPE` | `create a Users table:` \| `table Users:` \| `create data shape User:` + fields | Table schema with constraints. All three lead forms parse identically. Field declarations accept both `price, number` and `name is text`. The body can also declare per-row access rules — see `RLS_POLICY` below. |
| `RLS_POLICY` | `the deal's creator can read, change, or delete` \| `the deal's reviewer can read or change` \| `any admin can read` \| `anyone logged in can read` \| `anyone can read` \| `owner can update, delete` (legacy) | Per-row access rule attached to a `DATA_SHAPE`. Subjects: `creator` (maps to `user_id` on the row, auto-stamped on insert, auto-added to every SQLite table), `row_role` (maps to `<role>_id` field on the row), `any_role` (matches `req.user.role`), `anyone_logged_in`, `anyone`, plus legacy `owner` / `same_org` / `role`. Actions chain with comma + `or`: `read, change, or delete` (`change` is a synonym for `update`). The compiler auto-injects the matching filter on every CRUD operation against the table — lookup wraps the WHERE, save stamps `user_id` on insert, save (PUT /:id) switches to a 3-arg `db.update(table, where, data)` form, remove adds the field to the WHERE. Composes with `database is shared with tenant scope` so regulated apps stack both filters. Validator errors when a row-role rule names a missing field. |
| `CRUD` | `save X as User` / `look up all records in Users table` / `delete from Users where ...` | In-memory DB or SQL. **`look up all` / `get all` caps results at 50 by default.** Use `look up every` / `get every` to return all rows. When the target table has an `RLS_POLICY` with a `creator` or `row_role` subject, every CRUD operation auto-injects the ownership check (compile-time emit). Page-level `define X as: look up records in Y` nodes are **SSR-by-default**: the route handler pre-fetches them before sending HTML and injects `window.__CLEAR_INITIAL_STATE__` so the reactive runtime hydrates on first paint. Opt out with `fetch this data in the browser, not from the server` (sets `clientOnly:true`). |
| `BROWSER_FETCH_OPT_OUT` | `fetch this data in the browser, not from the server` (indented line under a `define X as: look up` or inline after a comma) | Modifier on a page-level CRUD lookup. Sets `clientOnly:true` on the CRUD node; the compiler skips it in the server-side SSR pre-fetch and lets the browser fetch it reactively. Use for real-time data, auth-scoped queries, or browser-only state that should not block first paint. Two valid forms: (1) indented on its own line under the define; (2) trailing after a comma on the same line as the define. |
...
| `OUTGOING_ALLOWLIST` | `allow outgoing requests to: 'api.stripe.com', 'api.openai.com'` | OWASP Piece 2 — top-of-file SSRF defense. The validator walks every `HTTP_REQUEST` and `EXTERNAL_FETCH` node and refuses to compile if any URL (a) is not a string literal OR (b) targets a host not in the allowlist. Synonyms: `allow outbound requests to`, `allow http requests to`, `allow external requests to`. Without the declaration, the existing private-IP block in the parser stays as the only check. |
| `CAN_RETURN_SENSITIVE` | `can return sensitive data` (body-line marker inside an endpoint) | OWASP Piece 3 — endpoint-level opt-in to return fields tagged `sensitive`. Without this marker, the runtime strips sensitive fields from API responses by default. Synonyms: `returns sensitive data`, `allow sensitive response`, `returns sensitive fields`. Lives alongside `requires login`, `safe to retry`, `with optimistic lock` as endpoint body modifiers. |

Field modifiers: `required`, `unique`, `default VALUE`, `auto` (timestamp), `hidden`, `sensitive`, `renamed to NEW_NAME`, `(number)` type hint, FK by capitalized name.
...
**`sensitive` field modifier (OWASP Piece 3 — encrypt at rest)** — `ssn is text, sensitive` marks a field as encrypted at rest. The compiler emits `sensitive: true` in the schema config. The runtime db layer (both SQLite and Postgres adapters) encrypts these fields with AES-256-GCM before insert/update and decrypts on read. On-disk format: `enc:v1:<iv-base64>:<ct-base64>:<authTag-base64>`. Key management: `SENSITIVE_KEY` env var (16+ chars), derived through scrypt with a domain-separator salt. If the key is missing, inserts FAIL CLOSED — refuse to write plaintext to disk. Reads of existing encrypted blobs return `[encrypted — set SENSITIVE_KEY]` placeholder. GCM auth tag means tampered ciphertext returns `[encrypted — wrong key or tampered]` rather than silent garbage. Composes with the `CAN_RETURN_SENSITIVE` endpoint marker — without the marker, sensitive fields are stripped from API responses regardless of encryption.

**Auto login rate-limit (OWASP Piece 4)** — when `AUTH_SCAFFOLD` is present, the compiler now auto-emits `rateLimit({ windowMs: 60000, max: 10 })` middleware on the auto-generated `POST /auth/login` route. No declaration needed; brute-force throttling is structural.

**Hardcoded-secrets linter (OWASP Piece 5)** — `validateHardcodedSecrets` walks every string literal in the AST and matches against high-confidence API-key shapes (Stripe `sk_live_`/`sk_test_`, AWS `AKIA…`, GitHub `ghp_/gho_/ghu_/ghs_`, Anthropic `sk-ant-`, OpenAI `sk-`/`sk-proj-`). Match → compile error naming the kind of key and suggesting an env var (`STRIPE_SECRET_KEY`, `AWS_ACCESS_KEY_ID`, `GITHUB_TOKEN`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`). Generic high-entropy strings are NOT flagged.
...
| `GUARD` | `check X is not missing, otherwise error 'msg'` | `enforce that X is not nothing, or fail with error message: 'msg'` | `throw new Error("msg")` (in agent) / `res.status(403).json({ error: "msg" })` (in endpoint, raw guard) / `res.status(403).json({ error: "msg", rule: "<rule-name>" })` (in endpoint, inside `rule:` block — rule name attached so the audit trail and runtime-witness harness can attribute every rejection to its named policy) |
...
| `HUMAN_CONFIRM` | `ask user to confirm 'message'` | (same) | Inserts a row into the Approvals table and returns HTTP 202 with `{ approval_id, message, status: 'pending' }`. The "auto-fire after first N runs" pattern is NOT sugar; write it as a visible conditional in source over a counter table you declare yourself. The previous `with graduation after N runs` sugar was removed 2026-05-14 as a PHILOSOPHY §1:1 cleanup. |
...
| Node Type | Syntax | Compiles To |
|-----------|--------|-------------|
_(RUNTIME_GRAMMAR, GRAMMAR_FRAME, GRAMMAR_MATCH_CALL — removed 2026-05-14. The `runtime grammar / frame / on match` block was a §1:1 violation: one keyword packaged a table, a registry, a matcher, and a dispatch block. Replaced by six composable primitives: `create a TABLE with rows:`, `define function NAME(input):`, `search for X in TABLE by FIELD or FIELD`, `if there's a match:`, `if no match:`, `call function EXPR with ARG`. See SYNTAX.md for each.)_
...
| `RULE_DEF` | `rule <name>:` + indented body of normal statements (`guard`, `validate`, `if`, etc.) | Top-level labeled wrapper around statements that name a business rule. Name is a kebab-case identifier or a quoted string the parser dasherizes (e.g. `rule 'Discount cap':` → `discount-cap`). Body parses with the same statement parser as endpoints — no new runtime semantics. Compiles to a labeled comment (`// rule: <name> (line N)`) followed by the body's normal emit. The prover walks every `rule_def` and produces a per-rule verdict (`proved`, `disproved`, `unverifiable`) attributed by name. `clear prove` and `clear test --prove` render a "Business rules in this file:" section with one badge per rule — the regulated-tier pitch surface. Validator hard-errors on `RULE_NAME_DUPLICATE`, `RULE_BODY_EMPTY`, `RULE_OUTSIDE_TOP_LEVEL`; warns on `rule_no_refusal_path` (body has no guard/validate/throw). Both targets (JS + Python) supported. Plan: `plans/plan-rule-keyword-rebuild-2026-05-02.md`. |
| `AGENT_BOUND_CLAIM` | `prove that agent 'X' cannot call <fn>` / `prove that agent 'X' cannot delete from <Entity>` / `prove that agent 'X' cannot modify <Entity>` | Top-level single-line proof obligation about an agent's tool-use surface. Three flavors: (1) **Direct** — `cannot call <fn>` walks the agent's static tool closure (own `has tools:` + recursive `uses skills:` tools); PROVED iff `<fn>` is not in the closure. (2) **Transitive — delete** — `cannot delete from <Entity>` walks the agent body plus every reachable tool body (transitively, following function calls) for any `remove` CRUD op against `<Entity>`. (3) **Transitive — modify** — `cannot modify <Entity>` covers `save`/`remove`/`upsert`/`update`. The prover emits PROVED / DISPROVED / UNVERIFIABLE per claim and surfaces them in `clear prove` output under "Agent tool-bound claims:". Soundness: Clear's tool-use loop is closed-world — `_askAIWithTools` only honors functions in the compile-time-built `_toolFns` dict, with an "Unknown tool" fallthrough for anything else. Singular/plural entity names are accepted on either side (`Users` vs `User`). Compiles to nothing — claims are proof obligations, not runtime code. Tests: `lib/prover/index.test.js` under `Prover — agent tool-bound claims`. |
...
| `STAT_CARD` | `stat card 'Pending Count':` + `value pending_count`, optional `delta '+1.8 pts vs last week'`, `sparkline [3,4,6,5,8]` or `sparkline energy_logs taking 'level'`, `icon 'inbox'` | Dashboard stat card with value, trend copy, mini sparkline, and Lucide icon. Data-driven `taking` form reads from `_state[VAR]` at runtime |
