...
| **47–48** | **2026-04-25 night** | **Overnight autonomous run: winner-harvest closes the WIN-data asymmetry, sweep cleanup unblocks regression measurement, decidable-core foundation lands.** Six background workers ran in parallel worktrees on a feature branch; nine pieces of work merged cleanly, $0 spend (all gm/cc-agent). Highlights: (1) `cleanupStale()` helper drops idle/done/old session rows before each sweep so the "duplicate session id" UNIQUE-constraint failure no longer blocks training runs; 4 new tests. (2) **Winner-harvest Phase 1 — `scripts/score-winning-runs.mjs`** scores every test_pass=1 row by exemplariness (compactness × first-try cleanness × archetype-feature uniqueness); top 5 winners turn out to be 3-line first-try api_service builds (canonical-example shape). 19 new scorer tests. (3) **Winner-harvest Phase 5 — held-out test set** of 5 curriculum tasks (`echo`, `todo-crud`, `contact-book`, `webhook-stripe`, `agent-summary`) tagged `held_out: true`; still graded by every sweep but never feeds the hint retriever or future canonical-examples library. Diverse mix L2–L9. (4) Phase 1 of the shell upgrade: `app_*` presets now compile to semantic HTML5 with the slate-on-ivory chrome from the Marcus mock. (5) Decidable Core **Path B Phase 1** — `live:` keyword foundation parses + emits + 11 tests. Phase B-2 (validator rule that REJECTS effect-shaped calls outside `live:`) is the next chunk. (6) Friction-baseline snapshot at `snapshots/friction-baseline-04-25-2026.txt` ranks the top-20 compile errors by minutes-cost-to-Meph. (7) Builder Mode flipped to default for new users. (8) Marcus landing polish (action-led headline + deal-desk demo placeholder). (9) `cold-start.js` import-side-effect bug fixed (was running `run()` on import). 2586 → 2605 compiler tests green. The winner-harvest Phases 2-4 (hand-curate canonical-examples file, $10 A/B sweep, auto-promotion) deliberately deferred — those need human curation + Russell's go on the spend. |
...
**What this buys you (in principle).** Meph makes the same mistake once, the system stores the fix, and on a future session that hits the same shape of error the system retrieves the past working example and hands it to him. The more people use Clear, the smarter Meph should get for everyone — without retraining the underlying AI.

**What we're seeing as of 2026-04-29.** The architecture works on its component pieces — the database fills, the retriever runs, the ranker is trained, and when a hint reaches Meph he uses it about 70% of the time and rates it helpful most of the time. **But hints don't reach Meph very often yet.** 1826 of 1886 logged compile cycles have NO hint in the result at all (97%) — the retriever's archetype filter is concentrated in a few app shapes, and unfamiliar shapes slip through with nothing matched. We do not have positive lift evidence in a controlled hint_on vs hint_off A/B yet (three honest sweeps on 2026-04-29 showed 0%, -20% on 1 fewer pass, and 0% lift respectively, all with effectively no hints firing). **Treat the flywheel as a measured architecture in active research, not a guaranteed product feature.** When the corpus broadens to cover more archetypes — or the retriever's archetype filter is loosened — we expect the lift to surface; until then, the claim stays research-grade.
...
**The current bottleneck:** not row count. The repo has enough passing rows to train early rankers. The blocker is causal proof that hints improve Meph on tasks hard enough to discriminate. `scripts/hint-effect-report.mjs` now gives the honest current answer: inconclusive. Saturated tasks like `counter` and `kpi-dashboard` move to an appendix. The next headline needs Deal Desk-style hard tasks, hint-on versus hint-off, a p-value, a confidence interval, and an underpowered verdict when the sample is too small.
