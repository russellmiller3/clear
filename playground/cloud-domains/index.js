...
/**
 * CC-5b background poller. Reads every pending app_domains row, does a
 * DNS lookup (via injected resolveCnameFn — production passes
 * node:dns/promises.resolveCname, tests pass a mock), runs the result
 * through verifyCname, and updates the row:
 *
 *   verified → status='verified', verified_at=NOW(), last_error=NULL
 *   wrong    → status='failed', last_error=reason text
 *   pending  → last_checked_at=NOW() (row stays pending)
 *
 * Resolver throws (ETIMEDOUT, NXDOMAIN before DNS has propagated) are
 * caught and treated as 'pending' so the poller is robust against
 * transient DNS failures — the row will be picked up on the next tick.
 *
 * Returns summary counts for observability / cron dashboards.
 *
 * @param {object} db
 * @param {(domain:string) => Promise<string[]>} resolveCnameFn
 * @returns {Promise<{checked:number, verified:number, failed:number, stillPending:number}>}
 */
export async function verifyPendingDomains(db, resolveCnameFn) {
  const pending = await listPendingDomains(db);
  let verified = 0, failed = 0, stillPending = 0;
  for (const row of pending) {
    let records;
    try {
      records = await resolveCnameFn(row.domain);
    } catch (err) {
      // DNS lookup error — treat as pending; next tick retries
      records = null;
    }
    ...
    if (result === 'verified') {
      ...
      const reason = `CNAME is wrong — expected ${row.expected_cname}, got [${(records || []).join(', ')}]`;
      await db.query(
        `UPDATE app_domains
      ...
      // pending — DNS hasn't propagated yet or resolver errored
      await db.query(
        `UPDATE app_domains SET last_checked_at = NOW() WHERE id = $1`,
        [row.id]
      );
      stillPending++;
    }
  }
  return { checked: pending.length, verified, failed, stillPending };
}
