...
/**
 * CC-2c variant of listAppsForUser enriched with current-month call
 * counts per app. LEFT JOIN so apps with zero usage come back with
 * calls_this_month = 0 (not null — caller never null-checks).
 *
 * Single query — the dashboard renders "N calls this month" next to
 * each app row without a second round-trip. Usage period is the
 * calendar month (matches cloud-quota.checkQuota's period for
 * consistency between "what I see" and "what I'm billed for").
 *
 * @param {object} db - pg Pool or compatible
 * @param {number} userId
 * @returns {Promise<Array<{id:number, slug:string, my_role:string, calls_this_month:number, ...}>>}
 */
export async function listAppsWithUsageForUser(db, userId) {
  const { rows } = await db.query(
    `SELECT a.*, tm.role AS my_role,
            COALESCE(u.calls_this_month, 0)::integer AS calls_this_month
     FROM apps a
     JOIN team_members tm ON tm.team_id = a.team_id
     LEFT JOIN (
       SELECT app_id, COUNT(*)::integer AS calls_this_month
       FROM usage_rows
  ...
  return rows.map(r => ({
    ...r,
    calls_this_month: Number(r.calls_this_month || 0),
  }));
}
