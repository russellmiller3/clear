// ASH-1 Sweep — Agent Self-Heal A/B (session 45 / ROADMAP "Agent Self-Heal").
//
// Hypothesis from Browser Use's "Bitter Lesson of Agent Harnesses" essay
// (2026-04): Meph's pass rate will rise when we re-enable Claude Code's
// built-in Bash/Read/Edit/Write tools, because he can self-heal gaps in
// the 28 MCP tools (one-off grep/cat/find/quick-script workflows the
// wrappers can't express). Counter-hypothesis: the built-ins let Meph
// sidestep Factor DB instrumentation (e.g., grabbing Bash to curl an
// endpoint skips meph_http_request → no test_pass=1 write), which could
// blur the signal.
//
// Design mirrors the session 44 hint A/B (same registry, spawner,
// validateSweepPreconditions path, buildPrompt, driveTaskOnWorker). The
// only difference is which env var gets flipped per condition: instead
// of CLEAR_HINT_DISABLE, we flip GHOST_MEPH_CC_ALLOWED_TOOLS — empty
// string ('') forces MCP-only, "Bash,Read,Edit,Write" re-enables the
// built-ins. The cc-agent reads this var in buildClaudeStreamJsonSpawnArgs
// at spawn time (session 45 wiring).
//
// Design:
//   - 5 tasks spanning difficulty levels (L3–L7) so the result doesn't
//     depend on picking one sensitive task.
//   - 5 trials per condition per task = 50 trials total.
//   - Runs in TWO passes: (1) tools_off — GHOST_MEPH_CC_ALLOWED_TOOLS=''
//     (current baseline); (2) tools_on — GHOST_MEPH_CC_ALLOWED_TOOLS set
//     to Bash,Read,Edit,Write. Workers inherit env from this parent
//     process, so we mutate process.env between passes.
//   - Each trial's pass/fail comes from the Factor DB row-window grader
//     (same as curriculum-sweep).
//
// Usage:
//   MEPH_BRAIN=cc-agent GHOST_MEPH_CC_TOOLS=1 \
//     node playground/supervisor/ab-ash1-sweep.js \
//       --tasks=counter,todo-crud,auth-todo,contact-book,validated-forms \
//       --trials=5 --workers=1 --strict
//
// Wall-clock estimate (workers=1, 50 trials × ~140s avg): ~120 min.
// Wall-clock estimate (workers=2): ~60 min (some parallel grader
// over-count risk — interpret cautiously or re-run serial to confirm).
...
/**
 * Aggregate trial results into {taskId → {tools_on, tools_off, lift}} buckets.
 * Each bucket has { trials, passes, passRate }. Lift is the signed delta
 * (tools_on − tools_off) pass rate.
 *
...
export function summarizeAbResults(results) {
  ...
  for (const r of results) {
    if (!byTask[r.taskId]) byTask[r.taskId] = { tools_on: null, tools_off: null };
  ...
  for (const taskId of Object.keys(byTask)) {
    for (const cond of ['tools_on', 'tools_off']) {
    ...
    const on = byTask[taskId].tools_on;
    const off = byTask[taskId].tools_off;
    if (on && off) {
...
export function formatSummaryTable(summary) {
  const rows = [];
  rows.push('| task               | tools_on   | tools_off  | lift    | avg_on  | avg_off |');
  rows.push('|--------------------|------------|------------|---------|---------|---------|');
  for (const taskId of Object.keys(summary)) {
    const on = summary[taskId].tools_on || { passes: 0, trials: 0, passRate: 0, avgElapsedMs: 0 };
    const off = summary[taskId].tools_off || { passes: 0, trials: 0, passRate: 0, avgElapsedMs: 0 };
 ...
 */
async function runCondition({ trials, workers, timeoutMs, toolsEnabled, strict, factorDB }) {
  // Mutate process.env so WorkerSpawner's `env: { ...process.env }` picks
  // up the right flag. Restored in finally{}.
  // NOTE: we set the var in BOTH branches (instead of delete + set). Empty
  // string is the canonical "disable built-ins" value that cc-agent reads
  // in buildClaudeStreamJsonSpawnArgs; leaving the var unset risks picking
  // up an ambient value from Russell's shell profile.
  const origValue = process.env.GHOST_MEPH_CC_ALLOWED_TOOLS;
  process.env.GHOST_MEPH_CC_ALLOWED_TOOLS = toolsEnabled ? 'Bash,Read,Edit,Write' : '';
  ...
  const results = [];
  try {
    console.log(`\n  Spawning ${workers} workers (GHOST_MEPH_CC_ALLOWED_TOOLS="${process.env.GHOST_MEPH_CC_ALLOWED_TOOLS}")...`);
    ...
    registry.close();
    try { unlinkSync(AB_REGISTRY_PATH); } catch {}
    if (origValue === undefined) delete process.env.GHOST_MEPH_CC_ALLOWED_TOOLS;
    else process.env.GHOST_MEPH_CC_ALLOWED_TOOLS = origValue;
  }
 ...
 */
export async function runAsh1Sweep({
  taskIds = ['counter', 'todo-crud', 'auth-todo', 'contact-book', 'validated-forms'],
  ...
  console.log('\n=== ASH-1 Sweep (Agent Self-Heal A/B) ===');
  ...
  console.log(`  Conditions: tools_off (MCP-only) vs tools_on (Bash,Read,Edit,Write re-enabled)`);

  if (dryRun) {
    const offTrials = expandTrials(taskIds, trialsPerCondition, 'tools_off');
    const onTrials = expandTrials(taskIds, trialsPerCondition, 'tools_on');
    console.log(`\n[DRY RUN] Would run ${offTrials.length} + ${onTrials.length} trials.`);
  ...
  let allResults = [];
  try {
    // Condition 1: tools_off (current MCP-only baseline)
    console.log('\n── Condition: tools_off (GHOST_MEPH_CC_ALLOWED_TOOLS="") ──');
    const offResults = await runCondition({
      trials: expandTrials(taskIds, trialsPerCondition, 'tools_off'),
      workers,
      timeoutMs,
      toolsEnabled: false,
      strict,
    ...
    // Condition 2: tools_on (Claude Code built-ins re-enabled)
    console.log('\n── Condition: tools_on (GHOST_MEPH_CC_ALLOWED_TOOLS="Bash,Read,Edit,Write") ──');
    const onResults = await runCondition({
      trials: expandTrials(taskIds, trialsPerCondition, 'tools_on'),
      workers,
      timeoutMs,
      toolsEnabled: true,
      strict,
...
if (_thisFile === _entryFile) {
  ...
  const opts = {
    taskIds: tasksArg ? tasksArg.split('=')[1].split(',') : ['counter', 'todo-crud', 'auth-todo', 'contact-book', 'validated-forms'],
    trialsPerCondition: trialsArg ? parseInt(trialsArg.split('=')[1]) : 5,
  ...
  runAsh1Sweep(opts)
    .then(() => process.exit(0))
    .catch(err => {
      console.error('ASH-1 sweep failed:', err.message);
      process.exit(1);
    });
}
