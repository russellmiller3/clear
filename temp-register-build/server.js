...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"194":3,"203":9,"204":9,"210":10,"213":15,"214":15,"215":16} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, name*, age (line 3)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 3, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, name: { required: true, type: "text" }, age: { type: "text" } } },
  },
  endpoints: {
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // clear:10
    const _vErrs = _validate(req.body, [{"field":"email","type":"text","required":true,"matches":"email"}, {"field":"password","type":"text","required":true,"min":8}, {"field":"name","type":"text","required":true,"max":100}, {"field":"age","type":"number","min":13}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:15
    const saved = await _clearTry(() => db.insert('users', _pick(person, UsersSchema)), { op: 'insert', table: 'users', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
