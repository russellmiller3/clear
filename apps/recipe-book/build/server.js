...
//
// TABLES:
//   Recipes: title*, ingredients*, instructions*, prep_time, servings, created_at (line 5)
//
// ENDPOINTS:
//   GET /api/recipes (line 17)
//   POST /api/recipes (line 21)
//   DELETE /api/recipes/:id [auth] (line 31)
//
// PAGES:
//   'Recipe Book' at / (line 37)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    recipes: { line: 5, file: "main.clear", fields: { title: { required: true, type: "text" }, ingredients: { required: true, type: "text" }, instructions: { required: true, type: "text" }, prep_time: { type: "number" }, servings: { type: "number" }, created_at: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/recipes": { line: 17, file: "main.clear", auth: false },
    "POST /api/recipes": { line: 21, file: "main.clear", auth: false },
    "DELETE /api/recipes/:id": { line: 31, file: "main.clear", auth: true },
  }
} : null;
...
db.createTable('recipes', RecipesSchema);
// Backend
// clear:17 — GET /api/recipes
app.get('/api/recipes', async (req, res) => {
  try {
    const all_recipes = await db.findAll('recipes');
    return res.json(all_recipes);
  } catch (err) {
    console.error('[GET /api/recipes] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/recipes', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:21 — POST /api/recipes
app.post('/api/recipes', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const recipe_data = req.body;
    const incoming = req.params;
    const _vErr = _validate(req.body, [{"field":"title","type":"text","required":true,"min":1,"max":100}, {"field":"ingredients","type":"text","required":true}, {"field":"instructions","type":"text","required":true}, {"field":"prep_time","type":"number"}, {"field":"servings","type":"number"}]);
    if (_vErr) return res.status(400).json({ error: _vErr });
    const new_recipe = await _clearTry(() => db.insert('recipes', _pick(recipe_data, RecipesSchema)), { op: 'insert', table: 'recipes', line: 28, file: 'main.clear', source: "" }); // clear:28
    return res.status(201).json({ ...new_recipe, message: "success" });
  } catch (err) {
    console.error('[POST /api/recipes] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/recipes', line: 21, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:31 — DELETE /api/recipes/:id
app.delete('/api/recipes/:id', async (req, res) => {
  try {
    const incoming = req.params;
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    await _clearTry(() => db.remove('recipes', { id: incoming.id }), { op: 'remove', table: 'recipes', line: 33, file: 'main.clear', source: "" }); // clear:33
    return res.status(200).json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/recipes/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/recipes/:id', line: 31, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
