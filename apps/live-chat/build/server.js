...
//
// TABLES:
//   Messages: sender*, content*, room, created_at (line 5)
//   Rooms: name*!, description (line 11)
//
// ENDPOINTS:
//   GET /api/rooms (line 20)
//   POST /api/rooms (line 24)
//   GET /api/messages (line 31)
//   POST /api/messages (line 35)
//   DELETE /api/messages/:id [auth] (line 43)
//
// PAGES:
//   'Live Chat' at / (line 58)
//   'Rooms' at /rooms (line 74)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    messages: { line: 5, file: "main.clear", fields: { sender: { required: true, type: "text" }, content: { required: true, type: "text" }, room: { type: "text" }, created_at: { type: "timestamp" } } },
    rooms: { line: 11, file: "main.clear", fields: { name: { required: true, unique: true, type: "text" }, description: { type: "text" } } },
  },
  endpoints: {
    "GET /api/rooms": { line: 20, file: "main.clear", auth: false },
    "POST /api/rooms": { line: 24, file: "main.clear", auth: false },
    "GET /api/messages": { line: 31, file: "main.clear", auth: false },
    "POST /api/messages": { line: 35, file: "main.clear", auth: false },
    "DELETE /api/messages/:id": { line: 43, file: "main.clear", auth: true },
  }
} : null;
...
const MessagesSchema = {
  ...
  room: { type: "text", default: "general" },
...
db.createTable('rooms', RoomsSchema);
// Backend
// Rooms
// clear:20 — GET /api/rooms
app.get('/api/rooms', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/rooms', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:24 — POST /api/rooms
app.post('/api/rooms', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const room_data = req.body;
    const incoming = req.params;
    const _vErr = _validate(req.body, [{"field":"name","type":"text","required":true,"min":1,"max":50}]);
    if (_vErr) return res.status(400).json({ error: _vErr });
    const new_room = await _clearTry(() => db.insert('rooms', _pick(room_data, RoomsSchema)), { op: 'insert', table: 'rooms', line: 27, file: 'main.clear', source: "" }); // clear:27
    return res.status(201).json({ ...new_room, message: "success" });
  } catch (err) {
    console.error('[POST /api/rooms] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/rooms', line: 24, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/messages', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/messages', line: 31, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:35 — POST /api/messages
app.post('/api/messages', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const message_data = req.body;
    const incoming = req.params;
    const _vErr = _validate(req.body, [{"field":"sender","type":"text","required":true,"min":1,"max":50}, {"field":"content","type":"text","required":true,"min":1,"max":1000}, {"field":"room","type":"text"}]);
    if (_vErr) return res.status(400).json({ error: _vErr });
    const new_message = await _clearTry(() => db.insert('messages', _pick(message_data, MessagesSchema)), { op: 'insert', table: 'messages', line: 40, file: 'main.clear', source: "" }); // clear:40
    return res.status(201).json({ ...new_message, message: "success" });
  } catch (err) {
    console.error('[POST /api/messages] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/messages', line: 35, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:43 — DELETE /api/messages/:id
app.delete('/api/messages/:id', async (req, res) => {
  try {
    const incoming = req.params;
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    await _clearTry(() => db.remove('messages', { id: incoming.id }), { op: 'remove', table: 'messages', line: 45, file: 'main.clear', source: "" }); // clear:45
    return res.status(200).json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/messages/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/messages/:id', line: 43, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// Real-time: stream new messages via SSE
app.get('/stream', (req, res) => {
  ...
  (async () => {
    const all_messages = await db.findAll('messages');
    res.write(`data: ${JSON.stringify(all_messages)}\n\n`);
  })();
});
// Background: clean up old messages every hour
