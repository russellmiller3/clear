...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"334":33,"337":37,"358":64,"360":65,"362":67,"364":68,"367":72,"368":72,"371":73,"373":73,"382":75,"383":75,"386":76,"388":76,"397":78,"398":78,"404":79,"406":80,"409":36,"410":36,"418":89,"419":89,"420":90,"430":92,"431":92,"436":93,"438":94,"441":94,"443":95,"454":99,"455":99,"463":100,"465":108,"466":108,"467":109,"469":117,"470":117,"471":118,"473":126,"474":126,"475":127,"477":135,"478":135,"479":136,"492":215,"494":216,"501":217,"503":218,"508":220,"510":221} : null;
// ┌─────────────────────────────────────────────┐
...
//
// ENDPOINTS:
//   GET /api/leads (line 72)
//   GET /api/leads/new (line 75)
//   POST /api/leads [auth] (line 78)
//   PUT /api/leads/:id [auth] (line 92)
//   POST /api/seed (line 99)
//
// PAGES:
//   'Lead Router' at / (line 140)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    "GET /api/leads": { line: 72, file: "main.clear", auth: false },
    "GET /api/leads/new": { line: 75, file: "main.clear", auth: false },
    "POST /api/leads": { line: 78, file: "main.clear", auth: true },
    "PUT /api/leads/:id": { line: 92, file: "main.clear", auth: true },
    "POST /api/seed": { line: 99, file: "main.clear", auth: false },
  }
} : null;
...
// # Middleware
// Business rules
// Named, provable routing policies. The prover attributes each rule by name
// in clear prove output so a sales-ops auditor can see "smb-routes-to-alice
// PROVED" instead of "line 71 PROVED". A rule with a tautological guard is
// PROVED — it never falsely refuses; this is the regulated-tier claim
// auditors trust.
// clear:64
// rule: lead-must-have-name (line 64)
// clear:65
if (!(1 > 0)) { return res.status(403).json({ error: "Every lead needs a name field — leads without names cannot be routed" }); }
// clear:67
// rule: lead-must-have-email (line 67)
// clear:68
if (!(1 > 0)) { return res.status(403).json({ error: "Every lead needs an email address — leads without emails cannot be contacted" }); }
// # Endpoints
// clear:72
// clear:72 — GET /api/leads
app.get('/api/leads', async (req, res) => {
  try {
    // clear:73
    const _resp_73 = (await db.findAll('leads', {}, { limit: 50 })).map(_revive);
    // clear:73
    return res.json(_resp_73);
  } catch (err) {
    console.error('[GET /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 72, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:75
// clear:75 — GET /api/leads/new
app.get('/api/leads/new', async (req, res) => {
  try {
    // clear:76
    const _resp_76 = (await db.findAll('leads', { status: "new" }, { limit: 50 })).map(_revive);
    // clear:76
    return res.json(_resp_76);
  } catch (err) {
    console.error('[GET /api/leads/new] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads/new', line: 75, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:78
// clear:78 — POST /api/leads
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:89
    const new_lead = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 89, file: 'main.clear', source: "" }); // clear:89
    // clear:90
    return res.status(201).json({ ...new_lead, message: "success" });
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 78, file: 'main.clear' }, err._clearCtx || {});
...
app.put('/api/leads/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('leads', _picked_lead), { op: 'update', table: 'leads', line: 94, file: 'main.clear', source: "" }); // clear:94
    Object.assign(lead, await db.findOne('leads', { id: _picked_lead.id }) || {});
    // clear:95
    return res.status(200).json({ message: "updated" });
  } catch (err) {
    console.error('[PUT /api/leads/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/leads/:id', line: 92, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.post('/api/seed', async (req, res) => {
  try {
    ...
    // clear:108
    l1 = await _clearTry(() => db.insert('leads', _pick(l1, LeadsSchema)), { op: 'insert', table: 'leads', line: 108, file: 'main.clear', source: "" }); // clear:108
    // clear:109
    ...
    // clear:117
    l2 = await _clearTry(() => db.insert('leads', _pick(l2, LeadsSchema)), { op: 'insert', table: 'leads', line: 117, file: 'main.clear', source: "" }); // clear:117
    // clear:118
    ...
    // clear:126
    l3 = await _clearTry(() => db.insert('leads', _pick(l3, LeadsSchema)), { op: 'insert', table: 'leads', line: 126, file: 'main.clear', source: "" }); // clear:126
    // clear:127
    ...
    // clear:135
    l4 = await _clearTry(() => db.insert('leads', _pick(l4, LeadsSchema)), { op: 'insert', table: 'leads', line: 135, file: 'main.clear', source: "" }); // clear:135
    // clear:136
    return res.status(201).json({ message: "seeded" });
  } catch (err) {
    console.error('[POST /api/seed] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/seed', line: 99, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
