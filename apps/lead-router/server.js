...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"322":21,"325":25,"341":45,"342":45,"345":46,"347":46,"356":48,"357":48,"360":49,"362":49,"371":51,"372":51,"378":52,"381":56,"385":58,"389":60,"393":62,"394":62,"395":63,"405":65,"406":65,"411":66,"413":67,"416":67,"418":68,"429":72,"430":72,"438":73,"440":81,"441":81,"442":82,"444":90,"445":90,"446":91,"448":99,"449":99,"450":100,"452":108,"453":108,"454":109,"467":141,"469":142,"476":143,"478":144,"483":146,"485":147} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Leads: name*, email*, company, size, source, assigned_to, status, created_at_date (line 25)
//
// ENDPOINTS:
//   GET /api/leads (line 45)
//   GET /api/leads/new (line 48)
//   POST /api/leads (line 51)
//   PUT /api/leads/:id [auth] (line 65)
//   POST /api/seed (line 72)
//
// PAGES:
//   'Lead Router' at / (line 113)
//
...
// Auth scaffolding from `allow signup and login` (clear:40)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    leads: { line: 25, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, company: { type: "text" }, size: { type: "text" }, source: { type: "text" }, assigned_to: { type: "text" }, status: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/leads": { line: 45, file: "main.clear", auth: false },
    "GET /api/leads/new": { line: 48, file: "main.clear", auth: false },
    "POST /api/leads": { line: 51, file: "main.clear", auth: false },
    "PUT /api/leads/:id": { line: 65, file: "main.clear", auth: true },
...
// # Endpoints
// clear:45
// clear:45 — GET /api/leads
app.get('/api/leads', async (req, res) => {
  try {
    // clear:46
    const _resp_46 = (await db.findAll('leads', {}, { limit: 50 })).map(_revive);
    // clear:46
    return res.json(_resp_46);
  } catch (err) {
    console.error('[GET /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 45, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:48
// clear:48 — GET /api/leads/new
app.get('/api/leads/new', async (req, res) => {
  try {
    // clear:49
    const _resp_49 = (await db.findAll('leads', { status: "new" }, { limit: 50 })).map(_revive);
    // clear:49
    return res.json(_resp_49);
  } catch (err) {
    console.error('[GET /api/leads/new] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads/new', line: 48, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:51
// clear:51 — POST /api/leads
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:62
    const new_lead = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 62, file: 'main.clear', source: "" }); // clear:62
    // clear:63
    return res.status(201).json({ ...new_lead, message: "success" });
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 51, file: 'main.clear' }, err._clearCtx || {});
...
app.put('/api/leads/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('leads', _picked_lead), { op: 'update', table: 'leads', line: 67, file: 'main.clear', source: "" }); // clear:67
    Object.assign(lead, await db.findOne('leads', { id: _picked_lead.id }) || {});
    // clear:68
    return res.status(200).json({ message: "updated" });
  } catch (err) {
    console.error('[PUT /api/leads/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/leads/:id', line: 65, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.post('/api/seed', async (req, res) => {
  try {
    ...
    // clear:81
    await _clearTry(() => db.insert('leads', _pick(l1, LeadsSchema)), { op: 'insert', table: 'leads', line: 81, file: 'main.clear', source: "" }); // clear:81
    // clear:82
    ...
    // clear:90
    await _clearTry(() => db.insert('leads', _pick(l2, LeadsSchema)), { op: 'insert', table: 'leads', line: 90, file: 'main.clear', source: "" }); // clear:90
    // clear:91
    ...
    // clear:99
    await _clearTry(() => db.insert('leads', _pick(l3, LeadsSchema)), { op: 'insert', table: 'leads', line: 99, file: 'main.clear', source: "" }); // clear:99
    // clear:100
    ...
    // clear:108
    await _clearTry(() => db.insert('leads', _pick(l4, LeadsSchema)), { op: 'insert', table: 'leads', line: 108, file: 'main.clear', source: "" }); // clear:108
    // clear:109
