...
**OWASP Top 10 by construction — load-bearing piece SHIPPED 2026-05-05; finishing cycles + Pieces 2-5 remain:**
- **OWASP Piece 1 — mandatory per-line access rules. Cycles 5+6+3+4 SHIPPED 2026-05-05** on `feature/owasp-1-cycle-5-creator-filter` (held for push). The compiler now auto-injects the per-row creator filter on every CRUD operation against a creator-scoped table — read, save, update, delete — across both JS and Python backends. Runtime auto-adds the `user_id` INTEGER column to every SQLite table (mirror of `tenant_id` precedent). Validator errors when a row-role rule names a missing field; IDOR-on-GET is locked as a hard error with a regression test. 11 commits, 12 new tests, 2952/2952 green. Marcus pitch can now claim "Clear refuses to compile any of the OWASP Top 10" with no asterisks. Remaining: cycle 2b (sweep ~335 test fixtures and flip the missing-rules warning to a hard error — risky; deferred to a focused session) plus the rest of the doc cascade (USER-GUIDE worked example, FAQ entries, intent.md row, landing/*.html).
- **OWASP Piece 2 — SSRF allowlist.** New top-of-file declaration `allow outgoing requests to: 'api.stripe.com', ...`. Build fails if `http_request` URL host isn't covered by the allowlist. Pattern matches the existing Slack `require channel allowlist` at `parser.js:4509`. Estimate: half a day.
- **OWASP Piece 3 — Sensitive field tag.** `text ssn sensitive`. Encrypts at rest using `SENSITIVE_KEY` env var, extends existing redact() at `compiler.js:488`, strips from API responses unless URL is tagged `can return sensitive data`. Estimate: half-to-one day.
- **OWASP Piece 4 — Auto login rate limiting.** Promote validator.js:1939 warning to auto-emit `rate limit 10 per minute` on the login URL when `allow signup and login` is declared. Override: `allow N login attempts per minute`. Estimate: a couple hours.
- **OWASP Piece 5 — Hardcoded secrets linter.** New `clear lint` rule that regex-matches against known key shapes (sk-..., AKIA..., ghp_..., long base64 tokens) and fails the build with a suggestion to use env vars. Estimate: a couple hours.
