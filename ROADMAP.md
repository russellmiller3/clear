...
| PERF-4 | **No virtual scrolling** | `display X as table` renders every row into the DOM. 1,000 rows = 1,000 DOM elements. | Only render visible rows + buffer. Intersection Observer or fixed-height virtualization. | Lower priority — pagination (PERF-1) already solves most cases. Virtual scroll is the polish layer. |
| PERF-5 | **`get all` with explicit `page N, M per page` still fetches all** | Even with pagination set, compiler emits `findAll(...)` then slices client-side. | Push `LIMIT ? OFFSET ?` into the SQL. Uses the new `options.limit` + add `options.offset`. | One more fix to make explicit pagination truly server-side. |
