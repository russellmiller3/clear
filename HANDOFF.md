...
1. **Real hint-lift measurement on a harder task.** The diagnostic sweep `bx0ofh7h9` kicked off this session ran clean — 20/20 trials passed, cc-agent did NOT die, no Piece 4 fix needed. BUT the lift on counter (L3) was +0.0% because both conditions hit 100% — the task is too easy to differentiate. To get a meaningful "Meph improves with data" number for RESEARCH.md, run `--tasks=todo-crud --trials=20 --workers=1 --strict` (or higher difficulty + larger sample). Counter at L3 doesn't move the needle in either direction; todo-crud at L4 was the morning sweep's intent and was where cc-agent died before reaching the data. **Why for launch:** the load-bearing claim "Meph gets smarter as data grows" needs a live production lift number, and counter alone can't produce one.
