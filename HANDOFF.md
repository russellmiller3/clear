...
1. **Remove the back-compat path for old `enforce that X or 'msg'` syntax.** The new `enforce that X, or fail with error message: 'msg'` form shipped 2026-05-03 with the parser accepting BOTH forms during transition. The bulk rewrite already moved every `.clear` app and standalone source file to the new form. The remaining old-form occurrences live in (a) `clear.test.js` inline strings — about 20 of them, embedded in JS string literals, missed by the line-based rewrite script; (b) `lib/prover/index.js` reason text (2 occurrences in error messages); (c) a few doc explanations (`AI-INSTRUCTIONS.md`, `USER-GUIDE.md`, `learnings.md`, `playground/system-prompt.md`) that may be intentional teaching examples. Plan: walk every `clear.test.js` inline string with a regex that handles JS-string-escape semantics, rewrite those to new form, then strip the back-compat fallback from `parser.js`'s `enforce_that` handler (the old `for k = ctx.tokens.length-1 ... ctx.tokens[k-1].canonical === 'or'` block). Verify the full suite stays at 2899/2899 + eval 35/35 + runtime witness 4/4. ~15-20 min. **Why deferred:** wanted Russell's eyes on the partial state before stripping back-compat, since stripping breaks any `.clear` source still using old form, anywhere on the system.
