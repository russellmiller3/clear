...
1. **Validate or retire the "Meph improves with data" claim.** Two sweeps this session produced honest live data: counter L3 = both conditions 100% (no signal), approval-queue L7 = -20% lift (1 fewer pass out of 5, noise), kpi-dashboard L7 = 0% lift but +27s elapsed penalty with hints. Across 3 tasks, ZERO trials show hint_on outperforming hint_off. 5 trials per condition is too small for statistical significance, but the direction of every signal is flat-or-negative. **The data-flywheel claim has no positive live-production evidence as of 2026-04-29 evening.** Three paths: (a) run a much bigger sweep — 20+ trials × 5+ tasks at varied difficulty (3-6 hr wall clock) before believing either way; (b) inspect the specific hint that fired on the failed approval-queue trial to find if the retriever is surfacing wrong patterns; (c) acknowledge the flywheel claim is research-in-progress and drop it from cold-pitch Marcus DMs until positive-lift evidence exists. **Why for launch:** "Meph gets smarter as data grows" sits prominently in marketing copy and the elevator pitch — putting unverified claims in front of paying customers is reputational risk. Either prove it or pull it.
