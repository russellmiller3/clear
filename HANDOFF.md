...
1. **Audit existing `async () =>` tests in `clear.test.js` and migrate to `itAsync`.** The proper-awaiting helpers (`describeAsync` + `itAsync`) shipped 2026-05-03 in `lib/testUtils.js` and the runtime-witness harness already uses them as the worked example. Hundreds of `async () =>` tests in `clear.test.js` still use the sync `it()` — most happen to work because their bodies are sync underneath, but any with a real `await` (spawn, fetch, sleep, real I/O) silently mis-counts. Audit shape: grep every `it('...', async () =>` in `clear.test.js`, classify each — pure-sync-body (no migration needed; rename optional), real-await-body (migrate to `await itAsync(...)` inside an `await describeAsync(...)`). Run the suite on a branch as you go; if any migrated test starts failing, that's a silent bug surfacing — fix or quarantine with a TODO. **Risk:** real failures in tests that were silently broken could break the pre-push gate. Migrate one describe block at a time so the diff stays reviewable.
