...
- `feature/landing-builders-page` — 3 commits. (1) The new `landing/builders.html` ranger / RevOps homepage (1174 lines) — hero, "wall you keep hitting" with Lovable/Bubble/Retool/Cursor by name, side-by-side React vs Clear, audience cards, case-card screenshots, AI-assistants section with helpdesk-agent code, secure-by-default with terminal mock, prove-your-rules section with audit-PDF mock, how-it-works, pricing teaser, dark CTA. Mirrors pricing.html visual system. (2) Studio Direct Edit toggle in `playground/ide.html` — toggle button next to Run/Stop, click any preview element to jump editor cursor + draft a "Help me edit this:" message in Meph chat. Verified end-to-end live (clicked rendered button on line 6 → chat input filled with the line's source). (3) Studio Bridge extension in `compiler.js` so Direct Edit also fires for full-stack apps (running-server iframes), not just srcdoc. Tests 2912/0. Ready to merge.
- `feature/prove-auto-check-gutter` — 1 commit, branched off main. Studio Prove redesign 4(a) v0: auto-prove badge in the toolbar that fires after every compile attempt, shows per-rule verdict counts (`Prove: 3 ok · 0 bad · 0 ?`), colour-codes by worst verdict (green / red / amber). Click the badge to expand a popover listing each rule with its verdict; click a row to jump the editor cursor to that source line. Pushed to GitHub. **Limitation called out:** the full inline editor-gutter integration originally specced for 4(a) needs CodeMirror's `gutter` / `GutterMarker` / `StateField` / `StateEffect` exports, which aren't in `playground/codemirror.bundle.js` today — that bundle rebuild is filed as a follow-up. This v0 ships the spell-check feel against existing exports. Ready to merge.
...
**Today's session totals (2026-05-04):** 3 commits on `feature/landing-builders-page` — the new ranger / RevOps homepage (1174 lines), the Studio Direct Edit toggle (62 lines added to ide.html, end-to-end live verified), and the Studio Bridge extension (31 lines added to compiler.js) so Direct Edit also fires for full-stack apps. Pre-commit tests 2912/0 green. Ready to merge.
