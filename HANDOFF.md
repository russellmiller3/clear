...
2. **Pre-existing playground bundle build is broken.** `npx esbuild index.js --bundle --format=esm --minify --outfile=playground/clear-compiler.min.js` fails with "Could not resolve fs/path/url" because `lib/packaging-cloudflare.js` imports node-only modules. Either add `--platform=node` (changes the bundle target), or split packaging-cloudflare out of the index.js graph for browser builds, or add `--external:fs --external:path --external:url`. Independent of any prover work — a stale playground bundle ships compiler changes invisibly to Studio users.
