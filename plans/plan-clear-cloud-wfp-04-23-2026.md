...
> [!NOTE] EXEC CORRECTION (cycle 4.4, 2026-04-22): the plan preamble ("Move pdf-parse/mammoth deps from deployed-app runtime into compile-time only (Studio dep)") assumes those packages are currently INSTALLED as runtime deps in `package.json`. They aren't — `compiler.js:509 _loadFileText` references them via `require('pdf-parse')` but they were never added to `package.json`, so `.pdf/.docx` knowledge silently fails today on the Node target too (the try/catch logs a warning and returns empty string). For Phase 4 cycle 4.4 this means:
>
> 1. Binary knowledge is NOT working anywhere today — so the "Workers target breaks it" framing is wrong; we're enabling it for the first time on Workers target.
> 2. Cycle 4.4 tests will SKIP cleanly if `pdf-parse` isn't available at test time rather than installing the dep across every dev's machine. The drift-guard assertions still run against any bundle produced — what we verify is that IF the PDF extracts, the Worker bundle has no `pdf-parse|mammoth|require(` references.
> 3. Phase 4 does NOT install `pdf-parse`/`mammoth`. That happens in a follow-up (documented in HANDOFF) so the Studio owner can decide whether to ship 25MB of binary-text-extraction code as a core dep.
