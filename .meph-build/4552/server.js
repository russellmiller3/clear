...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"151":2,"153":4,"159":7,"160":7,"163":8,"165":9,"174":11,"175":11,"178":12,"180":13,"181":13,"182":14,"184":15,"193":17,"194":17,"197":18,"198":18,"199":19} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counters: tick (line 4)
//
// ENDPOINTS:
//   GET /api/count (line 7)
//   POST /api/increment (line 11)
//   POST /api/reset (line 17)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 4, file: "main.clear", fields: { tick: { type: "number" } } },
  },
  endpoints: {
    "GET /api/count": { line: 7, file: "main.clear", auth: false },
    "POST /api/increment": { line: 11, file: "main.clear", auth: false },
    "POST /api/reset": { line: 17, file: "main.clear", auth: false },
  }
} : null;
...
// clear:7
// clear:7 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:8
    let total = await db.aggregate('counters', 'COUNT', 'tick', {});
    // clear:9
    return res.json({ count: total });
  } catch (err) {
    console.error('[GET /api/count] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/count', line: 7, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:11
// clear:11 — POST /api/increment
app.post('/api/increment', async (req, res) => {
  try {
    // clear:12
    let new_tick = { tick: 1 };
    // clear:13
    await _clearTry(() => db.insert('counters', _pick(new_tick, CountersSchema)), { op: 'insert', table: 'counters', line: 13, file: 'main.clear', source: "" }); // clear:13
    // clear:14
    let total = await db.aggregate('counters', 'COUNT', 'tick', {});
    // clear:15
    return res.json({ count: total });
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 11, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:17
// clear:17 — POST /api/reset
app.post('/api/reset', async (req, res) => {
  try {
    // clear:18
    await _clearTry(() => db.remove('counters', { tick: 1 }), { op: 'remove', table: 'counters', line: 18, file: 'main.clear', source: "" }); // clear:18
    // clear:19
    return res.json({ count: 0 });
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
