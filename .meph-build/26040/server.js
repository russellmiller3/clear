...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"179":16,"182":19,"192":26,"193":26,"199":27,"202":32,"203":32,"204":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, name*, age (line 19)
//
// ENDPOINTS:
//   POST /api/register (line 26)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 19, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, name: { required: true, type: "text" }, age: { type: "text" } } },
  },
  endpoints: {
    "POST /api/register": { line: 26, file: "main.clear", auth: false },
  }
} : null;
...
// Registration endpoint — validates input then saves new user
// clear:26
// clear:26 — POST /api/register
app.post('/api/register', async (req, res) => {
  try {
    ...
    // clear:32
    const saved = await _clearTry(() => db.insert('users', _pick(registration, UsersSchema)), { op: 'insert', table: 'users', line: 32, file: 'main.clear', source: "" }); // clear:32
    // clear:33
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/register] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/register', line: 26, file: 'main.clear' }, err._clearCtx || {});
