...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"169":3,"185":22,"194":28,"195":28,"201":29,"204":34,"205":34,"206":35} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, name*, age (line 22)
//
// ENDPOINTS:
//   POST /api/register (line 28)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 22, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, name: { required: true, type: "text" }, age: { type: "text" } } },
  },
  endpoints: {
    "POST /api/register": { line: 28, file: "main.clear", auth: false },
  }
} : null;
...
//   ┌──────────────────────▼──────────────────────┐
//   │ POST /api/register                          │
//   │ Validate → Save → Return 201 with id        │
//   │ Bad input → Return 400 with error details   │
//   └─────────────────────────────────────────────┘
// clear:22
// Data shape: Users
const UsersSchema = {
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // clear:34
    const saved = await _clearTry(() => db.insert('users', _pick(registration, UsersSchema)), { op: 'insert', table: 'users', line: 34, file: 'main.clear', source: "" }); // clear:34
    // clear:35
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/register] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/register', line: 28, file: 'main.clear' }, err._clearCtx || {});
