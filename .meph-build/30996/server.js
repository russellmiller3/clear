...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"174":3,"182":8,"183":8,"189":9,"192":12,"193":12,"194":13,"204":15,"205":15,"208":16,"210":17,"219":19,"220":19,"224":20,"226":20} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Posts: title*, body*, author (line 3)
//
// ENDPOINTS:
//   POST /api/posts (line 8)
//   GET /api/posts (line 15)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    posts: { line: 3, file: "main.clear", fields: { title: { required: true, type: "text" }, body: { required: true, type: "text" }, author: { type: "text" } } },
  },
  endpoints: {
    "POST /api/posts": { line: 8, file: "main.clear", auth: false },
    "GET /api/posts": { line: 15, file: "main.clear", auth: false },
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    // clear:12
    const saved = await _clearTry(() => db.insert('posts', _pick(article, PostsSchema)), { op: 'insert', table: 'posts', line: 12, file: 'main.clear', source: "" }); // clear:12
    // clear:13
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/posts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/posts', line: 8, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/posts', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/posts', line: 15, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
