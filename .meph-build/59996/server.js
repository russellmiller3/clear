...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"237":3,"239":7,"247":12,"255":17,"256":17,"259":18,"261":19,"263":20,"272":22,"273":22,"278":23,"280":24,"282":25,"284":26,"285":26,"286":27,"296":29,"297":29,"301":30,"303":31,"305":32,"306":32,"307":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, role* (line 7)
//   Articles: title*, body, author_id (line 12)
//
// ENDPOINTS:
//   GET /api/articles [auth] (line 17)
//   POST /api/articles [auth] (line 22)
//   DELETE /api/articles/:id [auth] (line 29)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 7, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, role: { required: true, type: "text" } } },
    articles: { line: 12, file: "main.clear", fields: { title: { required: true, type: "text" }, body: { type: "text" }, author_id: { type: "fk" } } },
  },
  endpoints: {
    "GET /api/articles": { line: 17, file: "main.clear", auth: true },
    "POST /api/articles": { line: 22, file: "main.clear", auth: true },
    "DELETE /api/articles/:id": { line: 29, file: "main.clear", auth: true },
  }
} : null;
...
db.createTable('articles', ArticlesSchema);
// clear:17
// clear:17 — GET /api/articles
app.get('/api/articles', async (req, res) => {
  try {
    // clear:18
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:19
    const articles = (await db.findAll('articles', {}, { limit: 50 })).map(_revive);
    // clear:20
    return res.json(articles);
  } catch (err) {
    console.error('[GET /api/articles] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/articles', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:22
// clear:22 — POST /api/articles
app.post('/api/articles', async (req, res) => {
  try {
    ...
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:24
    if (!(req.user?.role != "viewer")) { return res.status(403).json({ error: "Only editors and admins can create articles" }); }
    // clear:25
    article.author_id = req.user?.id;
    // clear:26
    const saved = await _clearTry(() => db.insert('articles', _pick(article, ArticlesSchema)), { op: 'insert', table: 'articles', line: 26, file: 'main.clear', source: "" }); // clear:26
    // clear:27
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/articles] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/articles', line: 22, file: 'main.clear' }, err._clearCtx || {});
...
// clear:29
// clear:29 — DELETE /api/articles/:id
app.delete('/api/articles/:id', async (req, res) => {
  try {
    ...
    // clear:32
    await _clearTry(() => db.remove('articles', { id: incoming?.id }), { op: 'remove', table: 'articles', line: 32, file: 'main.clear', source: "" }); // clear:32
    // clear:33
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/articles/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/articles/:id', line: 29, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
