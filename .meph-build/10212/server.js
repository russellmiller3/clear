...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"150":3,"160":10,"161":10,"166":11,"168":12,"172":14,"173":14,"174":15,"184":17,"185":17,"188":18,"190":18} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/bookings": { line: 17, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    // clear:11
    let conflict = await db.aggregate('bookings', 'COUNT', 'id', { date: booking?.date, time: booking?.time });
    // clear:12
    if (conflict > 0) {
      return res.status(400).json({ message: "Slot already booked" });
    }
    // clear:14
    const saved = await _clearTry(() => db.insert('bookings', _pick(booking, BookingsSchema)), { op: 'insert', table: 'bookings', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
...
// clear:17
// clear:17 — GET /api/bookings
app.get('/api/bookings', async (req, res) => {
  try {
    // clear:18
    const _resp_18 = (await db.findAll('bookings', {}, { limit: 50 })).map(_revive);
    // clear:18
    return res.json(_resp_18);
  } catch (err) {
    console.error('[GET /api/bookings] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/bookings', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
