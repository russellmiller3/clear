...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"150":5,"159":11,"163":12,"177":13,"179":14,"180":14,"185":16,"186":16,"189":17,"191":18} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   WeatherReadings: temp, humidity, conditions, fetched_at_date (line 5)
//
// ENDPOINTS:
//   GET /api/readings (line 16)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    weatherreadings: { line: 5, file: "main.clear", fields: { temp: { type: "number" }, humidity: { type: "number" }, conditions: { type: "text" }, fetched_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/readings": { line: 16, file: "main.clear", auth: false },
  }
} : null;
...
// clear:5
// Data shape: WeatherReadings
const WeatherReadingsSchema = {
  temp: { type: "number" },
  humidity: { type: "number" },
  conditions: { type: "text" },
  fetched_at_date: { type: "timestamp", auto: true }
};
db.createTable('weatherreadings', WeatherReadingsSchema);
// clear:11
// Scheduled: every 1 hour(s)
setInterval(async () => {
  try {
    // clear:12
    let reading = await (async () => {
    const _ctrl = new AbortController();
    const _timer = setTimeout(() => _ctrl.abort(), 30000);
    try {
      const _res = await fetch("https://api.weather-mock.example.com/current?city=SF", { method: 'GET', signal: _ctrl.signal });
      if (!_res.ok) { const _e = new Error(`External API error: ${_res.status} ${_res.statusText}`); _e._clearCtx = { service: 'external', line: 12, file: 'main.clear', source: 'call api' }; throw _e; }
      const _ct = _res.headers.get("content-type") || "";
      return _ct.includes("json") ? _res.json() : _res.text();
    } catch (_err) {
      if (!_err._clearCtx) { _err._clearCtx = { service: 'external', line: 12, file: 'main.clear', source: 'call api' }; }
      throw _err;
    } finally { clearTimeout(_timer); }
  })();
    // clear:13
    let new_reading = { temp: reading?.temp, humidity: reading?.humidity, conditions: reading?.conditions };
    // clear:14
    await _clearTry(() => db.insert('weatherreadings', _pick(new_reading, WeatherReadingsSchema)), { op: 'insert', table: 'weatherreadings', line: 14, file: 'main.clear', source: "" }); // clear:14
  } catch (_err) {
    console.error('Scheduled task error:', _err);
  }
}, 3600000);
// clear:16
// clear:16 — GET /api/readings
app.get('/api/readings', async (req, res) => {
  try {
    // clear:17
    const readings = (await db.findAll('weatherreadings', {}, { limit: 50 })).map(_revive);
    // clear:18
    return res.json(readings);
  } catch (err) {
    console.error('[GET /api/readings] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/readings', line: 16, file: 'main.clear' }, err._clearCtx || {});
