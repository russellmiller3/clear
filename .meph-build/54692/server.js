...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"259":3,"261":5,"268":9,"277":17,"278":17,"284":18,"287":21,"288":21,"289":22,"299":24,"300":24,"303":25,"305":26,"307":27,"309":28,"311":29,"313":30,"322":32,"323":32,"326":33,"328":34,"330":35} : null;
// ┌─────────────────────────────────────────────┐
...
//   Events: page_url*, event_type*, user_id, timestamp (line 9)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    ...
    events: { line: 9, file: "main.clear", fields: { page_url: { required: true, type: "text" }, event_type: { required: true, type: "text" }, user_id: { type: "fk" }, timestamp: { type: "text" } } },
  },
  endpoints: {
...
app.post('/api/events', async (req, res) => {
  try {
    ...
    // clear:18
    const _vErrs = _validate(req.body, [{"field":"page_url","type":"text"}, {"field":"event_type","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:21
    await _clearTry(() => db.update('events', _pick(event, EventsSchema)), { op: 'update', table: 'events', line: 21, file: 'main.clear', source: "" }); // clear:21
    // clear:22
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    // clear:26
    let total = await db.aggregate('events', 'COUNT', 'id', {});
    // clear:27
    let pageviews = await db.aggregate('events', 'COUNT', 'id', { event_type: "pageview" });
    // clear:28
    let clicks = await db.aggregate('events', 'COUNT', 'id', { event_type: "click" });
    // clear:29
    let signups = await db.aggregate('events', 'COUNT', 'id', { event_type: "signup" });
    // clear:30
    return res.json({ total_events: total, by_type: { pageview: pageviews, click: clicks, signup: signups } });
  } catch (err) {
...
app.get('/api/stats/daily', async (req, res) => {
  try {
    ...
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:34
    const events = (await db.findAll('events', {}, { limit: 50 })).map(_revive);
    // clear:35
