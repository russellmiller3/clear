...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"264":3,"277":13,"279":16,"280":16,"286":17,"289":20,"290":20,"291":21,"301":23,"302":23,"305":24,"307":24,"316":26,"317":26,"320":27,"322":28,"331":30,"332":30,"337":31,"339":32,"342":32,"344":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Requests: title*, description, category, priority, submitter*, status, assigned_to, resolution_notes (line 3)
//
// ENDPOINTS:
//   POST /api/requests (line 16)
//   GET /api/requests (line 23)
//   GET /api/requests/new (line 26)
//   PUT /api/requests/:id [auth] (line 30)
//
// PAGES:
//   'Submit Request' at / (line 35)
//   'Ops Dashboard' at /ops (line 46)
//
...
// Auth scaffolding from `allow signup and login` (clear:14)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    requests: { line: 3, file: "main.clear", fields: { title: { required: true, type: "text" }, description: { type: "text" }, category: { type: "text" }, priority: { type: "text" }, submitter: { required: true, type: "text" }, status: { type: "text" }, assigned_to: { type: "text" }, resolution_notes: { type: "text" } } },
  },
  endpoints: {
    "POST /api/requests": { line: 16, file: "main.clear", auth: false },
    "GET /api/requests": { line: 23, file: "main.clear", auth: false },
    "GET /api/requests/new": { line: 26, file: "main.clear", auth: false },
    "PUT /api/requests/:id": { line: 30, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/requests', async (req, res) => {
  try {
    ...
    // clear:20
    const saved = await _clearTry(() => db.insert('requests', _pick(request_data, RequestsSchema)), { op: 'insert', table: 'requests', line: 20, file: 'main.clear', source: "" }); // clear:20
    // clear:21
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/requests', line: 16, file: 'main.clear' }, err._clearCtx || {});
...
// clear:23
// clear:23 — GET /api/requests
app.get('/api/requests', async (req, res) => {
  try {
    // clear:24
    const _resp_24 = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:24
    return res.json(_resp_24);
  } catch (err) {
    console.error('[GET /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/requests', line: 23, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:26
// clear:26 — GET /api/requests/new
app.get('/api/requests/new', async (req, res) => {
  try {
    // clear:27
    const new_requests = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:28
    return res.json(new_requests);
  } catch (err) {
    console.error('[GET /api/requests/new] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/requests/new', line: 26, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:30
// clear:30 — PUT /api/requests/:id
app.put('/api/requests/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('requests', _picked_changes), { op: 'update', table: 'requests', line: 32, file: 'main.clear', source: "" }); // clear:32
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/requests/:id', line: 30, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
