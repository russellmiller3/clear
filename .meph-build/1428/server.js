...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"172":3,"174":5,"177":7,"183":10,"184":10,"190":11,"193":13,"194":13,"195":14,"205":16,"206":16,"209":17,"211":18} : null;
// ┌─────────────────────────────────────────────┐
...
//
// ENDPOINTS:
//   POST /api/items (line 10)
//   GET /api/items (line 16)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    "POST /api/items": { line: 10, file: "main.clear", auth: false },
    "GET /api/items": { line: 16, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/items', async (req, res) => {
  try {
    ...
    // clear:13
    const saved = await _clearTry(() => db.insert('items', _pick(entry, ItemsSchema)), { op: 'insert', table: 'items', line: 13, file: 'main.clear', source: "" }); // clear:13
    // clear:14
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/items] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/items', line: 10, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/items', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/items', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
