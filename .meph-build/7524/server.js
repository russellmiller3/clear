...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"152":3,"154":5,"160":8,"161":8,"164":9,"166":10,"175":12,"176":12,"179":13,"181":14,"182":14,"183":15,"185":16,"194":18,"195":18,"198":19,"200":20,"204":22} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counters: step (line 5)
//
// ENDPOINTS:
//   GET /api/count (line 8)
//   POST /api/increment (line 12)
//   POST /api/reset (line 18)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 5, file: "main.clear", fields: { step: { type: "number" } } },
  },
  endpoints: {
    "GET /api/count": { line: 8, file: "main.clear", auth: false },
    "POST /api/increment": { line: 12, file: "main.clear", auth: false },
    "POST /api/reset": { line: 18, file: "main.clear", auth: false },
  }
} : null;
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    let total = await db.aggregate('counters', 'COUNT', 'id', {});
    // clear:10
...
// clear:12
// clear:12 — POST /api/increment
app.post('/api/increment', async (req, res) => {
  try {
    // clear:13
    let new_entry = { step: 1 };
    // clear:14
    await _clearTry(() => db.insert('counters', _pick(new_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    let total = await db.aggregate('counters', 'COUNT', 'id', {});
    // clear:16
    return res.json({ count: total });
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 12, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:18
// clear:18 — POST /api/reset
app.post('/api/reset', async (req, res) => {
  try {
    // clear:19
    const items = (await db.findAll('counters')).map(_revive);
    // clear:20
    for (const item of items) {
      Counters = Counters.filter(_item => _item !== item);
    }
    // clear:22
    return res.json({ count: 0 });
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
