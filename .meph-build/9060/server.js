...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"172":3,"178":12,"187":18,"188":18,"194":19,"197":22,"198":22,"199":23,"209":25,"210":25,"213":26,"215":26} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Contacts: name*, email*!, phone, notes (line 12)
//
// ENDPOINTS:
//   POST /api/contacts (line 18)
//   GET /api/contacts (line 25)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    contacts: { line: 12, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, unique: true, type: "text" }, phone: { type: "text" }, notes: { type: "text" } } },
  },
  endpoints: {
    "POST /api/contacts": { line: 18, file: "main.clear", auth: false },
    "GET /api/contacts": { line: 25, file: "main.clear", auth: false },
  }
} : null;
...
//   Contact Book API
//   Tables: Contacts
//   Endpoints: POST /api/contacts (create), GET /api/contacts (list)
//   Validation: name required, email format + uniqueness
// clear:12
// Data shape: Contacts
...
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    // clear:22
    const saved = await _clearTry(() => db.insert('contacts', _pick(contact, ContactsSchema)), { op: 'insert', table: 'contacts', line: 22, file: 'main.clear', source: "" }); // clear:22
    // clear:23
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/contacts', line: 18, file: 'main.clear' }, err._clearCtx || {});
...
// clear:25
// clear:25 — GET /api/contacts
app.get('/api/contacts', async (req, res) => {
  try {
    // clear:26
    const _resp_26 = (await db.findAll('contacts', {}, { limit: 50 })).map(_revive);
    // clear:26
    return res.json(_resp_26);
  } catch (err) {
    console.error('[GET /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 25, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
