...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"150":3,"158":8,"163":9,"165":10,"181":12,"182":12,"185":13,"187":13,"196":15,"197":15,"202":16,"203":16,"204":17} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Logs: message*, level, created_at_date (line 3)
//
// ENDPOINTS:
//   GET /api/logs (line 12)
//   POST /api/logs (line 15)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    logs: { line: 3, file: "main.clear", fields: { message: { required: true, type: "text" }, level: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/logs": { line: 12, file: "main.clear", auth: false },
    "POST /api/logs": { line: 15, file: "main.clear", auth: false },
  }
} : null;
...
(function _scheduleDailyAt2_0() {
  const _runAt = async () => {
    try {
      // clear:9
      const old_logs = (await db.findAll('logs', (r) => created_at_date < 30, { limit: 50 })).map(_revive);
      // clear:10
      Logs = Logs.filter(_item => _item !== old_logs);
    } catch (_err) {
...
// clear:12
// clear:12 — GET /api/logs
app.get('/api/logs', async (req, res) => {
  try {
    // clear:13
    const _resp_13 = (await db.findAll('logs', {}, { limit: 50 })).map(_revive);
    // clear:13
    return res.json(_resp_13);
  } catch (err) {
    console.error('[GET /api/logs] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/logs', line: 12, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:15
// clear:15 — POST /api/logs
app.post('/api/logs', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const log = req.body;
    // clear:16
    const saved = await _clearTry(() => db.insert('logs', _pick(log, LogsSchema)), { op: 'insert', table: 'logs', line: 16, file: 'main.clear', source: "" }); // clear:16
    // clear:17
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/logs] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/logs', line: 15, file: 'main.clear' }, err._clearCtx || {});
