...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"141":3,"143":5,"151":10,"156":11,"158":12,"160":13,"177":15,"178":15,"181":16,"183":17} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Sessions: user_id*, token*, created_at_date (line 5)
//
// ENDPOINTS:
//   GET /api/session-count (line 15)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    sessions: { line: 5, file: "main.clear", fields: { user_id: { required: true, type: "fk" }, token: { required: true, type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/session-count": { line: 15, file: "main.clear", auth: false },
  }
} : null;
...
db.createTable('sessions', SessionsSchema);
// clear:10
// Scheduled: every day at 03:00
(function _scheduleDailyAt3_0() {
  const _runAt = async () => {
    try {
      // clear:11
      const old_sessions = (await db.findAll('sessions')).map(_revive);
      // clear:12
      for (const old_session of old_sessions) {
        // clear:13
        Sessions = Sessions.filter(_item => _item !== old_session);
      }
    } catch (_err) {
...
// clear:15
// clear:15 — GET /api/session-count
app.get('/api/session-count', async (req, res) => {
  try {
    // clear:16
    let session_count = await db.aggregate('sessions', 'COUNT', 'id', {});
    // clear:17
    return res.json({ count: session_count });
  } catch (err) {
    console.error('[GET /api/session-count] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/session-count', line: 15, file: 'main.clear' }, err._clearCtx || {});
