...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"154":3,"164":10,"165":10,"168":11,"170":11,"179":13,"180":13,"183":14,"185":15,"194":17,"195":17,"199":18,"201":18,"210":20,"211":20,"216":21,"217":21,"218":22} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Companies: name*, industry, website, description, created_at_date (line 3)
//
// ENDPOINTS:
//   GET /api/companies (line 10)
//   GET /api/companies/search with q (line 13)
//   GET /api/companies/:id (line 17)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    companies: { line: 3, file: "main.clear", fields: { name: { required: true, type: "text" }, industry: { type: "text" }, website: { type: "text" }, description: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/companies": { line: 10, file: "main.clear", auth: false },
    "GET /api/companies/search with q": { line: 13, file: "main.clear", auth: false },
    "GET /api/companies/:id": { line: 17, file: "main.clear", auth: false },
    "POST /api/companies": { line: 20, file: "main.clear", auth: false },
  }
} : null;
...
// clear:10
// clear:10 — GET /api/companies
app.get('/api/companies', async (req, res) => {
  try {
    // clear:11
    const _resp_11 = (await db.findAll('companies', {}, { limit: 50 })).map(_revive);
    // clear:11
    return res.json(_resp_11);
  } catch (err) {
    console.error('[GET /api/companies] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/companies', line: 10, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:13
// clear:13 — GET /api/companies/search with q
app.get('/api/companies/search with q', async (req, res) => {
  try {
    // clear:14
    ...
    console.error('[GET /api/companies/search with q] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/companies/search with q', line: 13, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:17
// clear:17 — GET /api/companies/:id
app.get('/api/companies/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:18
    const _resp_18 = _revive(await db.findOne('companies', { id: incoming?.id }));
    // clear:18
    return res.json(_resp_18);
  } catch (err) {
    console.error('[GET /api/companies/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/companies/:id', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:20
// clear:20 — POST /api/companies
app.post('/api/companies', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const company = req.body;
    // clear:21
    const saved = await _clearTry(() => db.insert('companies', _pick(company, CompaniesSchema)), { op: 'insert', table: 'companies', line: 21, file: 'main.clear', source: "" }); // clear:21
    // clear:22
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/companies] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/companies', line: 20, file: 'main.clear' }, err._clearCtx || {});
