...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"150":5,"157":9,"168":10,"170":11,"171":11,"172":12,"187":14,"188":14,"191":15,"193":16} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Messages: text*, created_at_date (line 5)
//
// ENDPOINTS:
//   GET /api/history (line 14)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    messages: { line: 5, file: "main.clear", fields: { text: { required: true, type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/history": { line: 14, file: "main.clear", auth: false },
  }
} : null;
...
const wss_notifications = new WebSocket.Server({ noServer: true });
const _clients_notifications = new Set();
wss_notifications.on('connection', (ws) => {
  _clients_notifications.add(ws);
  // Heartbeat: detect dead connections
  ws._isAlive = true;
  ws.on('pong', () => { ws._isAlive = true; });
  ws.on('message', (message) => {
  // clear:10
  let entry = { text: message };
  // clear:11
  await _clearTry(() => db.update('messages', _pick(entry, MessagesSchema)), { op: 'update', table: 'messages', line: 11, file: 'main.clear', source: "" }); // clear:11
  // clear:12
  wss.clients.forEach(_c => { if (_c.readyState === 1) _c.send(JSON.stringify(message)); });
  });
  ws.on('close', () => {
    _clients_notifications.delete(ws);
  });
});
// Heartbeat interval: ping all clients every 30s
setInterval(() => {
  wss_notifications.clients.forEach((ws) => {
...
// clear:14
// clear:14 — GET /api/history
app.get('/api/history', async (req, res) => {
  try {
    // clear:15
    const messages = (await db.findAll('messages', {}, { limit: 50 })).map(_revive);
    // clear:16
    return res.json(messages);
  } catch (err) {
    console.error('[GET /api/history] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/history', line: 14, file: 'main.clear' }, err._clearCtx || {});
