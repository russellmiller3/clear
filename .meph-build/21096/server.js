...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"188":3,"190":4,"192":7,"195":9,"196":9,"201":10,"203":11} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// ENDPOINTS:
//   POST /api/summarize (line 9)
//
// AGENTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  endpoints: {
    "POST /api/summarize": { line: 9, file: "main.clear", auth: false },
  }
} : null;
...
async function agent_summarizer(text) {
  // clear:4
  let result = await _askAI("You are a helpful text summarizer. Analyze the provided text, write a concise summary, and extract the key points.", text, [{"name":"summary","type":"text"},{"name":"key_points","type":"text"}], null);
  // clear:7
  return result;
}
// clear:9
// clear:9 — POST /api/summarize
app.post('/api/summarize', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const payload = req.body;
    // clear:10
    let result = await agent_summarizer(payload?.text);
    // clear:11
    return res.json(result);
  } catch (err) {
    console.error('[POST /api/summarize] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/summarize', line: 9, file: 'main.clear' }, err._clearCtx || {});
