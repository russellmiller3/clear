...
setInterval(async () => {
  try {
    // clear:12
    let users = await fetch("https://jsonplaceholder.typicode.com/users").then(r => r.json());
    // clear:13
    for (const user of users) {
      // clear:14
      let contact = { name: req.user?.name, email: req.user?.email, tag: "imported" };
      // clear:15
      await _clearTry(() => db.update('mailinglists', _pick(contact, MailingListSchema)), { op: 'update', table: 'mailinglists', line: 15, file: 'main.clear', source: "" }); // clear:15
    }
  } catch (_err) {
...
app.get('/api/mailing-list', async (req, res) => {
  try {
    // clear:18
    const contacts = (await db.findAll('mailinglists', {}, { limit: 50 })).map(_revive);
    // clear:19
