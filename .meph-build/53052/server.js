...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":3,"155":5,"161":8,"162":8,"165":9,"167":10,"181":16,"182":16,"187":17,"189":18,"192":20,"198":26,"209":29,"210":29,"215":30,"217":31,"220":33,"225":38} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "POST /api/reset": { line: 29, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    if (_clear_len(counters) == 0) {
      ...
      row.count = new_count;
      await _clearTry(() => db.update('counters', _pick(row, CountersSchema)), { op: 'update', table: 'counters', line: 26, file: 'main.clear', source: "" }); // clear:26
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    // clear:31
    if (_clear_len(counters) == 0) {
      let new_entry = { count: 0 };
      await _clearTry(() => db.insert('counters', _pick(new_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 33, file: 'main.clear', source: "" }); // clear:33
      return res.json({ count: 0 });
    } else {
      let row = first;
      row.count = 0;
      await _clearTry(() => db.update('counters', _pick(row, CountersSchema)), { op: 'update', table: 'counters', line: 38, file: 'main.clear', source: "" }); // clear:38
      return res.json({ count: 0 });
    }
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 29, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
