...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"147":3,"156":9,"157":9,"162":10,"164":11,"166":12,"167":12,"168":13} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Events: event_type*, amount, customer_email, received_at_date (line 3)
//
// ENDPOINTS:
//   POST /webhook/stripe (line 9)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    events: { line: 3, file: "main.clear", fields: { event_type: { required: true, type: "text" }, amount: { type: "number" }, customer_email: { type: "text" }, received_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "POST /webhook/stripe": { line: 9, file: "main.clear", auth: false },
  }
} : null;
...
// clear:9
// clear:9 — POST /webhook/stripe
app.post('/webhook/stripe', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const payload = req.body;
    // clear:10
    if (!(payload?.signature)) { return res.status(403).json({ error: "Missing signature" }); }
    // clear:11
    payload.event_type = payload?.type;
    // clear:12
    await _clearTry(() => db.update('events', _pick(payload, EventsSchema)), { op: 'update', table: 'events', line: 12, file: 'main.clear', source: "" }); // clear:12
    // clear:13
    return res.json({ received: true });
  } catch (err) {
    console.error('[POST /webhook/stripe] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /webhook/stripe', line: 9, file: 'main.clear' }, err._clearCtx || {});
