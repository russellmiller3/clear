...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"141":3,"149":8,"154":9,"156":10,"172":12,"173":12,"176":13,"178":14} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    "GET /api/session-count": { line: 12, file: "main.clear", auth: false },
  }
} : null;
...
(function _scheduleDailyAt3_0() {
  const _runAt = async () => {
    try {
      // clear:9
      const stale = (await db.findAll('sessions', { created_at_date: older }, { limit: 50 })).map(_revive);
      // clear:10
      Sessions = Sessions.filter(_item => _item !== stale);
    } catch (_err) {
...
// clear:12
// clear:12 — GET /api/session-count
app.get('/api/session-count', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/session-count', line: 12, file: 'main.clear' }, err._clearCtx || {});
