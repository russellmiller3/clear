...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"180":3,"182":5,"191":11,"192":11,"198":12,"201":15,"202":15,"203":16,"213":18,"214":18,"217":19,"219":20,"228":22,"229":22,"232":23,"234":24,"243":26,"244":26,"249":27,"251":28,"254":28,"256":29,"266":31,"267":31,"270":32,"272":33,"273":33,"274":34} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Contacts: name*, email*!, phone, notes (line 5)
//
// ENDPOINTS:
//   POST /api/contacts (line 11)
//   GET /api/contacts (line 18)
//   GET /api/contacts/:id (line 22)
//   POST /api/contacts/:id [auth] (line 26)
//   DELETE /api/contacts/:id [auth] (line 31)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    contacts: { line: 5, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, unique: true, type: "text" }, phone: { type: "text" }, notes: { type: "text" } } },
  },
  endpoints: {
    "POST /api/contacts": { line: 11, file: "main.clear", auth: false },
    "GET /api/contacts": { line: 18, file: "main.clear", auth: false },
    "GET /api/contacts/:id": { line: 22, file: "main.clear", auth: false },
    "POST /api/contacts/:id": { line: 26, file: "main.clear", auth: true },
    "DELETE /api/contacts/:id": { line: 31, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    // clear:12
    const _vErrs = _validate(req.body, [{"field":"name","type":"text","required":true}, {"field":"email","type":"text","required":true,"matches":"email"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:15
    const saved = await _clearTry(() => db.insert('contacts', _pick(contact, ContactsSchema)), { op: 'insert', table: 'contacts', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/contacts', line: 11, file: 'main.clear' }, err._clearCtx || {});
...
// clear:18
// clear:18 — GET /api/contacts
app.get('/api/contacts', async (req, res) => {
  try {
    // clear:19
    const contacts = (await db.findAll('contacts', {}, { limit: 50 })).map(_revive);
    // clear:20
    return res.json(contacts);
  } catch (err) {
    console.error('[GET /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:22
// clear:22 — GET /api/contacts/:id
app.get('/api/contacts/:id', async (req, res) => {
  try {
    // clear:23
    const contact = (await db.findAll('contacts', {}, { limit: 50 })).map(_revive);
    // clear:24
    return res.json(contact);
  } catch (err) {
    console.error('[GET /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts/:id', line: 22, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:26
// clear:26 — POST /api/contacts/:id
app.post('/api/contacts/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('contacts', _picked_changes), { op: 'update', table: 'contacts', line: 28, file: 'main.clear', source: "" }); // clear:28
    Object.assign(changes, await db.findOne('contacts', { id: _picked_changes.id }) || {});
    // clear:29
    return res.json({ message: "updated" });
  } catch (err) {
    console.error('[POST /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/contacts/:id', line: 26, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:31
// clear:31 — DELETE /api/contacts/:id
app.delete('/api/contacts/:id', async (req, res) => {
  try {
    // clear:32
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:33
    await _clearTry(() => db.remove('contacts', { id: req.params.id }), { op: 'remove', table: 'contacts', line: 33, file: 'main.clear', source: "" }); // clear:33
    // clear:34
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/contacts/:id', line: 31, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
