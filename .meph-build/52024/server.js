...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    ticks: { line: 5, file: "main.clear", fields: { action: { type: "text" } } },
  },
  endpoints: {
...
db.createTable('ticks', TicksSchema);
// clear:8
// clear:8 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    const ticks = (await db.findAll('ticks', {}, { limit: 50 })).map(_revive);
    // clear:10
    let total = _clear_len(ticks);
    // clear:11
...
app.post('/api/increment', async (req, res) => {
  try {
    // clear:14
    let tick = { action: "increment" };
    // clear:15
    await _clearTry(() => db.insert('ticks', _pick(tick, TicksSchema)), { op: 'insert', table: 'ticks', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
    const ticks = (await db.findAll('ticks', {}, { limit: 50 })).map(_revive);
    // clear:17
    let total = _clear_len(ticks);
