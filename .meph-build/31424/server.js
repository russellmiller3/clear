...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"157":9,"161":10,"163":11,"165":12,"167":13,"169":14,"171":15,"172":15,"178":17,"179":17,"182":18,"184":18} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   MailingList: name*, email*, tag, synced_at_date (line 3)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    mailinglists: { line: 3, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, tag: { type: "text" }, synced_at_date: { type: "timestamp" } } },
  },
  endpoints: {
...
setInterval(async () => {
  try {
    ...
    for (const user of users) {
      ...
      entry.tag = "imported";
      // clear:15
      await _clearTry(() => db.update('mailinglists', _pick(entry, MailingListSchema)), { op: 'update', table: 'mailinglists', line: 15, file: 'main.clear', source: "" }); // clear:15
    }
  } catch (_err) {
...
app.get('/api/mailing-list', async (req, res) => {
  try {
    // clear:18
    const _resp_18 = (await db.findAll('mailinglists', {}, { limit: 50 })).map(_revive);
    // clear:18
