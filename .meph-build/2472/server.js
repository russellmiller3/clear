...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"180":3,"182":5,"191":11,"192":11,"198":12,"201":15,"202":15,"203":16,"213":18,"214":18,"217":19,"219":19,"228":21,"229":21,"233":22,"235":22,"244":24,"245":24,"250":25,"252":26,"255":26,"257":27,"267":29,"268":29,"271":30,"273":31,"274":31,"275":32} : null;
// ┌─────────────────────────────────────────────┐
...
//   GET /api/contacts/:id (line 21)
//   PUT /api/contacts/:id [auth] (line 24)
//   DELETE /api/contacts/:id [auth] (line 29)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/contacts/:id": { line: 21, file: "main.clear", auth: false },
    "PUT /api/contacts/:id": { line: 24, file: "main.clear", auth: true },
    "DELETE /api/contacts/:id": { line: 29, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/contacts', async (req, res) => {
  try {
    // clear:19
    const _resp_19 = (await db.findAll('contacts', {}, { limit: 50 })).map(_revive);
    // clear:19
...
// clear:21
// clear:21 — GET /api/contacts/:id
app.get('/api/contacts/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:22
    const _resp_22 = _revive(await db.findOne('contacts', { id: incoming?.id }));
    // clear:22
    return res.json(_resp_22);
  } catch (err) {
    console.error('[GET /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts/:id', line: 21, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:24
// clear:24 — PUT /api/contacts/:id
app.put('/api/contacts/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('contacts', _picked_changes), { op: 'update', table: 'contacts', line: 26, file: 'main.clear', source: "" }); // clear:26
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/contacts/:id', line: 24, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:29
// clear:29 — DELETE /api/contacts/:id
app.delete('/api/contacts/:id', async (req, res) => {
  try {
    // clear:30
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:31
    await _clearTry(() => db.remove('contacts', { id: req.params.id }), { op: 'remove', table: 'contacts', line: 31, file: 'main.clear', source: "" }); // clear:31
    // clear:32
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/contacts/:id', line: 29, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
