...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"173":3,"175":5,"181":8,"182":8,"188":9,"191":10,"194":12,"195":12,"196":13,"206":15,"207":15,"210":16,"213":17,"215":18} : null;
// ┌─────────────────────────────────────────────┐
...
//
// ENDPOINTS:
//   POST /api/items (line 8)
//   GET /api/items (line 15)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    items: { line: 5, file: "main.clear", fields: { name: { required: true, type: "text" } } },
  },
  endpoints: {
    "POST /api/items": { line: 8, file: "main.clear", auth: false },
    "GET /api/items": { line: 15, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/items', async (req, res) => {
  try {
    ...
    // clear:10
    const _vErrs = _validate(req.body, [{"field":"name","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:12
    const saved = await _clearTry(() => db.insert('items', _pick(item, ItemsSchema)), { op: 'insert', table: 'items', line: 12, file: 'main.clear', source: "" }); // clear:12
    // clear:13
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/items] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/items', line: 8, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/items', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/items', line: 15, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
