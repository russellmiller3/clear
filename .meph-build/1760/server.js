...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":3,"155":5,"161":8,"162":8,"165":9,"167":10,"171":12,"173":13,"182":15,"183":15,"186":16,"188":17,"191":19,"194":21,"196":22,"198":23,"200":24,"201":24,"202":25,"211":27,"212":27,"215":28,"217":29,"218":29,"219":30} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   CountLog: count_val (line 5)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    countlogs: { line: 5, file: "main.clear", fields: { count_val: { type: "number" } } },
  },
  endpoints: {
...
// clear:5
// Data shape: CountLog
const CountLogSchema = {
  count_val: { type: "number", default: 0 }
};
db.createTable('countlogs', CountLogSchema);
// clear:8
// clear:8 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    const logs = (await db.findAll('countlogs', {}, { limit: 50 })).map(_revive);
    // clear:10
    if (_clear_len(logs) == 0) {
      return res.json({ count: 0 });
    }
    // clear:12
    let latest = logs[logs.length - 1];
    // clear:13
    return res.json({ count: latest?.count_val });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    // clear:17
    if (_clear_len(logs) == 0) {
      let first_log = { count_val: 1 };
      await _clearTry(() => db.insert('countlogs', _pick(first_log, CountLogSchema)), { op: 'insert', table: 'countlogs', line: 19, file: 'main.clear', source: "" }); // clear:19
      return res.json({ count: 1 });
    }
    // clear:21
    let latest = logs[logs.length - 1];
    // clear:22
    let new_count = (latest?.count_val + 1);
    // clear:23
    let new_log = { count_val: new_count };
    // clear:24
    await _clearTry(() => db.insert('countlogs', _pick(new_log, CountLogSchema)), { op: 'insert', table: 'countlogs', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
...
app.post('/api/reset', async (req, res) => {
  try {
    // clear:28
    let reset_log = { count_val: 0 };
    // clear:29
    await _clearTry(() => db.insert('countlogs', _pick(reset_log, CountLogSchema)), { op: 'insert', table: 'countlogs', line: 29, file: 'main.clear', source: "" }); // clear:29
    // clear:30
    return res.json({ count: 0 });
  } catch (err) {
