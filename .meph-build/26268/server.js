...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"176":3,"178":5,"186":10,"187":10,"193":11,"196":15,"197":15,"198":16,"208":18,"209":18,"212":19,"214":20,"223":22,"224":22,"228":23,"230":24,"231":24,"232":25} : null;
// ┌─────────────────────────────────────────────┐
...
//   DELETE /api/bookmarks/:id [auth] (line 22)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "DELETE /api/bookmarks/:id": { line: 22, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    // clear:11
    const _vErrs = _validate(req.body, [{"field":"url","type":"text"}, {"field":"url","type":"text"}, {"field":"title","type":"text"}]);
...
app.get('/api/bookmarks', async (req, res) => {
  try {
    // clear:19
    const bookmarks = (await db.findAll('bookmarks', {}, { limit: 50 })).map(_revive);
    // clear:20
...
// clear:22
// clear:22 — DELETE /api/bookmarks/:id
app.delete('/api/bookmarks/:id', async (req, res) => {
  try {
    ...
    // clear:24
    await _clearTry(() => db.remove('bookmarks', { id: incoming?.id }), { op: 'remove', table: 'bookmarks', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/bookmarks/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/bookmarks/:id', line: 22, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
