...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counts: { line: 5, file: "main.clear", fields: { type: { type: "text" } } },
  },
  endpoints: {
...
// Data shape: Counts
const CountsSchema = {
  type: { type: "text", default: "increment" }
};
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    let total = await db.aggregate('counts', 'COUNT', 'id', {});
    // clear:10
...
app.post('/api/increment', async (req, res) => {
  try {
    // clear:13
    let entry = { type: "increment" };
    // clear:14
    await _clearTry(() => db.insert('counts', _pick(entry, CountsSchema)), { op: 'insert', table: 'counts', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    let total = await db.aggregate('counts', 'COUNT', 'id', {});
    // clear:16
    return res.json({ count: total });
