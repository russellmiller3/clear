...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":3,"155":5,"161":8,"162":8,"165":9,"167":10,"171":12,"173":13,"182":15,"183":15,"186":16,"188":17,"191":19,"194":21,"196":22,"198":23,"200":24,"202":25,"203":25,"204":26,"213":28,"214":28,"217":29,"219":30,"222":32,"225":34,"227":35,"229":36,"231":37,"232":37,"233":38} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counters: current_count (line 5)
//
// ENDPOINTS:
//   GET /api/count (line 8)
//   POST /api/increment (line 15)
//   POST /api/reset (line 28)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 5, file: "main.clear", fields: { current_count: { type: "number" } } },
  },
  endpoints: {
    "GET /api/count": { line: 8, file: "main.clear", auth: false },
    "POST /api/increment": { line: 15, file: "main.clear", auth: false },
    "POST /api/reset": { line: 28, file: "main.clear", auth: false },
  }
} : null;
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    const all_counters = (await db.findAll('counters', {}, { limit: 50 })).map(_revive);
    // clear:10
    if (_clear_len(all_counters) == 0) {
      return res.json({ count: 0 });
    }
    // clear:12
    let counter = all_counters[0];
    // clear:13
    return res.json({ count: counter?.current_count });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    // clear:17
    if (_clear_len(all_counters) == 0) {
      let starter = { current_count: 1 };
      await _clearTry(() => db.insert('counters', _pick(starter, CountersSchema)), { op: 'insert', table: 'counters', line: 19, file: 'main.clear', source: "" }); // clear:19
      return res.json({ count: 1 });
    }
    // clear:21
    let counter = all_counters[0];
    // clear:22
    let updated_count = (counter?.current_count + 1);
    // clear:23
    let counter_id = counter?.id;
    // clear:24
    let updated_counter = { id: counter_id, current_count: updated_count };
    // clear:25
    await _clearTry(() => db.update('counters', _pick(updated_counter, CountersSchema)), { op: 'update', table: 'counters', line: 25, file: 'main.clear', source: "" }); // clear:25
    // clear:26
    return res.json({ count: updated_count });
  } catch (err) {
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    // clear:30
    if (_clear_len(all_counters) == 0) {
      let starter = { current_count: 0 };
      await _clearTry(() => db.insert('counters', _pick(starter, CountersSchema)), { op: 'insert', table: 'counters', line: 32, file: 'main.clear', source: "" }); // clear:32
      return res.json({ count: 0 });
    }
    // clear:34
    let counter = all_counters[0];
    ...
    let counter_id = counter?.id;
    // clear:36
    let reset_counter = { id: counter_id, current_count: 0 };
    // clear:37
    await _clearTry(() => db.update('counters', _pick(reset_counter, CountersSchema)), { op: 'update', table: 'counters', line: 37, file: 'main.clear', source: "" }); // clear:37
    // clear:38
    return res.json({ count: 0 });
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 28, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
