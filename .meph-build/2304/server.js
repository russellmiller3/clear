...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"239":5,"246":9,"254":14,"255":14,"258":15,"260":16,"262":17,"271":19,"272":19,"277":20,"279":21,"281":22,"282":22,"283":23,"293":25,"294":25,"299":26,"301":27,"304":27,"306":28,"316":30,"317":30,"321":31,"323":32,"324":32,"325":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password* (line 5)
//   Todos: title*, done, user_id* (line 9)
//
// ENDPOINTS:
//   GET /api/todos [auth] (line 14)
//   POST /api/todos [auth] (line 19)
//   PUT /api/todos/:id [auth] (line 25)
//   DELETE /api/todos/:id [auth] (line 30)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 5, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" } } },
    todos: { line: 9, file: "main.clear", fields: { title: { required: true, type: "text" }, done: { type: "text" }, user_id: { required: true, type: "fk" } } },
  },
  endpoints: {
    "GET /api/todos": { line: 14, file: "main.clear", auth: true },
    "POST /api/todos": { line: 19, file: "main.clear", auth: true },
    "PUT /api/todos/:id": { line: 25, file: "main.clear", auth: true },
    "DELETE /api/todos/:id": { line: 30, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/todos', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/todos', line: 14, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:19
// clear:19 — POST /api/todos
app.post('/api/todos', async (req, res) => {
  try {
    ...
    // clear:22
    const saved = await _clearTry(() => db.insert('todos', _pick(todo, TodosSchema)), { op: 'insert', table: 'todos', line: 22, file: 'main.clear', source: "" }); // clear:22
    // clear:23
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/todos] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/todos', line: 19, file: 'main.clear' }, err._clearCtx || {});
...
app.put('/api/todos/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('todos', _picked_changes), { op: 'update', table: 'todos', line: 27, file: 'main.clear', source: "" }); // clear:27
    Object.assign(changes, await db.findOne('todos', { id: _picked_changes.id }) || {});
    // clear:28
    return res.json({ message: "updated" });
  } catch (err) {
    console.error('[PUT /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/todos/:id', line: 25, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.delete('/api/todos/:id', async (req, res) => {
  try {
    ...
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:32
    await _clearTry(() => db.remove('todos', { id: incoming?.id }), { op: 'remove', table: 'todos', line: 32, file: 'main.clear', source: "" }); // clear:32
    // clear:33
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/todos/:id', line: 30, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
