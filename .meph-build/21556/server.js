...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"151":3,"153":5,"163":12,"164":12,"169":13,"171":14,"173":15,"175":16,"179":18,"180":18,"181":19,"191":21,"192":21,"195":22,"197":23} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Bookings: guest_name*, date*, time*, notes, created_at_date (line 5)
//
// ENDPOINTS:
//   POST /api/bookings (line 12)
//   GET /api/bookings (line 21)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    bookings: { line: 5, file: "main.clear", fields: { guest_name: { required: true, type: "text" }, date: { required: true, type: "text" }, time: { required: true, type: "text" }, notes: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "POST /api/bookings": { line: 12, file: "main.clear", auth: false },
    "GET /api/bookings": { line: 21, file: "main.clear", auth: false },
  }
} : null;
...
// clear:12
// clear:12 — POST /api/bookings
app.post('/api/bookings', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const booking = req.body;
    // clear:13
    let slot_date = booking?.date;
    // clear:14
    let slot_time = booking?.time;
    // clear:15
    const existing = (await db.findAll('bookings', {}, { limit: 50 })).map(_revive);
    // clear:16
    if (_clear_len(existing) > 0) {
      return res.status(400).json({ message: "Slot already booked" });
    }
    // clear:18
    const saved = await _clearTry(() => db.insert('bookings', _pick(booking, BookingsSchema)), { op: 'insert', table: 'bookings', line: 18, file: 'main.clear', source: "" }); // clear:18
    // clear:19
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/bookings] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/bookings', line: 12, file: 'main.clear' }, err._clearCtx || {});
...
// clear:21
// clear:21 — GET /api/bookings
app.get('/api/bookings', async (req, res) => {
  try {
    // clear:22
    const bookings = (await db.findAll('bookings', {}, { limit: 50 })).map(_revive);
    // clear:23
    return res.json(bookings);
  } catch (err) {
    console.error('[GET /api/bookings] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/bookings', line: 21, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
