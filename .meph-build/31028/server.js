...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"160":3,"162":5,"168":8,"175":12,"182":16,"183":16,"188":17,"189":17,"190":18,"200":20,"201":20,"204":21,"206":22,"215":24,"216":24,"222":25,"224":26,"225":26,"226":27,"236":29,"237":29,"240":30,"242":31,"251":33,"252":33,"258":34,"260":35,"261":35,"262":36} : null;
// ┌─────────────────────────────────────────────┐
...
//   POST /api/workspaces/:id/items (line 33)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "POST /api/workspaces/:id/items": { line: 33, file: "main.clear", auth: false },
  }
} : null;
...
// clear:33
// clear:33 — POST /api/workspaces/:id/items
app.post('/api/workspaces/:id/items', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const item = req.body;
    const incoming = req.params;
    // clear:34
    item.workspace_id = incoming?.id;
    // clear:35
    const saved = await _clearTry(() => db.insert('items', _pick(item, ItemsSchema)), { op: 'insert', table: 'items', line: 35, file: 'main.clear', source: "" }); // clear:35
    // clear:36
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/workspaces/:id/items] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/workspaces/:id/items', line: 33, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
