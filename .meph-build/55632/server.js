...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"174":3,"176":5,"184":10,"185":10,"191":11,"194":14,"195":14,"196":15,"206":17,"207":17,"210":18,"212":19,"221":21,"222":21,"225":22,"227":23} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Posts: title*, body*, author (line 5)
//
// ENDPOINTS:
//   POST /api/posts (line 10)
//   GET /api/posts (line 17)
//   GET /api/posts/:id (line 21)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    posts: { line: 5, file: "main.clear", fields: { title: { required: true, type: "text" }, body: { required: true, type: "text" }, author: { type: "text" } } },
  },
  endpoints: {
    "POST /api/posts": { line: 10, file: "main.clear", auth: false },
    "GET /api/posts": { line: 17, file: "main.clear", auth: false },
    "GET /api/posts/:id": { line: 21, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    // clear:11
    const _vErrs = _validate(req.body, [{"field":"title","type":"text","required":true}, {"field":"body","type":"text","required":true}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:14
    const saved = await _clearTry(() => db.insert('posts', _pick(entry, PostsSchema)), { op: 'insert', table: 'posts', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/posts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/posts', line: 10, file: 'main.clear' }, err._clearCtx || {});
...
// clear:17
// clear:17 — GET /api/posts
app.get('/api/posts', async (req, res) => {
  try {
    // clear:18
    const posts = (await db.findAll('posts', {}, { limit: 50 })).map(_revive);
    // clear:19
    return res.json(posts);
  } catch (err) {
    console.error('[GET /api/posts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/posts', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:21
// clear:21 — GET /api/posts/:id
app.get('/api/posts/:id', async (req, res) => {
  try {
    // clear:22
    const found = (await db.findAll('posts', {}, { limit: 50 })).map(_revive);
    // clear:23
    return res.json(found);
  } catch (err) {
    console.error('[GET /api/posts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/posts/:id', line: 21, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
