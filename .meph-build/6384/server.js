...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"172":3,"174":5,"183":11,"184":11,"190":12,"193":16,"194":16,"195":17,"205":19,"206":19,"209":20,"211":21} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/contacts": { line: 19, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    // clear:12
    const _vErrs = _validate(req.body, [{"field":"name","type":"text"}, {"field":"email","type":"text"}, {"field":"email","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:16
    const saved = await _clearTry(() => db.insert('contacts', _pick(contact, ContactsSchema)), { op: 'insert', table: 'contacts', line: 16, file: 'main.clear', source: "" }); // clear:16
    // clear:17
...
app.get('/api/contacts', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 19, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
