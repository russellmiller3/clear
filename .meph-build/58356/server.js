...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":3,"155":5,"161":8,"162":8,"165":9,"167":10,"171":12,"173":13,"175":14,"184":16,"185":16,"188":17,"190":18,"193":20,"196":22,"198":23,"200":24,"202":25,"204":26,"206":27,"207":27,"208":28,"217":30,"218":30,"221":31,"223":32,"226":34,"229":36,"231":37,"233":38,"235":39,"236":39,"237":40} : null;
// ┌─────────────────────────────────────────────┐
...
//   POST /api/increment (line 16)
//   POST /api/reset (line 30)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "POST /api/increment": { line: 16, file: "main.clear", auth: false },
    "POST /api/reset": { line: 30, file: "main.clear", auth: false },
  }
} : null;
...
app.get('/api/count', async (req, res) => {
  try {
    ...
    let row = rows[0];
    // clear:13
    let current_value = row?.value;
    // clear:14
    return res.json({ count: current_value });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    if (_clear_len(rows) == 0) {
      ...
      await _clearTry(() => db.insert('counters', _pick(new_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 20, file: 'main.clear', source: "" }); // clear:20
    ...
    // clear:26
    let counter = { id: row_id, value: new_value };
    // clear:27
    await _clearTry(() => db.update('counters', _pick(counter, CountersSchema)), { op: 'update', table: 'counters', line: 27, file: 'main.clear', source: "" }); // clear:27
    // clear:28
    return res.json({ count: new_value });
  } catch (err) {
    console.error('[POST /api/increment] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:30
// clear:30 — POST /api/reset
app.post('/api/reset', async (req, res) => {
  try {
    ...
    if (_clear_len(rows) == 0) {
      ...
      await _clearTry(() => db.insert('counters', _pick(zero_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 34, file: 'main.clear', source: "" }); // clear:34
      return res.json({ count: 0 });
    }
    // clear:36
    let row = rows[0];
    ...
    let row_id = row?.id;
    // clear:38
    let counter = { id: row_id, value: 0 };
    // clear:39
    await _clearTry(() => db.update('counters', _pick(counter, CountersSchema)), { op: 'update', table: 'counters', line: 39, file: 'main.clear', source: "" }); // clear:39
    // clear:40
    return res.json({ count: 0 });
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 30, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
