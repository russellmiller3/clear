...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"158":3,"160":5,"166":8,"173":12,"180":16,"181":16,"186":17,"187":17,"188":18,"198":20,"199":20,"202":21,"204":21,"213":23,"214":23,"220":24,"222":25,"223":25,"224":26,"234":28,"235":28,"238":29,"240":30} : null;
// ┌─────────────────────────────────────────────┐
...
//   POST /api/workspaces/:id/members (line 23)
//   GET /api/workspaces/:id/items (line 28)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "POST /api/workspaces/:id/members": { line: 23, file: "main.clear", auth: false },
    "GET /api/workspaces/:id/items": { line: 28, file: "main.clear", auth: false },
  }
} : null;
...
app.get('/api/workspaces', async (req, res) => {
  try {
    // clear:21
    const _resp_21 = (await db.findAll('workspaces', {}, { limit: 50 })).map(_revive);
    // clear:21
...
// clear:23
// clear:23 — POST /api/workspaces/:id/members
app.post('/api/workspaces/:id/members', async (req, res) => {
  try {
    ...
    // clear:25
    const saved = await _clearTry(() => db.insert('members', _pick(member, MembersSchema)), { op: 'insert', table: 'members', line: 25, file: 'main.clear', source: "" }); // clear:25
    // clear:26
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/workspaces/:id/members] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/workspaces/:id/members', line: 23, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:28
// clear:28 — GET /api/workspaces/:id/items
app.get('/api/workspaces/:id/items', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/workspaces/:id/items', line: 28, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
