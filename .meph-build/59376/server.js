...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"176":3,"178":5,"186":10,"187":10,"193":11,"196":15,"197":15,"198":16,"208":18,"209":18,"212":19,"214":19,"223":21,"224":21,"227":22,"229":23,"230":23,"231":24} : null;
// ┌─────────────────────────────────────────────┐
...
//   GET /api/bookmarks (line 18)
//   DELETE /api/bookmarks/:id [auth] (line 21)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/bookmarks": { line: 18, file: "main.clear", auth: false },
    "DELETE /api/bookmarks/:id": { line: 21, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    // clear:11
    const _vErrs = _validate(req.body, [{"field":"url","type":"text"}, {"field":"title","type":"text"}, {"field":"url","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:15
    const saved = await _clearTry(() => db.insert('bookmarks', _pick(bookmark, BookmarksSchema)), { op: 'insert', table: 'bookmarks', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
...
// clear:18
// clear:18 — GET /api/bookmarks
app.get('/api/bookmarks', async (req, res) => {
  try {
    // clear:19
    const _resp_19 = (await db.findAll('bookmarks', {}, { limit: 50 })).map(_revive);
    // clear:19
    return res.json(_resp_19);
  } catch (err) {
    console.error('[GET /api/bookmarks] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/bookmarks', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:21
// clear:21 — DELETE /api/bookmarks/:id
app.delete('/api/bookmarks/:id', async (req, res) => {
  try {
    // clear:22
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:23
    await _clearTry(() => db.remove('bookmarks', { id: req.params.id }), { op: 'remove', table: 'bookmarks', line: 23, file: 'main.clear', source: "" }); // clear:23
    // clear:24
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/bookmarks/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/bookmarks/:id', line: 21, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
