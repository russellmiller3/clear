...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"152":3,"162":10,"163":10,"168":11,"172":13,"176":15,"180":17,"181":17,"182":18,"192":20,"193":20,"196":21,"198":22,"207":24,"208":24,"211":25,"213":26} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Leads: company*, email*, size*, assigned_to, created_at_date (line 3)
//
// ENDPOINTS:
//   POST /api/leads (line 10)
//   GET /api/leads/by/:rep (line 20)
//   GET /api/leads (line 24)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    leads: { line: 3, file: "main.clear", fields: { company: { required: true, type: "text" }, email: { required: true, type: "text" }, size: { required: true, type: "text" }, assigned_to: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "POST /api/leads": { line: 10, file: "main.clear", auth: false },
    "GET /api/leads/by/:rep": { line: 20, file: "main.clear", auth: false },
    "GET /api/leads": { line: 24, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:17
    const saved = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 17, file: 'main.clear', source: "" }); // clear:17
    // clear:18
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 10, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/leads/by/:rep', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/leads/by/:rep', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:24
// clear:24 — GET /api/leads
app.get('/api/leads', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 24, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
