...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"180":3,"182":5,"189":9,"190":9,"196":10,"199":12,"200":12,"201":13,"211":15,"212":15,"215":16,"217":16,"226":18,"227":18,"231":19,"233":19,"242":21,"243":21,"248":22,"250":23,"253":23,"255":24,"265":26,"266":26,"269":27,"271":28,"272":28,"273":29} : null;
// ┌─────────────────────────────────────────────┐
...
//   GET /api/todos/:id (line 18)
//   POST /api/todos/:id [auth] (line 21)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/todos/:id": { line: 18, file: "main.clear", auth: false },
    "POST /api/todos/:id": { line: 21, file: "main.clear", auth: true },
...
app.get('/api/todos', async (req, res) => {
  try {
    // clear:16
    const _resp_16 = (await db.findAll('todos', {}, { limit: 50 })).map(_revive);
    // clear:16
    return res.json(_resp_16);
  } catch (err) {
...
// clear:18
// clear:18 — GET /api/todos/:id
app.get('/api/todos/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:19
    const _resp_19 = _revive(await db.findOne('todos', { id: incoming?.id }));
    // clear:19
    return res.json(_resp_19);
  } catch (err) {
    console.error('[GET /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/todos/:id', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:21
// clear:21 — POST /api/todos/:id
app.post('/api/todos/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('todos', _picked_changes), { op: 'update', table: 'todos', line: 23, file: 'main.clear', source: "" }); // clear:23
    Object.assign(changes, await db.findOne('todos', { id: _picked_changes.id }) || {});
    // clear:24
    return res.json(changes);
  } catch (err) {
    console.error('[POST /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/todos/:id', line: 21, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.delete('/api/todos/:id', async (req, res) => {
  try {
    ...
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:28
    await _clearTry(() => db.remove('todos', { id: req.params.id }), { op: 'remove', table: 'todos', line: 28, file: 'main.clear', source: "" }); // clear:28
    // clear:29
