...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"204":3,"206":5,"216":12,"218":13,"220":17,"223":19,"224":19,"229":20,"231":21,"233":22,"235":23,"237":24,"238":24,"239":25,"249":27,"250":27,"253":28,"255":29} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/tickets": { line: 27, file: "main.clear", auth: false },
  }
} : null;
...
// clear:12
async function agent_categorizer(ticket) {
  // clear:13
  let result = await _askAI("You are a support ticket categorization system. Analyze the ticket and return JSON with category (one of: billing, technical, account, general), priority (one of: low, medium, high, critical), and suggested_action (a brief action for the support team).", ticket, [{"name":"category","type":"text"},{"name":"priority","type":"text"},{"name":"suggested_action","type":"text"}], null);
  // clear:17
  return result;
}
// clear:19
...
app.post('/api/tickets', async (req, res) => {
  try {
    ...
    // clear:20
    let categorized = await agent_categorizer(entry);
    // clear:21
    entry.category = categorized?.category;
    // clear:22
    entry.priority = categorized?.priority;
    // clear:23
    entry.suggested_action = categorized?.suggested_action;
    // clear:24
    const saved = await _clearTry(() => db.insert('tickets', _pick(entry, TicketsSchema)), { op: 'insert', table: 'tickets', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
...
app.get('/api/tickets', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/tickets', line: 27, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
