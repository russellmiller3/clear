...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"134":3,"135":3,"139":4,"141":5} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// ENDPOINTS:
//   GET /api/greet/:name (line 3)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  endpoints: {
    "GET /api/greet/:name": { line: 3, file: "main.clear", auth: false },
  }
} : null;

// clear:3
// clear:3 — GET /api/greet/:name
app.get('/api/greet/:name', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:4
    let greeting = (("Hello, " + incoming?.name) + "!");
    // clear:5
    return res.json({ greeting: greeting });
  } catch (err) {
    console.error('[GET /api/greet/:name] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/greet/:name', line: 3, file: 'main.clear' }, err._clearCtx || {});
