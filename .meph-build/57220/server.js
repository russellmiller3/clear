...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"237":3,"239":5,"247":10,"249":13,"250":13,"253":14,"255":15,"264":17,"265":17,"270":18,"272":19,"273":19,"274":20} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Orders: amount*, status, created_at_date (line 5)
//
// ENDPOINTS:
//   GET /api/orders (line 13)
//   POST /api/orders [auth] (line 17)
//
// PAGES:
//   'Dashboard' at / (line 22)
//
...
// Auth scaffolding from `allow signup and login` (clear:11)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    orders: { line: 5, file: "main.clear", fields: { amount: { required: true, type: "number" }, status: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/orders": { line: 13, file: "main.clear", auth: false },
    "POST /api/orders": { line: 17, file: "main.clear", auth: true },
  }
} : null;
...
console.log(/* ERROR */);
// clear:13
// clear:13 — GET /api/orders
app.get('/api/orders', async (req, res) => {
  try {
    // clear:14
    const orders = (await db.findAll('orders', {}, { limit: 50 })).map(_revive);
    // clear:15
    return res.json(orders);
  } catch (err) {
    console.error('[GET /api/orders] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/orders', line: 13, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:17
// clear:17 — POST /api/orders
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // clear:19
    const saved = await _clearTry(() => db.insert('orders', _pick(order, OrdersSchema)), { op: 'insert', table: 'orders', line: 19, file: 'main.clear', source: "" }); // clear:19
    // clear:20
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/orders] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/orders', line: 17, file: 'main.clear' }, err._clearCtx || {});
