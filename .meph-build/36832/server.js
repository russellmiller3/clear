...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"261":3,"263":5,"274":14,"276":16,"278":17,"280":18,"283":19,"285":20,"288":21,"291":23,"292":23,"298":24,"301":27,"303":28,"305":29,"306":29,"307":30,"317":32,"318":32,"321":33,"323":34,"332":36,"333":36,"336":37,"338":38} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Leads: name*, email*, company, size, assigned_to, status (line 5)
//
// ENDPOINTS:
//   POST /api/leads (line 23)
//   GET /api/leads/new (line 32)
//   GET /api/leads (line 36)
//
// PAGES:
//   'Lead Intake' at / (line 40)
//
...
// Auth scaffolding from `allow signup and login` (clear:13)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    leads: { line: 5, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, company: { type: "text" }, size: { type: "text" }, assigned_to: { type: "text" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/leads": { line: 23, file: "main.clear", auth: false },
    "GET /api/leads/new": { line: 32, file: "main.clear", auth: false },
    "GET /api/leads": { line: 36, file: "main.clear", auth: false },
  }
} : null;
...
console.log(/* ERROR */);
// clear:16
function get_assigned_person(size) {
  // clear:17
  if (size == "Enterprise") {
    // clear:18
    return "charlie";
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:24
    const _vErrs = _validate(req.body, [{"field":"name","type":"text","required":true}, {"field":"email","type":"text","required":true}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:27
    let assigned = get_assigned_person(lead?.size);
    // clear:28
    lead.assigned_to = assigned;
    // clear:29
    const saved = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 29, file: 'main.clear', source: "" }); // clear:29
    // clear:30
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 23, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/leads', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 36, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
