...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"150":5,"157":9,"168":10,"169":10,"170":11,"185":13,"186":13,"189":14,"191":14} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    "GET /api/history": { line: 13, file: "main.clear", auth: false },
  }
} : null;
...
wss_notifications.on('connection', (ws) => {
  ...
  // clear:10
  await _clearTry(() => db.update('messages', _pick(message, MessagesSchema)), { op: 'update', table: 'messages', line: 10, file: 'main.clear', source: "" }); // clear:10
  // clear:11
...
// clear:13
// clear:13 — GET /api/history
app.get('/api/history', async (req, res) => {
  try {
    // clear:14
    const _resp_14 = (await db.findAll('messages', {}, { limit: 50 })).map(_revive);
    // clear:14
    return res.json(_resp_14);
  } catch (err) {
    console.error('[GET /api/history] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/history', line: 13, file: 'main.clear' }, err._clearCtx || {});
