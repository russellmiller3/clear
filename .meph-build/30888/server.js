...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"157":9,"161":10,"175":11,"177":12,"179":13,"181":14,"182":14,"187":16,"188":16,"191":17,"193":18} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   WeatherReadings: temp, humidity, conditions, fetched_at_date (line 3)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    weatherreadings: { line: 3, file: "main.clear", fields: { temp: { type: "number" }, humidity: { type: "number" }, conditions: { type: "text" }, fetched_at_date: { type: "timestamp" } } },
  },
  endpoints: {
...
setInterval(async () => {
  try {
    ...
    try {
      ...
      if (!_res.ok) { const _e = new Error(`External API error: ${_res.status} ${_res.statusText}`); _e._clearCtx = { service: 'external', line: 10, file: 'main.clear', source: 'call api' }; throw _e; }
      const _ct = _res.headers.get("content-type") || "";
      return _ct.includes("json") ? _res.json() : _res.text();
    } catch (_err) {
      if (!_err._clearCtx) { _err._clearCtx = { service: 'external', line: 10, file: 'main.clear', source: 'call api' }; }
      throw _err;
    } finally { clearTimeout(_timer); }
  })();
    // clear:11
    reading.temp = response?.temp;
    // clear:12
    reading.humidity = response?.humidity;
    // clear:13
    reading.conditions = response?.conditions;
    // clear:14
    await _clearTry(() => db.update('weatherreadings', _pick(reading, WeatherReadingsSchema)), { op: 'update', table: 'weatherreadings', line: 14, file: 'main.clear', source: "" }); // clear:14
  } catch (_err) {
