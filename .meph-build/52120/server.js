...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"264":2,"266":4,"275":11,"277":13,"278":13,"284":14,"287":16,"288":16,"289":17,"299":19,"300":19,"303":20,"305":21,"314":23,"315":23,"318":24,"320":25,"329":27,"330":27,"335":28,"337":29,"340":29,"342":30} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Requests: title*, description, amount, status (line 4)
//
// ENDPOINTS:
//   POST /api/requests (line 13)
//   GET /api/requests (line 19)
//   GET /api/requests/pending (line 23)
//   PUT /api/requests/:id [auth] (line 27)
//
// PAGES:
//   'Submit Request' at / (line 32)
//   'Approval Queue' at /approvals (line 40)
//
...
// Auth scaffolding from `allow signup and login` (clear:10)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    requests: { line: 4, file: "main.clear", fields: { title: { required: true, type: "text" }, description: { type: "text" }, amount: { type: "number" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/requests": { line: 13, file: "main.clear", auth: false },
    "GET /api/requests": { line: 19, file: "main.clear", auth: false },
    "GET /api/requests/pending": { line: 23, file: "main.clear", auth: false },
    "PUT /api/requests/:id": { line: 27, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/requests', async (req, res) => {
  try {
    ...
    // clear:16
    const saved = await _clearTry(() => db.insert('requests', _pick(request_data, RequestsSchema)), { op: 'insert', table: 'requests', line: 16, file: 'main.clear', source: "" }); // clear:16
    // clear:17
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/requests', line: 13, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/requests', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/requests', line: 19, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
...
app.get('/api/requests/pending', async (req, res) => {
  try {
    // clear:24
    const pending = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:25
...
app.put('/api/requests/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('requests', _picked_changes), { op: 'update', table: 'requests', line: 29, file: 'main.clear', source: "" }); // clear:29
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/requests/:id', line: 27, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.get("/approvals", (req, res) => res.sendFile('index.html', { root: __dirname }));
