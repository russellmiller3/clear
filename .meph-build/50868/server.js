...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counts: { line: 5, file: "main.clear", fields: { ticked: { type: "boolean" } } },
  },
  endpoints: {
...
app.post('/api/increment', async (req, res) => {
  try {
    // clear:13
    let tick = { ticked: true };
    // clear:14
    await _clearTry(() => db.insert('counts', _pick(tick, CountsSchema)), { op: 'insert', table: 'counts', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    let total = await db.aggregate('counts', 'COUNT', 'id', {});
    // clear:16
    return res.json({ count: total });
