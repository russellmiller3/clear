...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"176":3,"178":5,"186":10,"187":10,"193":11,"196":14,"197":14,"198":15,"208":17,"209":17,"212":18,"214":18,"223":20,"224":20,"227":21,"229":22,"230":22,"231":23} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Bookmarks: url*, title*, tags (line 5)
//
// ENDPOINTS:
//   POST /api/bookmarks (line 10)
//   GET /api/bookmarks (line 17)
//   DELETE /api/bookmarks/:id [auth] (line 20)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    bookmarks: { line: 5, file: "main.clear", fields: { url: { required: true, type: "text" }, title: { required: true, type: "text" }, tags: { type: "text" } } },
  },
  endpoints: {
    "POST /api/bookmarks": { line: 10, file: "main.clear", auth: false },
    "GET /api/bookmarks": { line: 17, file: "main.clear", auth: false },
    "DELETE /api/bookmarks/:id": { line: 20, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    // clear:11
    const _vErrs = _validate(req.body, [{"field":"url","type":"text","required":true,"matches":"url"}, {"field":"title","type":"text","required":true}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:14
    const saved = await _clearTry(() => db.insert('bookmarks', _pick(bookmark, BookmarksSchema)), { op: 'insert', table: 'bookmarks', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/bookmarks] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/bookmarks', line: 10, file: 'main.clear' }, err._clearCtx || {});
...
// clear:17
// clear:17 — GET /api/bookmarks
app.get('/api/bookmarks', async (req, res) => {
  try {
    // clear:18
    const _resp_18 = (await db.findAll('bookmarks', {}, { limit: 50 })).map(_revive);
    // clear:18
    return res.json(_resp_18);
  } catch (err) {
    console.error('[GET /api/bookmarks] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/bookmarks', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:20
// clear:20 — DELETE /api/bookmarks/:id
app.delete('/api/bookmarks/:id', async (req, res) => {
  try {
    // clear:21
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:22
    await _clearTry(() => db.remove('bookmarks', { id: req.params.id }), { op: 'remove', table: 'bookmarks', line: 22, file: 'main.clear', source: "" }); // clear:22
    // clear:23
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/bookmarks/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/bookmarks/:id', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
