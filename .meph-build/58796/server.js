...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"152":3,"154":5,"162":10,"163":10,"171":11,"173":14,"174":14,"175":15,"177":18,"178":18,"179":19,"181":22,"182":22,"183":23,"193":25,"194":25,"197":26,"199":27,"201":28,"203":29} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/kpis": { line: 25, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/seed', async (req, res) => {
  try {
    ...
    // clear:19
    let s3 = { amount: 280, region: "North" };
    // clear:22
...
// clear:25
// clear:25 — GET /api/kpis
app.get('/api/kpis', async (req, res) => {
  try {
    // clear:26
    let rev = await db.aggregate('sales', 'SUM', 'amount', {});
    // clear:27
    let cnt = await db.aggregate('sales', 'COUNT', 'id', {});
    // clear:28
    let avg_sz = await db.aggregate('sales', 'AVG', 'amount', {});
    // clear:29
    return res.json({ total_revenue: rev, order_count: cnt, avg_order: avg_sz });
  } catch (err) {
    console.error('[GET /api/kpis] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/kpis', line: 25, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
