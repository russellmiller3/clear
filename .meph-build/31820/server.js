...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"148":3,"157":9,"158":9,"163":10,"165":11,"167":12,"169":13,"171":14,"173":15,"174":15,"175":16} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 3, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, name: { required: true, type: "text" }, age: { required: true, type: "text" } } },
  },
  endpoints: {
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // clear:10
    if (!(registration?.email != "")) { return res.status(403).json({ error: "email is required" }); }
    // clear:11
    if (!(registration?.email)) { return res.status(403).json({ error: "email must be a valid email address" }); }
    // clear:12
    if (!(_clear_len(registration)?.password >= 8)) { return res.status(403).json({ error: "password must be at least 8 characters" }); }
    // clear:13
    if (!(registration?.name != "")) { return res.status(403).json({ error: "name is required" }); }
    // clear:14
    if (!(registration?.age >= 13)) { return res.status(403).json({ error: "age must be at least 13" }); }
    // clear:15
    const saved = await _clearTry(() => db.insert('users', _pick(registration, UsersSchema)), { op: 'insert', table: 'users', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
