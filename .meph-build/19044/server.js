...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"151":3,"153":5,"159":8,"160":8,"163":9,"172":11,"173":11,"176":12,"178":13,"187":15,"188":15,"191":16,"193":17,"194":17,"195":18,"197":19} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counts: amount (line 5)
//
// ENDPOINTS:
//   POST /api/reset (line 8)
//   GET /api/count (line 11)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counts: { line: 5, file: "main.clear", fields: { amount: { type: "number" } } },
  },
  endpoints: {
    "POST /api/reset": { line: 8, file: "main.clear", auth: false },
    "GET /api/count": { line: 11, file: "main.clear", auth: false },
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 8, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:11
// clear:11 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:12
    let total = await db.aggregate('counts', 'COUNT', 'amount', {});
    // clear:13
    return res.json({ count: total });
  } catch (err) {
    console.error('[GET /api/count] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/count', line: 11, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
...
app.post('/api/increment', async (req, res) => {
  try {
    // clear:16
    let new_entry = { amount: 1 };
    // clear:17
    await _clearTry(() => db.insert('counts', _pick(new_entry, CountsSchema)), { op: 'insert', table: 'counts', line: 17, file: 'main.clear', source: "" }); // clear:17
    // clear:18
    let total = await db.aggregate('counts', 'COUNT', 'amount', {});
    // clear:19
    return res.json({ count: total });
