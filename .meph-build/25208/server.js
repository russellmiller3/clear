...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"181":18,"183":20,"193":27,"194":27,"200":28,"203":33,"204":33,"205":34} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, name*, age (line 20)
//
// ENDPOINTS:
//   POST /api/register (line 27)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 20, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, name: { required: true, type: "text" }, age: { type: "text" } } },
  },
  endpoints: {
    "POST /api/register": { line: 27, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // clear:33
    const saved = await _clearTry(() => db.insert('users', _pick(user, UsersSchema)), { op: 'insert', table: 'users', line: 33, file: 'main.clear', source: "" }); // clear:33
    // clear:34
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/register] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/register', line: 27, file: 'main.clear' }, err._clearCtx || {});
