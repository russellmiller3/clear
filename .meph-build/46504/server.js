...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"151":3,"153":5,"159":8,"160":8,"163":9,"165":10,"174":12,"175":12,"178":13,"180":14,"181":14,"182":15,"184":16,"193":18,"194":18,"197":19} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Increments: tick (line 5)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    increments: { line: 5, file: "main.clear", fields: { tick: { type: "number" } } },
  },
  endpoints: {
...
// clear:5
// Data shape: Increments
const IncrementsSchema = {
  tick: { type: "number", default: 1 }
};
db.createTable('increments', IncrementsSchema);
// clear:8
// clear:8 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    let current_count = await db.aggregate('increments', 'COUNT', 'id', {});
    // clear:10
    return res.json({ count: current_count });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    let entry = { tick: 1 };
    // clear:14
    await _clearTry(() => db.insert('increments', _pick(entry, IncrementsSchema)), { op: 'insert', table: 'increments', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    let new_count = await db.aggregate('increments', 'COUNT', 'id', {});
    // clear:16
