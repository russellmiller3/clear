...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"151":2,"153":4,"159":7,"160":7,"163":8,"165":9,"174":11,"175":11,"180":12,"181":12,"182":13,"184":14,"194":16,"195":16,"198":17} : null;
// ┌─────────────────────────────────────────────┐
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    ticks: { line: 4, file: "main.clear", fields: { value: { type: "number" } } },
  },
  endpoints: {
    "GET /api/count": { line: 7, file: "main.clear", auth: false },
    "POST /api/increment": { line: 11, file: "main.clear", auth: false },
    "POST /api/reset": { line: 16, file: "main.clear", auth: false },
  }
} : null;
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:8
    let total = await db.aggregate('ticks', 'COUNT', 'id', {});
    // clear:9
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    const tick_data = req.body;
    // clear:12
    await _clearTry(() => db.insert('ticks', _pick(tick_data, TicksSchema)), { op: 'insert', table: 'ticks', line: 12, file: 'main.clear', source: "" }); // clear:12
    // clear:13
    let total = await db.aggregate('ticks', 'COUNT', 'id', {});
    // clear:14
    return res.json({ count: total });
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
