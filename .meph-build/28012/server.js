...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"237":3,"239":5,"247":10,"255":17,"256":17,"259":18,"261":19,"263":20,"272":22,"273":22,"278":23,"280":24,"282":25,"284":26,"285":26,"286":27,"296":29,"297":29,"300":30,"302":31,"304":32,"305":32,"306":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Users: email*!, password*, role* (line 5)
//   Articles: title*, body, author_id (line 10)
//
// ENDPOINTS:
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    users: { line: 5, file: "main.clear", fields: { email: { required: true, unique: true, type: "text" }, password: { required: true, type: "text" }, role: { required: true, type: "text" } } },
    articles: { line: 10, file: "main.clear", fields: { title: { required: true, type: "text" }, body: { type: "text" }, author_id: { type: "fk" } } },
  },
  endpoints: {
...
app.post('/api/articles', async (req, res) => {
  try {
    ...
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:24
    if (!(req.user?.role == "admin" || req.user?.role == "editor")) { return res.status(403).json({ error: "Forbidden" }); }
    // clear:25
...
app.delete('/api/articles/:id', async (req, res) => {
  try {
    ...
    // clear:32
    await _clearTry(() => db.remove('articles', { id: req.params.id }), { op: 'remove', table: 'articles', line: 32, file: 'main.clear', source: "" }); // clear:32
    // clear:33
