...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"264":3,"266":5,"279":15,"282":20,"283":20,"289":21,"292":24,"293":24,"294":25,"305":29,"306":29,"309":30,"311":31,"321":35,"322":35,"325":36,"327":37,"337":41,"338":41,"344":42,"346":43,"348":44,"351":44,"353":45} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Requests: title*, description, category, priority, submitter*, status, assigned_to, resolution_notes (line 5)
//
// ENDPOINTS:
//   POST /api/requests (line 20)
//   GET /api/requests/new (line 29)
//   GET /api/requests (line 35)
//   PUT /api/requests/:id [auth] (line 41)
//
// PAGES:
//   'Submit Request' at / (line 49)
//   'Ops Triage' at /ops (line 62)
//
...
// Auth scaffolding from `allow signup and login` (clear:16)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    requests: { line: 5, file: "main.clear", fields: { title: { required: true, type: "text" }, description: { type: "text" }, category: { type: "text" }, priority: { type: "text" }, submitter: { required: true, type: "text" }, status: { type: "text" }, assigned_to: { type: "text" }, resolution_notes: { type: "text" } } },
  },
  endpoints: {
    "POST /api/requests": { line: 20, file: "main.clear", auth: false },
    "GET /api/requests/new": { line: 29, file: "main.clear", auth: false },
    "GET /api/requests": { line: 35, file: "main.clear", auth: false },
    "PUT /api/requests/:id": { line: 41, file: "main.clear", auth: true },
  }
} : null;
...
// clear:15
console.log(/* ERROR */);
// POST /api/requests — public endpoint, any employee can submit a ticket
// clear:20
// clear:20 — POST /api/requests
app.post('/api/requests', async (req, res) => {
  try {
    ...
    // clear:21
    const _vErrs = _validate(req.body, [{"field":"title","type":"text","required":true}, {"field":"submitter","type":"text","required":true}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:24
    const saved = await _clearTry(() => db.insert('requests', _pick(request_data, RequestsSchema)), { op: 'insert', table: 'requests', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/requests', line: 20, file: 'main.clear' }, err._clearCtx || {});
...
// GET /api/requests/new — filter to only new/unassigned tickets
// clear:29
// clear:29 — GET /api/requests/new
app.get('/api/requests/new', async (req, res) => {
  try {
    // clear:30
    const new_tickets = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:31
    return res.json(new_tickets);
  } catch (err) {
...
// GET /api/requests — list all tickets
// clear:35
// clear:35 — GET /api/requests
app.get('/api/requests', async (req, res) => {
  try {
    // clear:36
    const all_tickets = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:37
    return res.json(all_tickets);
  } catch (err) {
    console.error('[GET /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/requests', line: 35, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// PUT /api/requests/:id — ops staff update status, assign, add notes
// clear:41
// clear:41 — PUT /api/requests/:id
app.put('/api/requests/:id', async (req, res) => {
  try {
    ...
    // clear:43
    changes.id = incoming?.id;
    // clear:44
    const _picked_changes = _pick(changes, RequestsSchema);
    _picked_changes.id = req.params.id;
    await _clearTry(() => db.update('requests', _picked_changes), { op: 'update', table: 'requests', line: 44, file: 'main.clear', source: "" }); // clear:44
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/requests/:id', line: 41, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// Employee submit page
// Ops triage view
