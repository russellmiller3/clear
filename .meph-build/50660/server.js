...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"236":2,"238":5,"240":7,"247":11,"248":11,"253":12,"254":12,"255":13,"265":15,"266":15,"269":16,"271":17,"275":19,"284":21,"285":21,"289":22,"291":23,"292":23,"293":24} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Store: key!, value (line 7)
//
// ENDPOINTS:
//   POST /api/store (line 11)
//   GET /api/store/:key (line 15)
//   DELETE /api/store/:key [auth] (line 21)
//
...
const _OWNER_EMAIL = "admin@store.com";
const _users = [];
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    stores: { line: 7, file: "main.clear", fields: { key: { unique: true, type: "text" }, value: { type: "text" } } },
  },
  endpoints: {
    "POST /api/store": { line: 11, file: "main.clear", auth: false },
    "GET /api/store/:key": { line: 15, file: "main.clear", auth: false },
    "DELETE /api/store/:key": { line: 21, file: "main.clear", auth: true },
  }
} : null;
...
// clear:11
// clear:11 — POST /api/store
app.post('/api/store', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const entry = req.body;
    // clear:12
    await _clearTry(() => db.update('stores', _pick(entry, StoreSchema)), { op: 'update', table: 'stores', line: 12, file: 'main.clear', source: "" }); // clear:12
    // clear:13
    return res.json(entry);
  } catch (err) {
    console.error('[POST /api/store] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/store', line: 11, file: 'main.clear' }, err._clearCtx || {});
...
// clear:15
// clear:15 — GET /api/store/:key
app.get('/api/store/:key', async (req, res) => {
  try {
    // clear:16
    const items = (await db.findAll('stores', {}, { limit: 50 })).map(_revive);
    // clear:17
    if (items == null) {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/store/:key', line: 15, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:21
// clear:21 — DELETE /api/store/:key
app.delete('/api/store/:key', async (req, res) => {
  try {
    ...
    // clear:23
    await _clearTry(() => db.remove('stores', { key: incoming?.key }), { op: 'remove', table: 'stores', line: 23, file: 'main.clear', source: "" }); // clear:23
    // clear:24
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/store/:key] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/store/:key', line: 21, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
