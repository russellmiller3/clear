...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":3,"155":5,"161":8,"162":8,"165":9,"167":10,"169":11,"178":13,"179":13,"182":14,"184":15,"185":15,"186":16,"188":17,"190":18,"199":20,"200":20,"203":21} : null;
// ┌─────────────────────────────────────────────┐
...
//   POST /api/increment (line 13)
//   POST /api/reset (line 20)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counts: { line: 5, file: "main.clear", fields: { note: { type: "text" } } },
  },
  endpoints: {
    "GET /api/count": { line: 8, file: "main.clear", auth: false },
    "POST /api/increment": { line: 13, file: "main.clear", auth: false },
    "POST /api/reset": { line: 20, file: "main.clear", auth: false },
  }
} : null;
...
// Data shape: Counts
const CountsSchema = {
  note: { type: "text", default: "increment" }
};
db.createTable('counts', CountsSchema);
// clear:8
// clear:8 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    const all_rows = (await db.findAll('counts', {}, { limit: 50 })).map(_revive);
    // clear:10
    let total = _clear_len(all_rows);
    // clear:11
...
// clear:13
// clear:13 — POST /api/increment
app.post('/api/increment', async (req, res) => {
  try {
    // clear:14
    let entry = { note: "increment" };
    // clear:15
    await _clearTry(() => db.insert('counts', _pick(entry, CountsSchema)), { op: 'insert', table: 'counts', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
    const all_rows = (await db.findAll('counts', {}, { limit: 50 })).map(_revive);
    // clear:17
    let total = _clear_len(all_rows);
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 13, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:20
// clear:20 — POST /api/reset
app.post('/api/reset', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
