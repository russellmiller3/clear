...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"147":3,"149":5,"158":11,"159":11,"164":12,"166":13,"168":14,"169":14,"170":15} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Events: event_type*, amount, customer_email, received_at_date (line 5)
//
// ENDPOINTS:
//   POST /webhook/stripe (line 11)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    events: { line: 5, file: "main.clear", fields: { event_type: { required: true, type: "text" }, amount: { type: "number" }, customer_email: { type: "text" }, received_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "POST /webhook/stripe": { line: 11, file: "main.clear", auth: false },
  }
} : null;
...
const EventsSchema = {
  ...
  received_at_date: { type: "timestamp", auto: true }
};
db.createTable('events', EventsSchema);
// clear:11
// clear:11 — POST /webhook/stripe
app.post('/webhook/stripe', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const payload = req.body;
    // clear:12
    if (!(payload?.signature != null)) { return res.status(403).json({ error: "Missing signature" }); }
    // clear:13
    payload.event_type = payload?.type;
    // clear:14
    await _clearTry(() => db.insert('events', _pick(payload, EventsSchema)), { op: 'insert', table: 'events', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    return res.json({ received: true });
  } catch (err) {
    console.error('[POST /webhook/stripe] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /webhook/stripe', line: 11, file: 'main.clear' }, err._clearCtx || {});
