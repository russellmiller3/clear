...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 5, file: "main.clear", fields: { count: { type: "number" } } },
  },
  endpoints: {
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    const all_rows = (await db.findAll('counters', {}, { limit: 50 })).map(_revive);
    // clear:10
    if (_clear_len(all_rows) == 0) {
      return res.json({ count: 0 });
    }
    // clear:12
    let the_row = all_rows[0];
    // clear:13
    return res.json({ count: the_row?.count });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    // clear:17
    if (_clear_len(all_rows) == 0) {
      let first_entry = { count: 1 };
      await _clearTry(() => db.insert('counters', _pick(first_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 19, file: 'main.clear', source: "" }); // clear:19
      return res.json({ count: 1 });
    }
    // clear:21
    let the_row = all_rows[0];
    // clear:22
    let new_count = (the_row?.count + 1);
    // clear:23
    the_row.count = new_count;
    // clear:24
    await _clearTry(() => db.update('counters', _pick(the_row, CountersSchema)), { op: 'update', table: 'counters', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
    return res.json({ count: new_count });
  } catch (err) {
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    // clear:29
    if (_clear_len(all_rows) == 0) {
      let zero_entry = { count: 0 };
      await _clearTry(() => db.insert('counters', _pick(zero_entry, CountersSchema)), { op: 'insert', table: 'counters', line: 31, file: 'main.clear', source: "" }); // clear:31
      return res.json({ count: 0 });
    }
    // clear:33
    let the_row = all_rows[0];
    ...
    the_row.count = 0;
    // clear:35
    await _clearTry(() => db.update('counters', _pick(the_row, CountersSchema)), { op: 'update', table: 'counters', line: 35, file: 'main.clear', source: "" }); // clear:35
    // clear:36
    return res.json({ count: 0 });
  } catch (err) {
