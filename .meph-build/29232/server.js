...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"141":3,"149":8,"154":9,"156":10,"158":11,"175":13,"176":13,"179":14,"181":15} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Sessions: user_id*, token*, created_at_date (line 3)
//
// ENDPOINTS:
//   GET /api/session-count (line 13)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    sessions: { line: 3, file: "main.clear", fields: { user_id: { required: true, type: "fk" }, token: { required: true, type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/session-count": { line: 13, file: "main.clear", auth: false },
  }
} : null;
...
// clear:13
// clear:13 — GET /api/session-count
app.get('/api/session-count', async (req, res) => {
  try {
    // clear:14
    let total = await db.aggregate('sessions', 'COUNT', 'id', {});
    // clear:15
    return res.json({ count: total });
  } catch (err) {
    console.error('[GET /api/session-count] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/session-count', line: 13, file: 'main.clear' }, err._clearCtx || {});
