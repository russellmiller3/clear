...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    increments: { line: 5, file: "main.clear", fields: { item: { type: "number" } } },
  },
  endpoints: {
...
app.get('/api/count', async (req, res) => {
  try {
    // clear:9
    let total = await db.aggregate('increments', 'COUNT', 'id', {});
    // clear:10
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    let new_row = { item: 1 };
    // clear:14
    await _clearTry(() => db.insert('increments', _pick(new_row, IncrementsSchema)), { op: 'insert', table: 'increments', line: 14, file: 'main.clear', source: "" }); // clear:14
    // clear:15
    let total = await db.aggregate('increments', 'COUNT', 'id', {});
    // clear:16
    return res.json({ count: total });
