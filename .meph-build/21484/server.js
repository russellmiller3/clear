...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 4, file: "main.clear", fields: { count: { type: "number" } } },
  },
  endpoints: {
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    // clear:9
    if (_clear_len(counters)) {
      let entry = { count: 0 };
      await _clearTry(() => db.insert('counters', _pick(entry, CounterSchema)), { op: 'insert', table: 'counters', line: 11, file: 'main.clear', source: "" }); // clear:11
    ...
    counter.count = 0;
    // clear:15
    await _clearTry(() => db.update('counters', _pick(counter, CounterSchema)), { op: 'update', table: 'counters', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
    return res.json({ count: 0 });
  } catch (err) {
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    if (_clear_len(counters)) {
      let entry = { count: 1 };
      await _clearTry(() => db.insert('counters', _pick(entry, CounterSchema)), { op: 'insert', table: 'counters', line: 29, file: 'main.clear', source: "" }); // clear:29
    ...
    // clear:34
    await _clearTry(() => db.update('counters', _pick(counter, CounterSchema)), { op: 'update', table: 'counters', line: 34, file: 'main.clear', source: "" }); // clear:34
    // clear:35
