...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"264":3,"266":4,"268":6,"277":14,"278":14,"284":15,"287":17,"288":17,"289":18,"299":20,"300":20,"303":21,"305":21,"314":23,"315":23,"318":24,"320":24,"329":26,"330":26,"335":27,"337":28,"340":28,"342":29} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Requests: title*, description, amount, status (line 6)
//
// ENDPOINTS:
//   POST /api/requests (line 14)
//   GET /api/requests (line 20)
//   GET /api/requests/pending (line 23)
//   PUT /api/requests/:id [auth] (line 26)
//
// PAGES:
//   'Submit Request' at / (line 31)
//   'Approval Queue' at /approve (line 39)
//
...
// Auth scaffolding from `allow signup and login` (clear:12)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    requests: { line: 6, file: "main.clear", fields: { title: { required: true, type: "text" }, description: { type: "text" }, amount: { type: "number" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/requests": { line: 14, file: "main.clear", auth: false },
    "GET /api/requests": { line: 20, file: "main.clear", auth: false },
    "GET /api/requests/pending": { line: 23, file: "main.clear", auth: false },
    "PUT /api/requests/:id": { line: 26, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/requests', async (req, res) => {
  try {
    ...
    // clear:15
    const _vErrs = _validate(req.body, [{"field":"title","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:17
    const saved = await _clearTry(() => db.insert('requests', _pick(entry, RequestsSchema)), { op: 'insert', table: 'requests', line: 17, file: 'main.clear', source: "" }); // clear:17
    // clear:18
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/requests', line: 14, file: 'main.clear' }, err._clearCtx || {});
...
// clear:20
// clear:20 — GET /api/requests
app.get('/api/requests', async (req, res) => {
  try {
    // clear:21
    const _resp_21 = (await db.findAll('requests', {}, { limit: 50 })).map(_revive);
    // clear:21
    return res.json(_resp_21);
  } catch (err) {
    console.error('[GET /api/requests] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/requests', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:23
// clear:23 — GET /api/requests/pending
app.get('/api/requests/pending', async (req, res) => {
  try {
    // clear:24
    const _resp_24 = (await db.findAll('requests', { status: "pending" }, { limit: 50 })).map(_revive);
    // clear:24
    return res.json(_resp_24);
  } catch (err) {
    console.error('[GET /api/requests/pending] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/requests/pending', line: 23, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:26
// clear:26 — PUT /api/requests/:id
app.put('/api/requests/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('requests', _picked_changes), { op: 'update', table: 'requests', line: 28, file: 'main.clear', source: "" }); // clear:28
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/requests/:id', line: 26, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
app.get("/approve", (req, res) => res.sendFile('index.html', { root: __dirname }));
