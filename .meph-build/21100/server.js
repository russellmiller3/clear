...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"260":4,"276":23,"286":31,"287":31,"293":32,"296":35,"297":35,"298":36,"308":38,"309":38,"312":39,"314":40,"316":41,"325":43,"326":43,"329":44,"331":45,"333":46,"335":47,"338":47,"340":48,"349":50,"350":50,"353":51,"355":52,"357":53,"359":54,"362":54,"364":55} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Reviews: author_email*, content*, status, created_at_date (line 23)
//
// ENDPOINTS:
//   POST /api/reviews (line 31)
//   GET /api/reviews/pending (line 38)
//   PUT /api/reviews/:id/approve (line 43)
//   PUT /api/reviews/:id/reject (line 50)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    reviews: { line: 23, file: "main.clear", fields: { author_email: { required: true, type: "text" }, content: { required: true, type: "text" }, status: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "POST /api/reviews": { line: 31, file: "main.clear", auth: false },
    "GET /api/reviews/pending": { line: 38, file: "main.clear", auth: true },
    "PUT /api/reviews/:id/approve": { line: 43, file: "main.clear", auth: true },
    "PUT /api/reviews/:id/reject": { line: 50, file: "main.clear", auth: true },
  }
} : null;
...
app.post('/api/reviews', async (req, res) => {
  try {
    ...
    // clear:32
    const _vErrs = _validate(req.body, [{"field":"author_email","type":"text","required":true}, {"field":"content","type":"text","required":true,"min":1,"max":500}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:35
    const saved = await _clearTry(() => db.insert('reviews', _pick(review, ReviewsSchema)), { op: 'insert', table: 'reviews', line: 35, file: 'main.clear', source: "" }); // clear:35
    // clear:36
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/reviews] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reviews', line: 31, file: 'main.clear' }, err._clearCtx || {});
...
// clear:38
// clear:38 — GET /api/reviews/pending
app.get('/api/reviews/pending', async (req, res) => {
  try {
    // clear:39
    if (req.user.role !== "admin") { return res.status(403).json({ error: "Requires role: admin" }); }
    // clear:40
    const pending_reviews = (await db.findAll('reviews', {}, { limit: 50 })).map(_revive);
    // clear:41
    return res.json(pending_reviews);
  } catch (err) {
    console.error('[GET /api/reviews/pending] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/reviews/pending', line: 38, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:43
// clear:43 — PUT /api/reviews/:id/approve
app.put('/api/reviews/:id/approve', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('reviews', _picked_review), { op: 'update', table: 'reviews', line: 47, file: 'main.clear', source: "" }); // clear:47
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/reviews/:id/approve', line: 43, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:50
// clear:50 — PUT /api/reviews/:id/reject
app.put('/api/reviews/:id/reject', async (req, res) => {
  try {
    ...
    const _picked_review = _pick(review, ReviewsSchema);
    _picked_review.id = req.params.id;
    await _clearTry(() => db.update('reviews', _picked_review), { op: 'update', table: 'reviews', line: 54, file: 'main.clear', source: "" }); // clear:54
    Object.assign(review, await db.findOne('reviews', { id: _picked_review.id }) || {});
    // clear:55
    return res.json(review);
  } catch (err) {
    console.error('[PUT /api/reviews/:id/reject] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/reviews/:id/reject', line: 50, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
