...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"153":2,"155":4,"161":7,"162":7,"165":8,"167":9,"170":11,"173":13,"175":14,"177":15,"178":15,"179":16,"188":18,"189":18,"192":19,"194":20,"198":22,"200":23,"209":25,"210":25,"213":26,"215":27,"218":29,"221":31,"223":32,"225":33,"227":34,"228":34,"229":35} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counter: value (line 4)
//
// ENDPOINTS:
//   POST /api/reset (line 7)
//   GET /api/count (line 18)
//   POST /api/increment (line 25)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 4, file: "main.clear", fields: { value: { type: "number" } } },
  },
  endpoints: {
    "POST /api/reset": { line: 7, file: "main.clear", auth: false },
    "GET /api/count": { line: 18, file: "main.clear", auth: false },
    "POST /api/increment": { line: 25, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    if (_clear_len(all_rows) == 0) {
      ...
      await _clearTry(() => db.insert('counters', _pick(zero_entry, CounterSchema)), { op: 'insert', table: 'counters', line: 11, file: 'main.clear', source: "" }); // clear:11
    ...
    row.value = 0;
    // clear:15
    await _clearTry(() => db.update('counters', _pick(row, CounterSchema)), { op: 'update', table: 'counters', line: 15, file: 'main.clear', source: "" }); // clear:15
    // clear:16
    return res.json({ count: 0 });
  } catch (err) {
    console.error('[POST /api/reset] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/reset', line: 7, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:18
// clear:18 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/count', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:25
// clear:25 — POST /api/increment
app.post('/api/increment', async (req, res) => {
  try {
    ...
    // clear:27
    if (_clear_len(all_rows) == 0) {
      let one_entry = { value: 1 };
      await _clearTry(() => db.insert('counters', _pick(one_entry, CounterSchema)), { op: 'insert', table: 'counters', line: 29, file: 'main.clear', source: "" }); // clear:29
      return res.json({ count: 1 });
    }
    // clear:31
    let row = all_rows[0];
    // clear:32
    let new_val = (row?.value + 1);
    // clear:33
    row.value = new_val;
    // clear:34
    await _clearTry(() => db.update('counters', _pick(row, CounterSchema)), { op: 'update', table: 'counters', line: 34, file: 'main.clear', source: "" }); // clear:34
    // clear:35
    return res.json({ count: new_val });
  } catch (err) {
    console.error('[POST /api/increment] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 25, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
