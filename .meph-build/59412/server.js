...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"152":2,"154":4,"160":7,"161":7,"164":8,"166":9,"167":9,"168":10,"177":12,"178":12,"181":13,"183":14,"185":15,"194":17,"195":17,"198":18,"200":19,"202":20,"204":21,"206":22,"207":22,"208":23} : null;
// ┌─────────────────────────────────────────────┐
...
//   GET /api/count (line 12)
//   POST /api/increment (line 17)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    states: { line: 4, file: "main.clear", fields: { count: { type: "number" } } },
  },
  endpoints: {
    "POST /api/reset": { line: 7, file: "main.clear", auth: false },
    "GET /api/count": { line: 12, file: "main.clear", auth: false },
    "POST /api/increment": { line: 17, file: "main.clear", auth: false },
  }
} : null;
...
db.createTable('states', StateSchema);
// clear:7
// clear:7 — POST /api/reset
app.post('/api/reset', async (req, res) => {
  try {
    // clear:8
    let new_state = { count: 0 };
    // clear:9
    await _clearTry(() => db.insert('states', _pick(new_state, StateSchema)), { op: 'insert', table: 'states', line: 9, file: 'main.clear', source: "" }); // clear:9
    // clear:10
...
// clear:12
// clear:12 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:13
    const states = (await db.findAll('states', {}, { limit: 50 })).map(_revive);
    // clear:14
    let current = states[states.length - 1];
    // clear:15
    return res.json({ count: current?.count });
  } catch (err) {
    console.error('[GET /api/count] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/count', line: 12, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:17
// clear:17 — POST /api/increment
app.post('/api/increment', async (req, res) => {
  try {
    ...
    // clear:21
    let new_state = { count: new_count };
    // clear:22
    await _clearTry(() => db.insert('states', _pick(new_state, StateSchema)), { op: 'insert', table: 'states', line: 22, file: 'main.clear', source: "" }); // clear:22
    // clear:23
    return res.json({ count: new_count });
  } catch (err) {
    console.error('[POST /api/increment] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/increment', line: 17, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
