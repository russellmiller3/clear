...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"178":3,"180":5,"189":11,"190":11,"193":12,"195":12,"204":14,"205":14,"211":15,"214":18,"215":18,"216":19,"226":21,"227":21,"232":22,"234":23,"237":23,"239":24,"249":26,"250":26,"253":27,"255":28,"256":28,"257":29} : null;
// ┌─────────────────────────────────────────────┐
...
//
// ENDPOINTS:
//   GET /api/contacts (line 11)
//   POST /api/contacts (line 14)
//   PUT /api/contacts/:id [auth] (line 21)
//   DELETE /api/contacts/:id [auth] (line 26)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    "GET /api/contacts": { line: 11, file: "main.clear", auth: false },
    "POST /api/contacts": { line: 14, file: "main.clear", auth: false },
    "PUT /api/contacts/:id": { line: 21, file: "main.clear", auth: true },
    "DELETE /api/contacts/:id": { line: 26, file: "main.clear", auth: true },
  }
} : null;
...
// clear:11
// clear:11 — GET /api/contacts
app.get('/api/contacts', async (req, res) => {
  try {
    // clear:12
    const _resp_12 = (await db.findAll('contacts', {}, { limit: 50 })).map(_revive);
    // clear:12
    return res.json(_resp_12);
  } catch (err) {
    console.error('[GET /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 11, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:14
// clear:14 — POST /api/contacts
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    // clear:18
    const saved = await _clearTry(() => db.insert('contacts', _pick(contact, ContactsSchema)), { op: 'insert', table: 'contacts', line: 18, file: 'main.clear', source: "" }); // clear:18
    // clear:19
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/contacts', line: 14, file: 'main.clear' }, err._clearCtx || {});
...
app.put('/api/contacts/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('contacts', _picked_changes), { op: 'update', table: 'contacts', line: 23, file: 'main.clear', source: "" }); // clear:23
    ...
    const _ctx = Object.assign({ endpoint: 'PUT /api/contacts/:id', line: 21, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:26
// clear:26 — DELETE /api/contacts/:id
app.delete('/api/contacts/:id', async (req, res) => {
  try {
    // clear:27
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    // clear:28
    await _clearTry(() => db.remove('contacts', { id: req.params.id }), { op: 'remove', table: 'contacts', line: 28, file: 'main.clear', source: "" }); // clear:28
    // clear:29
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/contacts/:id', line: 26, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
