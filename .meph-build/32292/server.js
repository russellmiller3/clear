...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"261":3,"263":5,"265":8,"276":16,"277":16,"283":17,"285":18,"288":21,"290":22,"294":24,"298":26,"299":26,"300":27,"310":29,"311":29,"314":30,"316":30,"325":32,"326":32,"329":33,"331":34} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Leads: name*, email*, company, size, assigned_to, status (line 8)
//
// ENDPOINTS:
//   POST /api/leads [auth] (line 16)
//   GET /api/leads (line 29)
//   GET /api/leads/new (line 32)
//
// PAGES:
//   'Lead Router' at / (line 36)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    leads: { line: 8, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, company: { type: "text" }, size: { type: "text" }, assigned_to: { type: "text" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/leads": { line: 16, file: "main.clear", auth: true },
    "GET /api/leads": { line: 29, file: "main.clear", auth: false },
    "GET /api/leads/new": { line: 32, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:18
    const _vErrs = _validate(req.body, [{"field":"name","type":"text"}, {"field":"email","type":"text"}]);
    ...
    // clear:26
    const saved = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 26, file: 'main.clear', source: "" }); // clear:26
    // clear:27
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 16, file: 'main.clear' }, err._clearCtx || {});
...
// clear:29
// clear:29 — GET /api/leads
app.get('/api/leads', async (req, res) => {
  try {
    // clear:30
    const _resp_30 = (await db.findAll('leads', {}, { limit: 50 })).map(_revive);
    // clear:30
    return res.json(_resp_30);
  } catch (err) {
    console.error('[GET /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 29, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:32
// clear:32 — GET /api/leads/new
app.get('/api/leads/new', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/leads/new', line: 32, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
