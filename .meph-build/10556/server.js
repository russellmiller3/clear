...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"237":3,"245":9,"247":11,"248":11,"251":12,"253":13,"262":15,"263":15,"268":16,"270":17,"271":17,"272":18} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Orders: amount*, status, created_at_date (line 3)
//
// ENDPOINTS:
//   GET /api/orders (line 11)
//   POST /api/orders [auth] (line 15)
//
// PAGES:
//   'Dashboard' at / (line 20)
//
...
// Auth scaffolding from `allow signup and login` (clear:8)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    orders: { line: 3, file: "main.clear", fields: { amount: { required: true, type: "number" }, status: { type: "text" }, created_at_date: { type: "timestamp" } } },
  },
  endpoints: {
    "GET /api/orders": { line: 11, file: "main.clear", auth: false },
    "POST /api/orders": { line: 15, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/orders', line: 11, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:15
// clear:15 — POST /api/orders
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // clear:17
    const saved = await _clearTry(() => db.insert('orders', _pick(order, OrdersSchema)), { op: 'insert', table: 'orders', line: 17, file: 'main.clear', source: "" }); // clear:17
    // clear:18
    return res.json(saved);
  } catch (err) {
    console.error('[POST /api/orders] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/orders', line: 15, file: 'main.clear' }, err._clearCtx || {});
