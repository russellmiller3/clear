...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"177":3,"179":5,"189":12,"191":13,"193":14,"196":15,"198":16,"201":17,"204":19,"205":19,"211":20,"214":23,"216":24,"217":24,"218":25,"228":27,"229":27,"232":28,"234":29,"243":31,"244":31,"247":32,"249":33} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Leads: name*, email*, size, assigned_to, status (line 5)
//
// ENDPOINTS:
//   POST /api/leads (line 19)
//   GET /api/leads (line 27)
//   GET /api/leads/new (line 31)
//
// PAGES:
//   'Lead Intake' at / (line 35)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    leads: { line: 5, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, size: { type: "text" }, assigned_to: { type: "text" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/leads": { line: 19, file: "main.clear", auth: false },
    "GET /api/leads": { line: 27, file: "main.clear", auth: false },
    "GET /api/leads/new": { line: 31, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // clear:23
    lead.assigned_to = get_assigned_person(lead?.size);
    // clear:24
    const saved = await _clearTry(() => db.insert('leads', _pick(lead, LeadsSchema)), { op: 'insert', table: 'leads', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/leads] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/leads', line: 19, file: 'main.clear' }, err._clearCtx || {});
...
app.get('/api/leads', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/leads', line: 27, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:31
// clear:31 — GET /api/leads/new
app.get('/api/leads/new', async (req, res) => {
  try {
    // clear:32
    const new_leads = (await db.findAll('leads', {}, { limit: 50 })).map(_revive);
    // clear:33
    return res.json(new_leads);
  } catch (err) {
    console.error('[GET /api/leads/new] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/leads/new', line: 31, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
