...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"152":3,"154":5,"161":9,"162":9,"165":10,"167":11,"180":16,"181":16,"184":17,"186":18,"189":20,"193":24,"203":27,"204":27,"207":28,"209":29,"212":31,"216":35} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Counters: name!, count (line 5)
//
// ENDPOINTS:
//   GET /api/count (line 9)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    counters: { line: 5, file: "main.clear", fields: { name: { unique: true, type: "text" }, count: { type: "number" } } },
  },
  endpoints: {
    "GET /api/count": { line: 9, file: "main.clear", auth: false },
...
// clear:9
// clear:9 — GET /api/count
app.get('/api/count', async (req, res) => {
  try {
    // clear:10
    const row = (await db.findAll('counters', { name: "main" }, { limit: 50 })).map(_revive);
    // clear:11
    if (row) {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/count', line: 9, file: 'main.clear' }, err._clearCtx || {});
...
app.post('/api/increment', async (req, res) => {
  try {
    ...
    if (row) {
      ...
      await _clearTry(() => db.update('counters', _pick(row, CountersSchema)), { op: 'update', table: 'counters', line: 20, file: 'main.clear', source: "" }); // clear:20
      return res.json({ count: row?.count });
    } else {
      let new_row = { name: "main", count: 1 };
      await _clearTry(() => db.insert('counters', _pick(new_row, CountersSchema)), { op: 'insert', table: 'counters', line: 24, file: 'main.clear', source: "" }); // clear:24
...
app.post('/api/reset', async (req, res) => {
  try {
    ...
    if (row) {
      row.count = 0;
      await _clearTry(() => db.update('counters', _pick(row, CountersSchema)), { op: 'update', table: 'counters', line: 31, file: 'main.clear', source: "" }); // clear:31
      return res.json({ count: 0 });
    } else {
      let zero_row = { name: "main", count: 0 };
      await _clearTry(() => db.insert('counters', _pick(zero_row, CountersSchema)), { op: 'insert', table: 'counters', line: 35, file: 'main.clear', source: "" }); // clear:35
      return res.json({ count: 0 });
    }
  } catch (err) {
