...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"259":3,"261":5,"268":11,"277":17,"278":17,"284":18,"287":21,"288":21,"289":22,"299":24,"300":24,"303":25,"305":26,"307":27,"309":28,"318":30,"319":30,"322":31,"324":32,"326":33} : null;
// ┌─────────────────────────────────────────────┐
...
//   GET /api/stats/daily [auth] (line 30)
//
...
// Auth scaffolding from `allow signup and login` (clear:9)
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  ...
  endpoints: {
    ...
    "GET /api/stats/daily": { line: 30, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    // clear:27
    const all_events = (await db.findAll('events')).map(_revive);
    // clear:28
    return res.json({ total: total_events, events: all_events });
  } catch (err) {
...
app.get('/api/stats/daily', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/stats/daily', line: 30, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
