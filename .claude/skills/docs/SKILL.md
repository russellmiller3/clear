...
8. **`studio/system-prompt.md`** — **Meph's live system prompt in Studio.** This is what actually ships to end users — if a new feature or syntax isn't here, Meph won't know about it and users will get stale guidance. Every new syntax, every behavior change, every new input type, every new directive needs a mention here with a canonical example.

> Note: `.claude/skills/write-clear/SKILL.md` used to be in this list but was removed (Session 38). `AI-INSTRUCTIONS.md` already covers conventions for Claude; `studio/system-prompt.md` is what actually ships to Meph. The write-clear skill is redundant surface to maintain.
