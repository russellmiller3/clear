...
//
// TABLES:
//   Books: title*, author*, status (line 4)
//
// ENDPOINTS:
//   POST /api/books [auth] (line 9)
//   PUT /api/books/:id [auth] (line 14)
//   GET /api/books (line 20)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    books: { line: 4, file: "main.clear", fields: { title: { required: true, type: "text" }, author: { required: true, type: "text" }, status: { type: "text" } } },
  },
  endpoints: {
    "POST /api/books": { line: 9, file: "main.clear", auth: true },
    "PUT /api/books/:id": { line: 14, file: "main.clear", auth: true },
    "GET /api/books": { line: 20, file: "main.clear", auth: false },
  }
} : null;
...
db.createTable('books', BooksSchema);
// clear:9
// clear:9 — POST /api/books
app.post('/api/books', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const book = req.body;
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    const new_book = await _clearTry(() => db.insert('books', _pick(book, BooksSchema)), { op: 'insert', table: 'books', line: 11, file: 'main.clear', source: "" }); // clear:11
    return res.json(new_book);
  } catch (err) {
    console.error('[POST /api/books] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/books', line: 9, file: 'main.clear' }, err._clearCtx || {});
...
app.put('/api/books/:id', async (req, res) => {
  try {
    ...
    const _picked_book = _pick(book, BooksSchema);
    _picked_book.id = req.params.id;
    await _clearTry(() => db.update('books', _picked_book), { op: 'update', table: 'books', line: 17, file: 'main.clear', source: "" }); // clear:17
    Object.assign(book, await db.findOne('books', { id: _picked_book.id }) || {});
    return res.json({ message: "returned" });
  } catch (err) {
    console.error('[PUT /api/books/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/books/:id', line: 14, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:20
// clear:20 — GET /api/books
app.get('/api/books', async (req, res) => {
  try {
    const books = await db.findAll('books');
    return res.json(books);
  } catch (err) {
    console.error('[GET /api/books] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/books', line: 20, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
