...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"199":2,"201":4,"209":9,"210":9,"216":10,"219":13,"220":13,"221":14,"231":16,"232":16,"235":17,"237":17,"246":19,"247":19,"251":20,"253":20} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Posts: title*, body*, author (line 4)
//
// ENDPOINTS:
//   POST /api/posts (line 9)
//   GET /api/posts (line 16)
//   GET /api/posts/:id (line 19)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    posts: { line: 4, file: "main.clear", fields: { title: { required: true, type: "text" }, body: { required: true, type: "text" }, author: { type: "text" } } },
  },
  endpoints: {
    "POST /api/posts": { line: 9, file: "main.clear", auth: false },
    "GET /api/posts": { line: 16, file: "main.clear", auth: false },
    "GET /api/posts/:id": { line: 19, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    // clear:10
    const _vErrs = _validate(req.body, [{"field":"title","type":"text"}, {"field":"body","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:13
    const saved = await _clearTry(() => db.insert('posts', _pick(article, PostsSchema)), { op: 'insert', table: 'posts', line: 13, file: 'main.clear', source: "" }); // clear:13
    // clear:14
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/posts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/posts', line: 9, file: 'main.clear' }, err._clearCtx || {});
...
// clear:16
// clear:16 — GET /api/posts
app.get('/api/posts', async (req, res) => {
  try {
    // clear:17
    const _resp_17 = (await db.findAll('posts', {}, { limit: 50 })).map(_revive);
    // clear:17
    return res.json(_resp_17);
  } catch (err) {
    console.error('[GET /api/posts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/posts', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:19
// clear:19 — GET /api/posts/:id
app.get('/api/posts/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:20
    const _resp_20 = _revive(await db.findOne('posts', { id: incoming?.id }));
    // clear:20
    return res.json(_resp_20);
  } catch (err) {
    console.error('[GET /api/posts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/posts/:id', line: 19, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
