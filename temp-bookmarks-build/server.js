...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"199":3,"207":8,"208":8,"214":9,"217":13,"218":13,"219":14,"229":16,"230":16,"233":17,"235":17,"244":19,"245":19,"248":20,"249":20,"250":21} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Bookmarks: url*, title*, tags (line 3)
//
// ENDPOINTS:
//   POST /api/bookmarks (line 8)
//   GET /api/bookmarks (line 16)
//   DELETE /api/bookmarks/:id (line 19)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    bookmarks: { line: 3, file: "main.clear", fields: { url: { required: true, type: "text" }, title: { required: true, type: "text" }, tags: { type: "text" } } },
  },
  endpoints: {
    "POST /api/bookmarks": { line: 8, file: "main.clear", auth: false },
    "GET /api/bookmarks": { line: 16, file: "main.clear", auth: false },
    "DELETE /api/bookmarks/:id": { line: 19, file: "main.clear", auth: false },
  }
} : null;

// clear:3
// Data shape: Bookmarks
const BookmarksSchema = {
  url: { type: "text", required: true },
  title: { type: "text", required: true },
  tags: { type: "text" }
};
db.createTable('bookmarks', BookmarksSchema);
// clear:8
// clear:8 — POST /api/bookmarks
app.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    // clear:9
    const _vErrs = _validate(req.body, [{"field":"url","type":"text","required":true,"matches":"url"}, {"field":"title","type":"text","required":true}, {"field":"tags","type":"text"}]);
    if (_vErrs) return res.status(400).json({ errors: _vErrs });
    // clear:13
    const saved = await _clearTry(() => db.insert('bookmarks', _pick(bookmark, BookmarksSchema)), { op: 'insert', table: 'bookmarks', line: 13, file: 'main.clear', source: "" }); // clear:13
    // clear:14
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/bookmarks] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/bookmarks', line: 8, file: 'main.clear' }, err._clearCtx || {});
...
// clear:16
// clear:16 — GET /api/bookmarks
app.get('/api/bookmarks', async (req, res) => {
  try {
    // clear:17
    const _resp_17 = (await db.findAll('bookmarks', {}, { limit: 50 })).map(_revive);
    // clear:17
    return res.json(_resp_17);
  } catch (err) {
    console.error('[GET /api/bookmarks] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/bookmarks', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:19
// clear:19 — DELETE /api/bookmarks/:id
app.delete('/api/bookmarks/:id', async (req, res) => {
  try {
    // clear:20
    await _clearTry(() => db.remove('bookmarks', { id: req.params.id }), { op: 'remove', table: 'bookmarks', line: 20, file: 'main.clear', source: "" }); // clear:20
    // clear:21
    return res.status(204).json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/bookmarks/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/bookmarks/:id', line: 19, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
