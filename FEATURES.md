...
| **Custom domain attach (CC-5 cycle 1)** | `POST/GET/DELETE /api/apps/:appSlug/domains` + per-app dashboard panel | Customer types `deals.acme.com`, sees a "Verifying DNS" pill, gets a copy-pasteable CNAME hint. Per-domain status: pending / verified / failed / removed. Cross-tenant attach returns 404 (locked in by tests). 23 routes integration tests + new `app_domains` table migration. |
| **DNS verification poller (CC-5b)** | `playground/cloud-domains/index.js` — `pollOnce`, `resolveDomainCname`, `startDomainPoller`, `bootstrapDomainPoller` | Studio bootstrap starts a 1-minute tick (gated on `DATABASE_URL` — skips cleanly in in-memory mode). Each tick reads pending `app_domains` rows, resolves CNAME via `node:dns/promises`, classifies records via the existing `verifyCname` helper, updates status to `verified` (records match) / `failed` (records exist but wrong target, with `last_error` populated) / leaves `pending` (records empty / DNS not propagated yet). Per-row try/catch keeps one bad lookup from killing the cycle. Verified rows are skipped on the next pass — `verified_at` doesn't drift. 33 new tests across 4 TDD cycles (pollOnce, resolveDomainCname, startDomainPoller, bootstrapDomainPoller). Cycle 2 of CC-5; cycle 3 (CC-5c Fly cert provisioner) is the remaining follow-up. |
