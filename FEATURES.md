...
- **API-call audit trail (2026-05-03 night, extended late night).** When `allow signup and login` is declared, every state-changing request (POST/PUT/PATCH/DELETE) is captured to a durable `audit_log` SQL table with the caller's user_id + email + tenant_id, the route, the method, the response status, an ISO timestamp, AND a sanitized `body_summary` of the request payload (sensitive fields like `password` / `token` / `secret` / `api_key` / `jwt` / `auth` are redacted to `[redacted]`; full body capped at 1KB). `GET /audit` (authenticated) returns the log; under shared tenant scope it's filtered to the caller's tenant. Read-only requests (GET/HEAD/OPTIONS) are skipped. The compliance buyer's three questions — who, when, what — all answerable in one row.
