...
| Requirements/Ralph loop | Studio `/api/chat` + supervisor | Complex apps draft end-to-end requirements before source mutation. The server rejects vague/compound approved requirements, then Ralph audits implementation evidence after build and blocks false done when requirements are missing. Compiler failures are sent back to Meph as hidden Snap feedback instead of visible chat junk or `HINT_APPLIED` repair tags. Start at `FAQ.md` -> "Where does Ralph live"; implementation lives in `studio/ralph-layer.js`, `studio/supervisor/requirements-facts.js`, and `studio/supervisor/requirements-audit.js`. |
...
| Compiler-feedback retry checks | `studio/snap-layer.test.js` + `studio/meph-tools.test.js` | Verifies compiler errors are converted into private Meph feedback, while the old exact-error `HINT_APPLIED` repair-tag injector stays out of normal compile retries. |
