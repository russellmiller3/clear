...
| Provable correctness — agent-bounded rules | `bounds_agent_output: true` flag on PROVED verdicts | When a `rule:` fires AFTER an agent invocation (`call 'X' with Y` or `ask claude '…'`) in the same body, the prover marks the verdict with `bounds_agent_output: true`. The translator surfaces the regulated-tier pitch sentence: *"PROVED for every possible deal — agent output cannot bypass this rule (the rule fires after the agent returns)."* The agent is non-deterministic but cannot bypass the structural gate; every agent output flows through the rule's runtime check before the next line runs. Detection lives in `lib/prover/index.js::collectRuleDefs` (sibling-statement walk), translator in `lib/proof-business-language.mjs::translateRules`, 3 tests in `lib/prover/index.test.js`. 2026-05-07. |
