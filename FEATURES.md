...
- **API-call audit trail (2026-05-03 night).** When `allow signup and login` is declared, every state-changing request (POST/PUT/PATCH/DELETE) is captured to a durable `audit_log` SQL table with the caller's user_id + email + tenant_id, the route, the method, the response status, and an ISO timestamp. `GET /audit` (authenticated) returns the log; under shared tenant scope it's filtered to the caller's tenant so cross-tenant leakage is prevented at the audit layer too. Read-only requests (GET/HEAD/OPTIONS) are skipped — the log captures CHANGES, not traffic. Marcus's compliance buyer's "show me every state change in the last hour" used to be answered by "grep server logs" — now it's `GET /audit`.
