...
| Curated pattern DB for Meph | Studio seeds `factor-db.sqlite` with the 13 canonical Clear apps from `CLAUDE.md` (8 core + 5 Marcus). Meph can search it through `browse_templates` with `action: "search"`, and failed compiles get the closest trusted patterns in the hint text. Meph does not raw-write this table; future promotion should go through compile/test validation before insertion. See `studio/supervisor/pattern-library.js` and `studio/supervisor/factor-db.js`. |
