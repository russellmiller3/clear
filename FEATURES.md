...
| Provable correctness — `clear test` auto-prove | Default behavior of `clear test <file>` | Every test session also runs the prover. Appends one line at the bottom: `Proofs: N proved, M failed, K unknown, U unverifiable (run \`clear prove <file>\` for details)`. Opt out with `--no-prove`. Under `--json`, the bundle ships in the same envelope as test results. PC-8, 2026-05-02. |
| Provable correctness — business-language translator | `node scripts/proof-business-language.mjs <file.clear>` | Translates the prover's verdicts into sentences a CRO or compliance buyer can read. PROVED → "We proved: <test_name>, for every possible <vars>." UNVERIFIABLE → "<test_name> talks to the world (database / email / AI / time)." FAILED → "Counterexample found." Headline summarises the bundle in plain English. `--json` ships a machine-readable payload Studio (or any caller) can render inline. 27 tests passing. 2026-05-02. |
