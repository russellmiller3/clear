...
| Requirements/Ralph loop | Studio `/api/chat` + supervisor | Complex apps draft end-to-end requirements before source mutation. The server rejects vague/compound approved requirements, then Ralph audits implementation evidence after build and blocks false done when requirements are missing. Start at `FAQ.md` -> "Where does Ralph live"; implementation lives in `studio/ralph-layer.js`, `studio/supervisor/requirements-facts.js`, and `studio/supervisor/requirements-audit.js`. |
