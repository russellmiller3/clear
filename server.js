...
//
// TABLES:
//   Contacts: name*, email*, phone, company (line 5)
//
// ENDPOINTS:
//   GET /api/contacts (line 12)
//   POST /api/contacts (line 17)
//   DELETE /api/contacts/:id [auth] (line 22)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    contacts: { line: 5, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, phone: { type: "text" }, company: { type: "text" } } },
  },
  endpoints: {
    "GET /api/contacts": { line: 12, file: "main.clear", auth: false },
    "POST /api/contacts": { line: 17, file: "main.clear", auth: false },
    "DELETE /api/contacts/:id": { line: 22, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/contacts', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 12, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:17
// clear:17 — POST /api/contacts
app.post('/api/contacts', async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object') return res.status(400).json({ error: 'Request body is required (send JSON with Content-Type: application/json)' });
    const data = req.body;
    const saved = await _clearTry(() => db.insert('contacts', _pick(data, ContactsSchema)), { op: 'insert', table: 'contacts', line: 18, file: 'main.clear', source: "" }); // clear:18
    return res.json(saved);
  } catch (err) {
    console.error('[POST /api/contacts] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/contacts', line: 17, file: 'main.clear' }, err._clearCtx || {});
...
// clear:22
// clear:22 — DELETE /api/contacts/:id
app.delete('/api/contacts/:id', async (req, res) => {
  try {
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    await _clearTry(() => db.remove('contacts'), { op: 'remove', table: 'contacts', line: 24, file: 'main.clear', source: "" }); // clear:24
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/contacts/:id', line: 22, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
