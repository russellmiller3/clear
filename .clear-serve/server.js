...
//
// TABLES:
//   Contacts: name*, email*, phone, company (line 4)
//
// ENDPOINTS:
//   GET /api/contacts (line 10)
//   POST /api/contacts (line 14)
//   DELETE /api/contacts/:id [auth] (line 18)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    contacts: { line: 4, file: "main.clear", fields: { name: { required: true, type: "text" }, email: { required: true, type: "text" }, phone: { type: "text" }, company: { type: "text" } } },
  },
  endpoints: {
    "GET /api/contacts": { line: 10, file: "main.clear", auth: false },
    "POST /api/contacts": { line: 14, file: "main.clear", auth: false },
    "DELETE /api/contacts/:id": { line: 18, file: "main.clear", auth: true },
  }
} : null;
...
app.get('/api/contacts', async (req, res) => {
  try {
    ...
    const _ctx = Object.assign({ endpoint: 'GET /api/contacts', line: 10, file: 'main.clear' }, err._clearCtx || {});
...
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    const data = req.body;
    const saved = await _clearTry(() => db.insert('contacts', _pick(data, ContactsSchema)), { op: 'insert', table: 'contacts', line: 15, file: 'main.clear', source: "" }); // clear:15
...
// clear:18
// clear:18 — DELETE /api/contacts/:id
app.delete('/api/contacts/:id', async (req, res) => {
  try {
    if (!req.user) { return res.status(401).json({ error: "Authentication required" }); }
    await _clearTry(() => db.remove('contacts'), { op: 'remove', table: 'contacts', line: 20, file: 'main.clear', source: "" }); // clear:20
    return res.json({ message: "deleted" });
  } catch (err) {
    console.error('[DELETE /api/contacts/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/contacts/:id', line: 18, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
