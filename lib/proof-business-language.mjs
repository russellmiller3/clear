...
export function translateRules(bundle) {
  ...
  for (const r of rules) {
    ...
    if (verdict === 'proved') {
      ...
      // The headline reads the same as before. The agent-bounding claim —
      // "even an AI-suggested value cannot bypass this rule" — is appended
      // ONLY when the prover marked the rule as bounding agent output.
      // That flag fires when an agent invocation (named-agent call or
      // `ask claude` directly) ran earlier in the same body, so every
      // agent output flows through this rule's gate before the next line
      // runs. This is the regulated-tier pitch to the CRO: the AI is
      // non-deterministic, but the rule ALWAYS fires.
      const baseSentence = `PROVED for every possible ${entityWord}`;
      const sentence = r.bounds_agent_output === true
        ? `${baseSentence} — agent output cannot bypass this rule (the rule fires after the agent returns)`
        : baseSentence;
      lines.push({
        verdict: 'proved',
        name,
...
export function formatProveOutput(bundle) {
  ...
  // Empty file — nothing was proved AND nothing was tested. Mirror
  // formatBundle's "Summary: No tests in the program" voice in plain English.
  if (ruleResult.counts.total === 0 && testResult.counts.total === 0) {
