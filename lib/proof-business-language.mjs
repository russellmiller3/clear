...
// when at least one rule exists; empty when no rules.
export function translateRules(bundle) {
  ...
  for (const r of rules) {
    ...
    if (verdict === 'proved') {
      counts.proved += 1;
      lines.push({ verdict: 'proved', name, sentence: 'PROVED for every possible deal' });
  ...
  let headline = '';
  if (counts.total > 0) {
    headline = `We proved ${counts.proved} of ${counts.total} named rules in this app, for every possible deal.`;
  }
