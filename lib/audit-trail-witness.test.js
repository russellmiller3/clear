...
async function startCompiledServer(serverJS, port) {
  const tempPath = join(REPO_ROOT, `_audit-witness-${Date.now()}-${process.pid}-${Math.random().toString(36).slice(2, 8)}.cjs`);
  writeFileSync(tempPath, serverJS, 'utf8');
  const proc = spawn(process.execPath, [tempPath], {
    cwd: REPO_ROOT,
    env: { ...process.env, PORT: String(port), NODE_ENV: 'test', JWT_SECRET: 'audit-witness-secret' },
...
describe('audit trail — design sanity', () => {
  it('compiled JS contains the audit log and middleware (no scope)', () => {
    ...
    expect(js).toContain('const _audit_log = []');
  ...
  it("tenant-scope variant filters /audit response by tenant", () => {
    ...
    expect(js).toContain('e.tenant_id === req.user.tenant_id');
  });
});
