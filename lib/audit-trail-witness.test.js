...
describe('audit trail — design sanity', () => {
  ...
  // CSV export — same audit data, served as text/csv for compliance tools
  // (SOC 2 evidence collectors etc.) that ingest CSV more naturally than
  // JSON. Same auth + tenant scoping as /audit. Quote-escapes every value
  // to handle body_summary text containing commas, quotes, or newlines.
  it("emits a /audit.csv route alongside /audit", () => {
    const js = (compileProgram(SRC_NO_TENANT).serverJS) || compileProgram(SRC_NO_TENANT).javascript;
    expect(js).toContain("app.get('/audit.csv'");
    expect(js).toContain("text/csv; charset=utf-8");
    expect(js).toContain('attachment; filename="audit.csv"');
  });
  it("CSV escape function quotes values containing commas, quotes, or newlines", () => {
    const js = (compileProgram(SRC_NO_TENANT).serverJS) || compileProgram(SRC_NO_TENANT).javascript;
    expect(js).toContain('function _csvEscape');
    // Doubles-up quotes inside quoted strings (RFC 4180)
    expect(js).toMatch(/replace\(\/"\/g,\s*'""'\)/);
  });
  it("/audit.csv emits header row with all audit columns", () => {
    const js = (compileProgram(SRC_NO_TENANT).serverJS) || compileProgram(SRC_NO_TENANT).javascript;
    expect(js).toContain("['id', 'ts', 'user_id', 'user_email', 'tenant_id', 'method', 'path', 'status', 'body_summary']");
  });
  it("tenant-scope variant filters /audit.csv response by tenant", () => {
    const js = (compileProgram(SRC_TENANT).serverJS) || compileProgram(SRC_TENANT).javascript;
    // Both /audit and /audit.csv should filter by tenant under shared scope
    const occurrences = (js.match(/db\.findAll\('audit_log', \{ tenant_id: req\.user\.tenant_id \}\)/g) || []).length;
...
await describeAsync('audit trail — end-to-end HTTP (no tenant scope)', async () => {
  await itAsync('every state change Alice makes lands in /audit; her GET /audit does not', async () => {
    ...
    try {
      ...
      // Body summary captures WHAT was sent, with password redacted.
      expect(typeof signupRow.body_summary).toBe('string');
      if (!/alice@a\.test/.test(signupRow.body_summary)) {
        throw new Error('signup body_summary missing email; got: ' + signupRow.body_summary);
      }
      if (/secret-pw-1234/.test(signupRow.body_summary)) {
        throw new Error('signup body_summary leaked password! got: ' + signupRow.body_summary);
      }
      if (!/\[redacted\]/.test(signupRow.body_summary)) {
        throw new Error('signup body_summary missing redaction marker; got: ' + signupRow.body_summary);
      }
      ...
      for (const r of dealRows) {
        ...
        // Body summary captures non-sensitive fields verbatim.
        expect(typeof r.body_summary).toBe('string');
        if (!/status/.test(r.body_summary)) {
          throw new Error('deal body_summary missing status field; got: ' + r.body_summary);
        }
      }
