...
const TOOL_IMPLS = {
	...
	propose_remove_field: proposeRemoveField,
};
...
export const TOOL_DEFINITIONS = [
	...
	{
		name: 'propose_remove_field',
		description:
			'Propose a PERMANENT, destructive change: drop a field from a table. ' +
			'The column is removed from the database and every existing value in ' +
			'that column is destroyed — this is real data loss, not a hide. ' +
			'Rollback restores the field definition but cannot recover the deleted ' +
			'values. The owner is required to type a confirmation phrase (e.g. ' +
			"'DELETE field notes') before the ship endpoint will accept the change. " +
			"Prefer `propose_hide_field` unless the owner has explicitly used the " +
			"words 'permanently' / 'forever' / 'wipe' — hide preserves data and is " +
			'reversible with one click, while remove is irreversible and requires ' +
			'typed confirmation. Only choose this tool when the owner has made the ' +
			'destructive intent unambiguous.',
		input_schema: {
			type: 'object',
			properties: {
				table: {
				...
				field: {
					type: 'string',
					description: 'The exact field name to permanently remove, e.g. "notes".',
				},
			},
			required: ['table', 'field'],
		},
