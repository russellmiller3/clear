...
`;
	...
	try {
		...
		try {
			const fn = mod.agent_productbot || mod.default?.agent_productbot;
			// Agent functions are module-scope but NOT exported — reload the module
			// and use `eval` against its source to expose the fn. Simpler: re-read
			// the emitted bundle as text, tack on a named export for the agent
			// so we can `import` it. We do this because our emit is stable (the
			// fn lives at top level) but not exported.
			const srcText = result.workerBundle['src/index.js'] + '\nexport { agent_productbot };\n';
			const exposeEntry = join(dir, 'expose.mjs');
			writeFileSync(exposeEntry, srcText, 'utf8');
			const exposed = await import(pathToFileURL(exposeEntry).href);
...
`;
	...
	try {
		const srcText = src + '\nexport { agent_faqbot };\n';
		const entry = join(dir, 'expose.mjs');
		writeFileSync(entry, srcText, 'utf8');
		const mod = await import(pathToFileURL(entry).href);
