...
/**
 * Mark a test as pending/skipped — counts separately, never fails the suite.
 * Use for specs of a feature that isn't built yet (so the intent stays visible
 * in the suite instead of being deleted). Pass a reason as the 3rd argument.
 *   it.skip('emits the Lenat SVG map', () => {...}, 'unbuilt: data-graph-stage emit')
 */
test.skip = function (name, _fn, reason) {
	skipped++;
	console.log(`⏭️  ${name}${reason ? ' — ' + reason : ''} (skipped)`);
};
