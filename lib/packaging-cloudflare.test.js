...
// Phase 6.10: the Workers bundle now splits the Workers-safe helpers and
// agent functions into src/agents.js, keeping src/index.js focused on
// routing. Tests that assert a helper's SHAPE (signature, body text) look
// at the combined bundle so they don't care which file it landed in.
// Drift-guard tests keep checking src/index.js directly — the routing file
// is still the canonical "no Node-isms" surface.
// Codex chunk #2: skip node --check in sandboxes that block child processes (EPERM).
function isNodeSpawnBlocked(e) {
	const text = String(e?.message || e?.stderr?.toString?.() || e || '');
	return e?.code === 'EPERM' || text.includes('EPERM');
}

function skipNodeCheckWhenBlocked(result) {
...
`;
	...
	it('emitted src/index.js passes node --check as ESM', () => {
		...
		if (skipNodeCheckWhenBlocked(r)) return;
...
describe('Live app editing identity survives Cloudflare packaging', () => {
...
`;
	...
	it('injectClearCloudMeta adds tenant/app identity inside the document head', () => {
		const html = '<!doctype html><html><head><title>Owner</title></head><body></body></html>';
		const out = pkgCf.injectClearCloudMeta(html, {
			tenantSlug: 'clear-acme',
			appSlug: 'owner-dashboard',
		});
		expect(out).toContain('<head>');
		expect(out).toContain('<meta name="clear-cloud"');
		expect(out).toContain('"tenantSlug":"clear-acme"');
		expect(out).toContain('"appSlug":"owner-dashboard"');
	});

	it('prepareCloudflareWorkerBundle injects the widget route only when cloud identity is present', () => {
		const result = compileProgram(OWNER_APP, { target: 'cloudflare' });
		const withoutIdentity = pkgCf.prepareCloudflareWorkerBundle(result, { appName: 'owner-dashboard' });
		const withIdentity = pkgCf.prepareCloudflareWorkerBundle(result, {
			appName: 'owner-dashboard',
			tenantSlug: 'clear-acme',
			appSlug: 'owner-dashboard',
		});

		expect(withoutIdentity['src/index.js'].includes("pathname === '/__meph__/widget.js'")).toBe(false);
		expect(withIdentity['src/index.js']).toContain("pathname === '/__meph__/widget.js'");
		expect(withIdentity['src/index.js']).toContain('const __CLEAR_MEPH_WIDGET_JS__ = ');
	});

	it('packageBundle writes clear-cloud meta and widget route into the deployed bundle', () => {
		const result = compileProgram(OWNER_APP, { target: 'cloudflare' });
		const outDir = mkdtempSync(join(tmpdir(), 'clear-cf-live-edit-'));
		try {
			const res = packageBundle(result, outDir, {
				target: 'cloudflare',
				appName: 'owner-dashboard',
				tenantSlug: 'clear-acme',
				appSlug: 'owner-dashboard',
			...
			const indexJs = readFileSync(resolve(outDir, 'src', 'index.js'), 'utf8');
...
const LIVE_EDIT_BRIDGE_APP = `build for web and javascript backend
...
function liveEditBridgeBundle() {
	const result = compileProgram(LIVE_EDIT_BRIDGE_APP, { target: 'cloudflare' });
	return pkgCf.prepareCloudflareWorkerBundle(result, {
		appName: 'owner-dashboard',
		tenantSlug: 'clear-acme',
		appSlug: 'owner-dashboard',
	});
...
async function importGeneratedWorker(bundle, prefix) {
	...
	return { dir, worker: mod.default };
}

console.log('\n📦 Live app editing Cloudflare API bridge\n');

await testAsync('cf-live-edit-bridge: configured error is non-404 for every widget API path', async () => {
	const { dir, worker } = await importGeneratedWorker(liveEditBridgeBundle(), 'clear-cf-live-bridge-missing-');
	try {
		const cases = [
			['POST', '/__meph__/api/propose'],
			['POST', '/__meph__/api/ship'],
			['POST', '/__meph__/api/cloud-rollback'],
			['GET', '/__meph__/api/deploy-history?appSlug=owner-dashboard'],
		];
		for (const [method, path] of cases) {
			...
			if (method !== 'GET') init.body = JSON.stringify({ ok: true });
			const response = await worker.fetch(new Request('https://customer.test' + path, init), {}, {});
			const body = await response.json();
			expect(response.status).toBe(503);
			expect(body.error).toContain('CLEAR_LIVE_EDIT_BASE_URL');
		}
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
...
await testAsync('cf-live-edit-bridge: POST proxy preserves auth header and JSON body', async () => {
	const { dir, worker } = await importGeneratedWorker(liveEditBridgeBundle(), 'clear-cf-live-bridge-post-');
	...
	try {
		...
		const body = JSON.stringify({ prompt: 'add region field' });
		const response = await worker.fetch(
			new Request('https://customer.test/__meph__/api/propose?draft=1', {
				method: 'POST',
				headers: {
			...
			{ CLEAR_LIVE_EDIT_BASE_URL: 'https://studio.clear.test/live/' },
			{},
		);
		const responseBody = await response.json();

		expect(response.status).toBe(202);
		expect(responseBody.proxied).toBe(true);
		expect(calls).toHaveLength(1);
		expect(calls[0].url).toBe('https://studio.clear.test/live/__meph__/api/propose?draft=1');
...
await testAsync('cf-live-edit-bridge: GET deploy-history proxies query and skips body', async () => {
	const { dir, worker } = await importGeneratedWorker(liveEditBridgeBundle(), 'clear-cf-live-bridge-get-');
	const originalFetch = globalThis.fetch;
	const calls = [];
	try {
		globalThis.fetch = async (url, init = {}) => {
			...
			return new Response(JSON.stringify({ versions: [{ id: 'v2' }, { id: 'v1' }] }), {
				status: 200,
				headers: { 'Content-Type': 'application/json' },
			});
		};
		...
		const response = await worker.fetch(
			new Request('https://customer.test/__meph__/api/deploy-history?appSlug=owner-dashboard', {
				method: 'GET',
				headers: { Authorization: 'Bearer owner-token' },
			}),
			{ CLEAR_LIVE_EDIT_BASE_URL: 'https://studio.clear.test' },
			{},
		);
		const responseBody = await response.json();

		expect(response.status).toBe(200);
		expect(responseBody.versions).toHaveLength(2);
		expect(calls).toHaveLength(1);
		expect(calls[0].url).toBe('https://studio.clear.test/__meph__/api/deploy-history?appSlug=owner-dashboard');
	...
	await testUtils.testAsync('cf-1.7: packaged hello-world serves 200 under wrangler dev --local', async () => {
		...
		try {
			...
			try {
				...
				// Phase 2: /api/hello compiles `send back 'hi'` → { message: 'hi' }
				// (Phase 1 stub emitted { ok: true, endpoint }; the real compiled
				// Response now wraps string literals in { message: ... } per
				// compileRespond's Workers branch.)
				if (body.message !== 'hi' && body.endpoint !== '/api/hello') {
...
// ─────────────────────────────────────────────────────────────────────────
// Phase 3 cycle 3.4 — _askAIWithTools_workers marshals tool_use without fs
// ─────────────────────────────────────────────────────────────────────────
//
// Agents with `has tools: send_email, fetch_weather` give the model a set of
// JavaScript functions it can reach for mid-turn. The loop: model emits
// tool_use blocks, helper invokes the local JS function, wraps result as
// tool_result, feeds back. Pure JS — no fs, no process — just fetch +
// closure calls. Bounded by maxTurns to prevent runaway loops.

describe('Phase 3 cycle 3.4 — _askAIWithTools_workers tool-marshaling is pure JS', () => {
	const AGENT_WITH_TOOLS = `build for javascript backend

function send_email with 'to' and 'body':
  send back true

agent 'notifier' receives request:
  has tools: send_email
  set reply to ask claude 'notify the user'
  send back reply

when user requests data from /api/notify:
  set reply to ask 'notifier' with 'ok'
  send back reply
`;

	it('emit contains _askAIWithTools_workers when the agent has tools', () => {
		const result = compileProgram(AGENT_WITH_TOOLS, { target: 'cloudflare' });
		const src = combinedBundleText(result.workerBundle);
		expect(src).toContain('_askAIWithTools_workers');
	});

	it('tool-marshal helper takes env as first param and is async', () => {
		const result = compileProgram(AGENT_WITH_TOOLS, { target: 'cloudflare' });
		const src = combinedBundleText(result.workerBundle);
		expect(src).toContain('async function _askAIWithTools_workers(env,');
	});

	it('tool-marshal helper bounds iteration (maxTurns)', () => {
		const result = compileProgram(AGENT_WITH_TOOLS, { target: 'cloudflare' });
		const src = combinedBundleText(result.workerBundle);
		// A loop cap is the difference between a bug and a cost disaster.
		// 10 turns mirrors the Node variant.
		expect(src).toContain('maxTurns');
	});

	it('tool-marshal helper returns tool_result payload on tool call', () => {
		const result = compileProgram(AGENT_WITH_TOOLS, { target: 'cloudflare' });
		const src = combinedBundleText(result.workerBundle);
		expect(src).toContain('tool_use');
		expect(src).toContain('tool_result');
	});

	it('whole Workers bundle stays free of Node-isms even with tools', () => {
		const result = compileProgram(AGENT_WITH_TOOLS, { target: 'cloudflare' });
		const src = combinedBundleText(result.workerBundle);
		// The Phase 3 hard rule — no leaks from the Node path, even when the
		// app has the more complex tool-marshalling helper.
		const forbidden = ['require(', 'child_process', 'fs.', '/tmp', 'execSync', 'spawn'];
		for (const f of forbidden) {
			if (src.includes(f)) {
				throw new Error(`Workers bundle contains forbidden Node-ism "${f}" in tool-use path`);
			}
			expect(src.includes(f)).toBe(false);
		}
	});
...
// ─────────────────────────────────────────────────────────────────────────
// Phase 3 cycle 3.7 — allow signup and login emits webcrypto (no bcryptjs)
// ─────────────────────────────────────────────────────────────────────────
//
// `allow signup and login` is the one-line auth shortcut in Clear. On Node
// it compiles to Express routes that call bcrypt.hash / bcrypt.compare. On
// Workers it compiles to the same endpoints but backed by PBKDF2 via the
// Web Crypto API (runtime/auth-webcrypto.mjs). Zero bcryptjs, zero
// jsonwebtoken, pure crypto.subtle.

describe('Phase 3 cycle 3.7 — allow signup and login emits webcrypto', () => {
	const AUTH_APP = `build for javascript backend
...
`;
	...
	it('Workers bundle inlines hashPassword from auth-webcrypto', () => {
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		expect(src).toContain('function hashPassword');
	});

	it('Workers bundle inlines verifyPassword from auth-webcrypto', () => {
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		expect(src).toContain('function verifyPassword');
	});

	it('Workers bundle uses crypto.subtle for PBKDF2', () => {
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		expect(src).toContain('crypto.subtle.deriveBits');
		expect(src).toContain('PBKDF2');
	});

	it('Workers bundle contains ZERO bcryptjs CODE references', () => {
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		// Documentation mentions of "bcryptjs" (to explain what the Workers
		// variant replaces) are fine — the auth-webcrypto JSDoc block
		// references it as a context. We care about CODE references: any
		// import / require / method-call pattern that would actually hit
		// the Workers loader.
		const forbidden = ['bcrypt.hash', 'bcrypt.compare', "require('bcryptjs')", 'require("bcryptjs")', "from 'bcryptjs'", 'from "bcryptjs"'];
		for (const f of forbidden) {
			if (src.includes(f)) {
				throw new Error(`Workers bundle contains forbidden bcryptjs code reference "${f}"`);
			}
			expect(src.includes(f)).toBe(false);
		}
	});

	it('Workers bundle exports the webcrypto iteration constant', () => {
		// A runtime-present constant makes a future iterations bump trivially
		// detectable by `grep 600000` across the bundle.
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		expect(src).toContain('PBKDF2_ITERATIONS');
		expect(src).toContain('600000');
	});

	it('Node target still emits bcryptjs-based auth (unchanged)', () => {
		// Invariant: the Node target's auth emit is untouched by Phase 3.
		const result = compileProgram(AUTH_APP);
		const js = result.serverJS || result.javascript || '';
		expect(js).toContain('bcryptjs');
		expect(js).toContain('bcrypt.hash');
		expect(js).toContain('bcrypt.compare');
	});

	it('auth-app Workers bundle still has zero Node-isms (drift cross-check)', () => {
		// The auth helpers landing in src/index.js must not sneak a
		// require() or fs. call in. This is the Phase 3 hard rule extended
		// over the auth surface.
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		const forbidden = ['require(', 'child_process', 'fs.', '/tmp', 'execSync', 'spawn'];
		for (const f of forbidden) {
			if (src.includes(f)) {
				throw new Error(`Workers auth bundle contains forbidden Node-ism "${f}"`);
			}
			expect(src.includes(f)).toBe(false);
		}
	});

	it('auth-app Workers bundle parses clean under node --check (ESM)', () => {
		const result = compileProgram(AUTH_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		const dir = mkdtempSync(join(tmpdir(), 'clear-cf37-check-'));
		const file = join(dir, 'index.mjs');
		writeFileSync(file, src, 'utf8');
		try {
			execFileSync(process.execPath, ['--check', file], { stdio: 'pipe' });
...
// ─────────────────────────────────────────────────────────────────────────
// Phase 3 cycle 3.8 — end-to-end: auth + agent app compiles + helpers work
// ─────────────────────────────────────────────────────────────────────────
//
// The acceptance moment: compile a template that has BOTH `allow signup and
// login` AND `ask claude`, then actually exercise the hashing helpers from
// the emitted bundle using Node's native Web Crypto (same API Workers gives
// a deployed Worker). Signup → hash, login → verify, agent-call shape — all
// prove out on a single bundle.
//
// Per Phase 3 briefing: Workers runtime is simulated via Node 20+'s native
// Web Crypto. No miniflare needed — cycle 3.0's drift-guard already
// proved the bundle is Workers-safe at upload time.

// Cycle 3.8's async tests must run at module-level with explicit await,
// NOT inside a describe() block — describe()'s fn() is synchronous and
// doesn't await the testAsync promise chain (see learnings.md Phase 1:
// "testUtils.it() doesn't await async fn bodies — silent green"). Putting
// them at module scope with `await` around each testAsync call is the
// idiom that reports correctly. Same pattern as cycle 1.7's wrangler smoke.

console.log('\n📦 Phase 3 cycle 3.8 — end-to-end: auth + agent compile + helpers work\n');

const CYCLE_3_8_AUTH_PLUS_AGENT = `build for javascript backend
...
when user requests data from /api/notify:
  set reply to ask 'notifier' with 'hi'
  send back reply
`;

await testAsync('cf-3.8: compiled bundle contains every Phase 3 helper', async () => {
	const result = compileProgram(CYCLE_3_8_AUTH_PLUS_AGENT, { target: 'cloudflare' });
	const src = result.workerBundle['src/index.js'];
	// Auth lineage
	expect(src).toContain('function hashPassword');
	expect(src).toContain('function verifyPassword');
	// AI lineage
	expect(src).toContain('_askAI_workers');
	// Zero Node-isms across the combined bundle
	const forbidden = ['require(', 'child_process', 'fs.', '/tmp', 'execSync', 'spawn', 'bcrypt.hash'];
	for (const f of forbidden) {
		if (src.includes(f)) {
			throw new Error(`Phase 3 E2E bundle contains forbidden Node-ism "${f}"`);
		}
	}
});
...
await testAsync('cf-3.8: combined bundle parses clean under node --check', async () => {
	const result = compileProgram(CYCLE_3_8_AUTH_PLUS_AGENT, { target: 'cloudflare' });
	const src = result.workerBundle['src/index.js'];
	const dir = mkdtempSync(join(tmpdir(), 'clear-cf38-check-'));
	const file = join(dir, 'index.mjs');
	writeFileSync(file, src, 'utf8');
	try {
		execFileSync(process.execPath, ['--check', file], { stdio: 'pipe' });
...
await testAsync('cf-3.8: bundle is importable as a Node ESM module (dynamic import)', async () => {
	// Write the bundle to a temp dir and dynamic-import it so the full module
	// body executes. Strongest smoke — a dangling syntax error or hoist bug
	// would throw at import time. Phase 6.10: the emit now splits into
	// src/index.js + (when agents present) src/agents.js + src/workflows/*.js,
	// so we walk the full workerBundle map and write every file — otherwise
	// src/index.js's `import { _askAI_workers } from './agents.js'` can't
	// resolve.
	const result = compileProgram(CYCLE_3_8_AUTH_PLUS_AGENT, { target: 'cloudflare' });
	const dir = mkdtempSync(join(tmpdir(), 'clear-cf38-import-'));
	try {
		const { mkdirSync } = await import('fs');
		// Scope-assert: drop a package.json with type:module so Node treats
		// every .js file under this dir as ESM. Without it Node walks up to
		// whatever package.json it finds (usually CJS) and rejects `import`.
		writeFileSync(join(dir, 'package.json'), JSON.stringify({ type: 'module' }), 'utf8');
		...
		// Import the entry. Workers bundle names src/index.js — mirror that so
		// the relative imports to ./agents.js and ../agents.js resolve.
		const entry = join(dir, 'src', 'index.js');
		const mod = await import('file://' + entry.replace(/\\/g, '/'));
...
await testAsync('cf-3.8: full signup → login roundtrip works via auth-webcrypto', async () => {
	// Drive the real auth-webcrypto module (same source that gets inlined
	// into the Workers bundle) so we prove Marcus's signup → login loop
	// actually works on the Web Crypto primitives Workers provides.
	const { hashPassword, verifyPassword } = await import('../runtime/auth-webcrypto.mjs');

	const plaintext = 'marcus-picks-a-password-2026';
	const stored = await hashPassword(plaintext);
	expect(stored.startsWith('v1:')).toBe(true);

	const goodLogin = await verifyPassword(plaintext, stored);
	expect(goodLogin).toBe(true);

	const badLogin = await verifyPassword('wrong-password', stored);
	expect(badLogin).toBe(false);

	const stored2 = await hashPassword(plaintext);
	expect(stored2).not.toEqual(stored); // random salt
	expect(await verifyPassword(plaintext, stored2)).toBe(true);
});
