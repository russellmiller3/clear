...
import { planRename as planRenameDefault } from './migration-planner.js';
...
export function createEditApi(app, deps) {
	const planRenameImpl = deps.planRename || planRenameDefault;
	...
	app.post('/__meph__/api/propose', requireOwner, async (req, res) => {
		...
		// LAE Phase C cycle 7 — rename detection.
		// A destructive "remove field" might be the first half of a rename:
		// the user added the new field already and now wants to drop the old
		// one. planRename compares the before/after ASTs and surfaces the
		// rename candidate with keep/discard options so the widget can ask
		// "copy the values?" instead of silently dropping data.
		if (proposal.classification.type === 'destructive') {
			try {
				const beforeProgram = parse(source);
				const afterProgram = parse(proposal.newSource);
				const renameResult = planRenameImpl({ beforeProgram, afterProgram });
				if (renameResult && renameResult.detected === 'rename') {
					responseBody.migrationPlan = renameResult;
				}
			} catch {
				// parse or planRename threw — skip migration plan, still ship
				// the proposal so the widget can proceed with the basic confirm UI.
			}
		}
	...
	app.post('/__meph__/api/ship', requireOwner, async (req, res) => {
		...
		const cloudContext = {
			...
			// LAE Phase C cycle 6 — destructive ships record their version row
			// with via:'widget-destructive' so the deploy ledger can distinguish
			// destructive from additive ships after the fact. The audit row
			// already captures the destruction story; this tag is what makes
			// destructive ships findable by version-history queries without
			// having to re-derive intent from the diff.
			via: req.body && req.body.classification && req.body.classification.type === 'destructive'
				? 'widget-destructive'
				: 'widget',
		};
