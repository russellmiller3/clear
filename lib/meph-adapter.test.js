...
`,
		...
		const s = req.system.toLowerCase();
		expect(s.includes('widget')).toBe(true);
		expect(s.includes('owner')).toBe(true);
		// Mentions safety taxonomy the tools enforce
		expect(s.includes('additive') || s.includes('reversible')).toBe(true);
		expect(req.messages[0].role).toBe('user');
		expect(req.messages[0].content.includes('add an email field to Users')).toBe(true);
	});

	it('includes all six propose_ tools (3 additive + 2 reversible + 1 destructive)', () => {
		const req = buildMephRequest({ prompt: 'x', source: '' });
		const names = req.tools.map((t) => t.name).sort();
		expect(names).toEqual([
			'propose_add_endpoint',
			'propose_add_field',
			'propose_add_page',
			'propose_hide_field',
			'propose_remove_field',
			'propose_rename_field',
		]);
	});
