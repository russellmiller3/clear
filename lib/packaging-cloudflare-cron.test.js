...
// ─────────────────────────────────────────────────────────────────────────
// Cycle 5.6 — end-to-end: compile a template with a scheduled agent +
// background block, simulate the Workers scheduled event, assert the body
// actually ran. No miniflare needed — Node 20+ has every API the handler
// touches (crypto.subtle, WHATWG Fetch, URL). The d1Mock we built in
// Phase 2 stands in for env.DB.
//
// Async tests run at module scope (NOT inside describe/it) because
// testUtils.it() is sync-only — async bodies inside it() resolve after
// it()'s try/catch has already recorded a pass. See Phase 3 learnings.
// ─────────────────────────────────────────────────────────────────────────

console.log('\n📦 Phase 5 cycle 5.6 — scheduled() handler fires end-to-end\n');

const CYCLE_5_6_HEARTBEAT = `build for javascript backend

create a Pings table:
  ts, required

background 'heartbeat':
  runs every 1 hour
  new_ping = { ts: 'now' }
  save new_ping as new Ping
`;

await testAsync('cf-5.6: heartbeat scheduled block inserts a row on each tick', async () => {
	const result = compileProgram(CYCLE_5_6_HEARTBEAT, { target: 'cloudflare' });
	...
	if (!migrations) throw new Error('heartbeat app emitted no migrations file');

	const dir = mkdtempSync(join(tmpdir(), 'clear-cf56-'));
	try {
		const entry = join(dir, 'index.mjs');
		writeFileSync(entry, result.workerBundle['src/index.js'], 'utf8');
		const mod = await import(pathToFileURL(entry).href);

		if (typeof mod.default?.scheduled !== 'function') {
			throw new Error('compiled bundle missing scheduled() handler');
		}

		const env = { DB: d1Mock() };
		env.DB.exec(migrations);

		// Simulate Cloudflare's scheduled event for the matching cron.
		const event = { cron: '0 * * * *', scheduledTime: Date.now() };
		await mod.default.scheduled(event, env, { waitUntil: () => {} });

		// Assert the row arrived — proves the handler branched on event.cron,
		// executed the body, and the env.DB.prepare/bind/run path worked.
		const { results } = await env.DB.prepare('SELECT * FROM pings').all();
		if (!Array.isArray(results) || results.length !== 1) {
			throw new Error('expected exactly 1 ping row after 1 tick, got ' + JSON.stringify(results));
		}
		if (results[0].ts !== 'now') {
			throw new Error('ping row ts mismatch: ' + JSON.stringify(results[0]));
		}

		// Fire it again — two invocations, two rows.
		await mod.default.scheduled(event, env, { waitUntil: () => {} });
		const { results: results2 } = await env.DB.prepare('SELECT * FROM pings').all();
		if (results2.length !== 2) {
			throw new Error('expected 2 ping rows after 2 ticks, got ' + results2.length);
		}
	} finally {
...
const CYCLE_5_6_TWO_CRONS = `build for javascript backend
...
background 'hourly':
  runs every 1 hour
  hourly_log = { kind: 'hourly' }
  save hourly_log as new Log

background 'per-ten-min':
  runs every 10 minutes
  ten_log = { kind: 'ten' }
  save ten_log as new Log
`;

await testAsync('cf-5.6: scheduled dispatches the right branch per event.cron', async () => {
	// Two background blocks on different cadences. Firing the 10-minute cron
	// must only run the "ten" body; firing the hourly cron must only run the
	// "hourly" body. Pinning the dispatcher so a future emit doesn't
	// accidentally fall through to every branch on every tick.
	const result = compileProgram(CYCLE_5_6_TWO_CRONS, { target: 'cloudflare' });
	if (result.errors.length !== 0) {
		throw new Error('compile errors: ' + JSON.stringify(result.errors));
	}

	const dir = mkdtempSync(join(tmpdir(), 'clear-cf56-two-'));
	try {
		const entry = join(dir, 'index.mjs');
		writeFileSync(entry, result.workerBundle['src/index.js'], 'utf8');
		const mod = await import(pathToFileURL(entry).href);
		...
		const env = { DB: d1Mock() };
		env.DB.exec(result.workerBundle['migrations/001-init.sql']);

		// Fire the 10-minute cron
		await mod.default.scheduled({ cron: '*/10 * * * *' }, env, { waitUntil: () => {} });
		const after10 = await env.DB.prepare('SELECT kind FROM logs').all();
		if (after10.results.length !== 1 || after10.results[0].kind !== 'ten') {
			throw new Error('10-min tick should only fire ten; got: ' + JSON.stringify(after10.results));
		}

		// Fire the hourly cron
		await mod.default.scheduled({ cron: '0 * * * *' }, env, { waitUntil: () => {} });
		const afterHour = await env.DB.prepare('SELECT kind FROM logs').all();
		if (afterHour.results.length !== 2) {
			throw new Error('hourly tick should add one row; got: ' + JSON.stringify(afterHour.results));
		}
		const kinds = afterHour.results.map((r) => r.kind).sort();
		if (kinds.join(',') !== 'hourly,ten') {
			throw new Error('kinds mismatch: ' + kinds.join(','));
		}

		// Fire an unknown cron (shouldn't happen in prod, but test the
		// fall-through: no branch matches → nothing fires → no error).
		await mod.default.scheduled({ cron: '* * * * *' }, env, { waitUntil: () => {} });
		const afterUnknown = await env.DB.prepare('SELECT kind FROM logs').all();
		if (afterUnknown.results.length !== 2) {
			throw new Error('unknown cron should not add rows; got: ' + afterUnknown.results.length);
		}
	} finally {
		try { rmSync(dir, { recursive: true, force: true }); } catch (_e) {}
	}
...
await testAsync('cf-5.6: scheduled body errors are caught, not re-thrown', async () => {
	// Each cron branch wraps its body in try/catch so one scheduled failure
	// doesn't take down the Worker. Verify: compile a block whose body would
	// throw (missing column), fire it, handler returns cleanly.
	const SRC = `build for javascript backend
...
background 'broken':
  runs every 1 hour
  broken_log = { no_such_column: 'x' }
  save broken_log as new Log
`;
	const result = compileProgram(SRC, { target: 'cloudflare' });
	if (result.errors.length !== 0) {
		throw new Error('compile errors: ' + JSON.stringify(result.errors));
	...
	const dir = mkdtempSync(join(tmpdir(), 'clear-cf56-err-'));
	try {
		const entry = join(dir, 'index.mjs');
		writeFileSync(entry, result.workerBundle['src/index.js'], 'utf8');
		const mod = await import(pathToFileURL(entry).href);
		...
		// Silence the console.error that the handler's try/catch logs so the
		// test output doesn't look like a failure when it's actually success.
		const origError = console.error;
		console.error = () => {};
		try {
			// This must NOT throw — the handler swallows errors per branch.
			await mod.default.scheduled({ cron: '0 * * * *' }, env, { waitUntil: () => {} });
		} finally {
