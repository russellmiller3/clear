...
const TEST_TMP = join(process.cwd(), '.tmp');
mkdirSync(TEST_TMP, { recursive: true });

function mkTempDb() {
	const dir = mkdtempSync(join(TEST_TMP, 'clear-hidden-'));
...
describe('runtime db — hidden field filtering on read', () => {
	...
	it('backfills renamed fields from hidden source columns during schema evolution', () => {
		const { db, cleanup } = mkTempDb();
		const T = uniq();
		try {
			db.createTable(T, {
			...
			db.insert(T, { name: 'Eve', notes: 'old value' });
			db.createTable(T, {
				name: { type: 'text' },
			...
			const rows = db.findAll(T);
			expect(rows[0].reason).toBe('old value');
			expect(rows[0].notes).toBe(undefined);
			const raw = db.findOne(T, { name: 'Eve' }, { includeHidden: true });
			expect(raw.notes).toBe('old value');
		} finally {
			cleanup();
		}
	});
