...
//
// When opts.target === 'cloudflare', dispatches to the Cloudflare Workers
// for Platforms bundler — writes src/index.js + wrangler.toml into outDir
// instead of the Node server.js / Dockerfile layout. Both paths are
// additive: the default (Node) output is unchanged when target is unset.

import { mkdirSync, writeFileSync, copyFileSync, existsSync } from 'fs';
import { resolve, dirname, basename, extname } from 'path';
import { fileURLToPath } from 'url';
import { packageCloudflareBundle } from './packaging-cloudflare.js';
...
export function packageBundle(result, outDir, opts = {}) {
	// Cloudflare Workers target takes a completely different on-disk layout
	// (src/index.js + wrangler.toml) so dispatch early and skip all the
	// Node-flavored Dockerfile / clear-runtime wiring below.
	if (opts.target === 'cloudflare') {
		return packageCloudflareBundle(result, outDir, opts);
	}
