...
// ─────────────────────────────────────────────────────────────────────────
// Cycle 2.5 — D1 UPDATE requires id, instructive error
// Mirrors the runtime "no-id-throws" landed in session 42 (runtime/db.js
// update() throws a 400 with a hint). The D1 emit path must preserve that
// guard so silent no-op updates can't corrupt a fresh Worker.
// ─────────────────────────────────────────────────────────────────────────

const UPDATE_APP = `build for javascript backend
database is local memory
...
when user calls PUT /api/todos/:id sending todo:
  requires login
  save todo to Todos
  send back todo
`;

describe('Phase 2 cycle 2.5 — D1 UPDATE requires id, instructive error', () => {
	it('emits UPDATE with ? placeholder for id', () => {
		const result = compileProgram(UPDATE_APP, { target: 'cloudflare' });
		expect(result.errors.length).toBe(0);
		const src = result.workerBundle['src/index.js'];
		expect(src).toContain('env.DB.prepare(');
		expect(src).toContain('UPDATE todos');
		expect(/WHERE\s+id\s*=\s*\?/.test(src)).toBe(true);
		expect(src).toContain('.bind(');
	});

	it('UPDATE path includes runtime guard that throws when id is missing', () => {
		// The Node runtime/db.js update() throws a 400 with a helpful hint
		// when an update record lacks id. The D1 emit should carry the same
		// invariant — either inline in the emit or via the db-d1 shim.
		const result = compileProgram(UPDATE_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		// A string hint referencing "id" and "without" should appear — one
		// of: "without an id", "use \"save ... as new\"", or the word
		// combination "no id". Keep the assertion loose but clearly tied
		// to the invariant.
		const hasHint = /without an id|no id|requires id|id is required/i.test(src);
		expect(hasHint).toBe(true);
	});

	it('UPDATE does NOT call db.update() (Node path)', () => {
		const result = compileProgram(UPDATE_APP, { target: 'cloudflare' });
		const src = result.workerBundle['src/index.js'];
		expect(src.includes('db.update(')).toBe(false);
	});
});

// ─────────────────────────────────────────────────────────────────────────
// Cycle 2.6 — Migrations emit as SQL in workerBundle['migrations/001-init.sql']
// Every `define <Table>:` block becomes a CREATE TABLE in SQLite dialect
// (D1 = SQLite). The deploy step applies these via the CF API before the
// Worker sees its first request.
// ─────────────────────────────────────────────────────────────────────────

const MIGRATIONS_APP = `build for javascript backend
database is local memory
...
create a Posts table:
  title, required
  body
  author_id is a number
`;

describe('Phase 2 cycle 2.6 — D1 migrations emit alongside bundle', () => {
	it('result.workerBundle has a migrations/001-init.sql file', () => {
		const result = compileProgram(MIGRATIONS_APP, { target: 'cloudflare' });
		expect(result.errors.length).toBe(0);
		expect(typeof result.workerBundle['migrations/001-init.sql']).toBe('string');
		expect(result.workerBundle['migrations/001-init.sql'].length).toBeGreaterThan(0);
	});

	it('migrations file has one CREATE TABLE per defined table', () => {
		const result = compileProgram(MIGRATIONS_APP, { target: 'cloudflare' });
		const sql = result.workerBundle['migrations/001-init.sql'];
		expect(sql).toContain('CREATE TABLE');
		// Both Users + Posts
		expect(/CREATE\s+TABLE[^\n]+users/i.test(sql)).toBe(true);
		expect(/CREATE\s+TABLE[^\n]+posts/i.test(sql)).toBe(true);
	});

	it('migrations use SQLite dialect: INTEGER PRIMARY KEY AUTOINCREMENT', () => {
		const result = compileProgram(MIGRATIONS_APP, { target: 'cloudflare' });
		const sql = result.workerBundle['migrations/001-init.sql'];
		// D1 is SQLite — must NOT emit Postgres SERIAL or MySQL AUTO_INCREMENT
		expect(sql).toContain('INTEGER PRIMARY KEY AUTOINCREMENT');
		expect(sql.includes('SERIAL')).toBe(false);
		expect(sql.includes('AUTO_INCREMENT')).toBe(false);
	});

	it('migrations use IF NOT EXISTS so re-applying is idempotent', () => {
		const result = compileProgram(MIGRATIONS_APP, { target: 'cloudflare' });
		const sql = result.workerBundle['migrations/001-init.sql'];
		expect(sql).toContain('CREATE TABLE IF NOT EXISTS');
	});

	it('migrations are NOT emitted for the Node target (regression guard)', () => {
		const result = compileProgram(MIGRATIONS_APP);
		expect(result.workerBundle).toBeUndefined();
	});

	it('migrations include every field from the define blocks', () => {
		const result = compileProgram(MIGRATIONS_APP, { target: 'cloudflare' });
		const sql = result.workerBundle['migrations/001-init.sql'];
		// Users has email + role; Posts has title + body + author_id
