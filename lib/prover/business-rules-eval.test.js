...
when user sends deal to /api/deals:
  rule enterprise-discount-cap:
    enforce that deal's discount_percent is less than 40 or 'Enterprise discounts capped at 40%'
  send back 'ok'
`,
  },
  {
...
when user sends deal to /api/deals:
  rule min-deal-size:
    enforce that deal's list_price is greater than 1000 or 'Deals under $1000 use the small-deal queue, not this workflow'
  send back 'ok'
`,
  },
  {
...
when user sends deal to /api/deals:
  rule max-deal-size-needs-cfo:
    enforce that deal's list_price is less than 5000000 or 'Deals over $5M require CFO sign-off via the executive queue'
  send back 'ok'
`,
  },
  {
...
when user sends deal to /api/deals:
  rule risk-score-bounded:
    enforce that deal's risk_score is less than 11 or 'Risk score out of bounds — AI assigned a value beyond 10'
  send back 'ok'
`,
  },
  {
...
when user sends deal to /api/deals:
  rule discount-not-negative:
    enforce that deal's discount_percent is greater than -1 or 'Negative discounts (price increases) are not allowed via this URL'
  send back 'ok'
`,
  },
...
when user sends pto_request to /api/pto:
  rule pto-request-positive-days:
    enforce that pto_request's days is greater than 0 or 'PTO requests must be for at least one day'
  send back 'ok'
`,
  },
  {
...
when user sends pto_request to /api/pto:
  rule pto-request-not-too-long:
    enforce that pto_request's days is less than 30 or 'PTO over 30 days requires HR review via the leave-of-absence URL'
  send back 'ok'
`,
  },
  {
...
when user sends expense to /api/expenses:
  rule expense-receipt-required:
    enforce that expense's receipt_url is not '' or 'Expenses over $25 require a receipt URL — see policy doc'
  send back 'ok'
`,
  },
...
when user sends lead to /api/leads:
  rule lead-must-have-email:
    enforce that lead's email is not '' or 'Lead has no email — cannot route'
  send back 'ok'
`,
  },
  {
...
when user sends lead to /api/leads:
  rule lead-score-bounded:
    enforce that lead's score is less than 101 or 'Lead score out of bounds — scorer should never produce > 100'
  send back 'ok'
`,
  },
...
when user sends ticket to /api/tickets:
  rule ticket-priority-bounded:
    enforce that ticket's priority is less than 6 or 'Priority must be 1-5'
  send back 'ok'
`,
  },
  {
...
when user sends ticket to /api/tickets:
  rule ticket-subject-not-empty:
    enforce that ticket's subject is not '' or 'Ticket subject required'
  send back 'ok'
`,
  },
...
rule always-fails-arithmetic:
  enforce that 1 + 1 is greater than 5 or 'always wrong'
`,
  },
...
rule rule-touches-ai:
  ...
  enforce that scored is not nothing or 'AI did not respond'
`,
  },
  {
...
rule rule-touches-network:
  ...
  enforce that reply is not nothing or 'service down'
`,
  },
...
when user sends deal to /api/deals:
  rule rule-with-conditional:
    ...
    enforce that deal's discount_percent is less than 50 or 'Discount cap'
  send back 'ok'
`,
  },
...
when user sends deal to /api/deals:
  rule computed-margin-rule:
    ...
    enforce that margin is greater than 0 or 'Deal must have positive margin'
  send back 'ok'
`,
  },
];
