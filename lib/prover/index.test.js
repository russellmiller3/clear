...
// ---------------------------------------------------------------------------
await describeAsync('Prover — basic arithmetic', async () => {

  await itAsync('proves addition with concrete inputs', async () => {
    const src = `
...
`;
  ...
  await itAsync('proves multiplication', async () => {
    const src = `
...
`;
  ...
  await itAsync('flags a wrong assertion as FAILED with the observed value', async () => {
    const src = `
define function add(a, b):
  return a + b
...
`;
  ...
  await itAsync('proves chained arithmetic', async () => {
    const src = `
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — comparisons and booleans', async () => {

  await itAsync('proves a greater-than check', async () => {
    const src = `
...
`;
  ...
  await itAsync('proves a not-overdue case', async () => {
    const src = `
define function is_overdue(due_day, now_day):
  return now_day > due_day
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — conditionals', async () => {

  await itAsync('proves an if-then branch', async () => {
    const src = `
...
`;
  ...
  await itAsync('proves the otherwise branch', async () => {
    const src = `
define function classify(score):
  if score >= 90:
    return 'A'
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — nested function calls', async () => {

  await itAsync('proves a function calling another pure function', async () => {
    const src = `
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — impurity detection', async () => {

  await itAsync('marks tests touching impure ops as UNVERIFIABLE, not proved', async () => {
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — edge cases', async () => {

  await itAsync('handles a program with no tests at all', async () => {
    const src = `
define function add(a, b):
  return a + b
`;
  ...
  await itAsync('surfaces parse errors instead of crashing', async () => {
  ...
  await itAsync('proves multiple tests in a program', async () => {
    const src = `
define function add(a, b):
  return a + b
...
`;
  ...
  await itAsync('a single partial test makes the bundle status partial, not proved', async () => {
    const src = `
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — formatter', async () => {

  await itAsync('formatBundle produces readable text for a passing program', async () => {
    const src = `
define function add(a, b):
  return a + b
...
`;
  ...
  await itAsync('formatBundle shows counterexample on failure', async () => {
    const src = `
define function add(a, b):
  return a + b
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — structured enforcement tags', async () => {

  await itAsync('emits kind:tautology for a constant-true guard', async () => {
    const src = `
rule tautology-rule:
...
`;
  ...
  await itAsync('emits kind:structural-enforcement with opaqueExpression for member-access guards', async () => {
    const src = `
...
`;
  ...
  await itAsync('emits one tag per guard for multi-guard rules', async () => {
    const src = `
when user sends deal to /api/deals:
  rule two-guards:
...
`;
  ...
  await itAsync('NEVER leaks symbolic-engine internals into the prose reason field', async () => {
...
`;
  ...
  await itAsync('derives a clean plain-English reason that names the entity', async () => {
    const src = `
...
`;
  ...
  await itAsync('proves a conditional rule whose guards live inside if/otherwise branches', async () => {
...
`;
  ...
  await itAsync('still marks an empty-body conditional rule UNVERIFIABLE (no guards on either branch)', async () => {
...
// ---------------------------------------------------------------------------
await describeAsync('Prover — bounds_agent_output annotation', async () => {

  await itAsync('flags a rule that fires AFTER a call-by-name agent invocation', async () => {
    const src = `
...
`;
  ...
  await itAsync('does NOT flag a rule with no preceding agent call', async () => {
    const src = `
when user sends deal to /api/deals:
  rule plain-cap:
    enforce that deal's discount_percent is less than 30, or fail with error message: 'too high'
...
`;
  ...
  await itAsync('does NOT flag a rule that fires BEFORE the agent call', async () => {
...
`;
  ...
  await itAsync('does NOT flag a rule downstream of a tool-using agent (tool actions cannot be bounded after-the-fact)', async () => {
...
await describeAsync('Prover — agent tool-bound claims', async () => {

  await itAsync('PROVES that an agent without the function in its closure cannot call it', async () => {
    const src = `
...
`;
  ...
  await itAsync('DISPROVES the claim and names the path when the function IS in the agent\'s direct tools', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('DISPROVES via skills closure when the function enters scope through a skill', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('marks UNVERIFIABLE when the named agent does not exist', async () => {
    const src = `
...
`;
  ...
  await itAsync('handles an agent with NO tools at all (every claim against it proves)', async () => {
    const src = `
...
`;
  ...
  await itAsync('TRANSITIVE: PROVES "cannot delete from" when no reachable tool body has a remove on the entity', async () => {
    const src = `
...
`;
  ...
  await itAsync('TRANSITIVE: DISPROVES "cannot delete from" when a tool body has a remove on the entity', async () => {
    const src = `
create a Deals table:
  amount, number
...
`;
  ...
  await itAsync('TRANSITIVE: DISPROVES through a transitive function call chain', async () => {
    const src = `
...
`;
  ...
  await itAsync('TRANSITIVE: PROVES "cannot modify" when reads are the only operations', async () => {
    const src = `
create a Deals table:
  amount, number
...
`;
  ...
  await itAsync('TRANSITIVE: DISPROVES "cannot modify" on a save (insert/update) operation', async () => {
    const src = `
create a Users table:
  email, text
...
`;
  ...
  await itAsync('TRANSITIVE: catches CRUD that lives directly in the AGENT BODY (not just tool bodies)', async () => {
    const src = `
create a Logs table:
  message, text
...
`;
  ...
  await itAsync('TRANSITIVE: claim against a different entity does NOT trip on operations against other tables', async () => {
    const src = `
create a Logs table:
  message, text
...
`;
  ...
  await itAsync('SYMBOLIC: PROVES "cannot call charge_card with amount is greater than 1000" when call sites pass literal small values', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('SYMBOLIC: DISPROVES when a call site passes a free symbolic value (Claude-controlled)', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('SYMBOLIC: PROVES "cannot call charge_card with amount is greater than 1000" when there are zero reachable call sites', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('SYMBOLIC: UNVERIFIABLE when the constrained function is not defined in this file', async () => {
    const src = `
agent 'Refund Bot' receives request:
  reply = ask claude 'Refund' with request
  return reply
...
`;
  ...
  await itAsync('SYMBOLIC: UNVERIFIABLE when the constrained argument name does not match any parameter', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('SYMBOLIC: DISPROVES when the constrained function IS itself a tool the agent can invoke (Claude controls every arg)', async () => {
...
`;
  ...
  await itAsync('SYMBOLIC: catches a literal value that DOES satisfy the constraint (DISPROVED with explanation)', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
...
`;
  ...
  await itAsync('POLICY: PROVES "upholds all policies" when no reachable code touches a protected table', async () => {
    const src = `
create a Users table:
  email, text
...
`;
  ...
  await itAsync('POLICY: DISPROVES when a reachable tool body touches a protected table', async () => {
    const src = `
create a Users table:
  email, text
...
`;
  ...
  await itAsync('POLICY: emits one subverdict per rule (multi-rule policy block)', async () => {
    const src = `
create a Logs table:
  message, text
...
`;
  ...
  await itAsync('POLICY: marks runtime-only rules UNVERIFIABLE rather than falsely PROVING them', async () => {
    const src = `
policy:
...
`;
  ...
  await itAsync('POLICY: PROVES dont_delete_without_where naturally — Clear deletes always carry a condition', async () => {
...
`;
  ...
  await itAsync('POLICY: UNVERIFIABLE when no policy block is declared', async () => {
    const src = `
agent 'Bot' receives request:
  reply = ask claude 'X' with request
  return reply
...
`;
  ...
  await itAsync('multiple claims against the same agent are all evaluated independently', async () => {
    const src = `
define function charge_card(amount, token):
  return amount
