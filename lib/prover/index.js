...
function collectRuleDefs(nodes, out, ast) {
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'rule_def') {
      ...
      // structural guarantee extends to agent-produced inputs — every
      // agent output flows through this rule before the next line runs.
      const boundsAgent = containsAgentInvocation(precedingSiblings);
