...
function symVerdict(check, left, right) {
  ...
  // Fall back to literal comparison for non-eq checks.
  const sl = simplify(left);
  const sr = simplify(right);
  if (sl.kind === 'lit' && sr.kind === 'lit') {
    let passed = false;
    switch (check) {
      case 'neq': passed = sl.value !== sr.value; break;
      case 'gt':  passed = sl.value >  sr.value; break;
      case 'gte': passed = sl.value >= sr.value; break;
      case 'lt':  passed = sl.value <  sr.value; break;
      case 'lte': passed = sl.value <= sr.value; break;
    }
    return { passed, observed: sl, expected: sr, check };
  }
  return { passed: false, unknown: true, observed: sl, expected: sr, check };
}
