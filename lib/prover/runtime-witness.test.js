...
// Async work — top-level await ensures the process actually waits for
// every spawn / fetch to finish before reporting and exiting. The
// describe/it harness CAN'T do this on its own; it() is sync.
console.log('\n📦 lib/prover — runtime witness (every PROVED rule actually rejects bad inputs)\n');
let asyncPassed = 0;
let asyncFailed = 0;
for (const c of CASES) {
  try {
    await runWitnessCase(c);
    console.log(`✅ ${c.name}: 20 violating inputs all rejected with rule name in error`);
    asyncPassed++;
  } catch (err) {
    console.log(`❌ ${c.name}: 20 violating inputs all rejected with rule name in error`);
...
if (asyncFailed > 0) {
  console.log(`\n${asyncFailed} of ${CASES.length} runtime-witness cases FAILED — the prover's PROVED verdict is not corroborated by runtime evidence.`);
  process.exit(1);
}
