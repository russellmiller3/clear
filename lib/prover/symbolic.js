...
const HANDLERS = {
  ...
  binary_op: (n, env) => {
    ...
    const result = simplify(Op(n.operator, left, right));
    // Path-constraint check: if this expression is structurally identical
    // to an assumption that holds on the current path, simplify to true.
    // Conversely if its negation matches an assumption, simplify to false.
    if (env.assumptions && env.assumptions.length > 0) {
      for (const a of env.assumptions) {
        if (sameSym(a, result)) return Lit(true);
        if (a.kind === 'op' && a.op === '!' && a.args.length === 1 && sameSym(a.args[0], result)) return Lit(false);
        // The other direction: assumption is `expr` and we're asking for `not expr`.
        if (result.kind === 'op' && result.op === '!' && result.args.length === 1 && sameSym(result.args[0], a)) return Lit(false);
      }
    }
    return result;
  },
  ...
  if_then: (n, env) => {
    ...
    // Path-constraint propagation: assume `cond` in the then-branch and
    // `not cond` in the else-branch. This lets claims that depend on the
    // assumption (e.g. `x > 0` inside `if x > 0:`) prove cleanly.
    thenEnv.assumptions = [...(env.assumptions || []), simplify(cond)];
    elseEnv.assumptions = [...(env.assumptions || []), simplify(Op('!', cond))];
...
function simplifyOnce(value) {
  ...
  switch (op) {
    ...
    case '*': {
      ...
      // PC-1: distributivity — k * (a + b) → k*a + k*b. Soundness gate is
      // numeric typing on every factor: untyped + could be string concat,
      // and distributing a string concat across multiplication is nonsense.
      // Generalizes to FOIL when more than one factor is a sum:
      //   (a + b) * (c + d) → a*c + a*d + b*c + b*d.
      // The fixed-point simplify loop then re-collects like terms, so the
      // round-trip stays canonical — `2*x + 2*x` reduces to `4*x` etc.
      if (flat.every(isNumeric)) {
        const sumIdx = flat.findIndex(a => a.kind === 'op' && a.op === '+');
        if (sumIdx !== -1) {
          const expanded = expandDistribution(flat);
...
// Distribute multiplication over addition: k * (a + b) → k*a + k*b.
// Generalizes to FOIL when more than one factor is a sum:
//   (a + b) * (c + d) → a*c + a*d + b*c + b*d
//   k * (a + b) * (c + d) → k*a*c + k*a*d + k*b*c + k*b*d
//
// Caller already verified every arg is numeric and at least one arg is an
// Op('+', ...) — the soundness gate lives at the call site so this helper
// can stay focused on the cartesian-product mechanics.
//
// Returns a sum Op('+', ...products), or null if no expansion was useful
// (i.e. there were no sum factors after all — defensive).
function expandDistribution(factors) {
  // Start with one "term" containing no factors yet. Each step multiplies
  // every existing term by every arg of the next factor (cartesian product).
  let terms = [[]];
  for (const factor of factors) {
    if (factor.kind === 'op' && factor.op === '+') {
      const next = [];
      for (const existing of terms) {
        for (const summand of factor.args) {
          next.push([...existing, summand]);
        }
      }
      terms = next;
    } else {
      // Non-sum factor: append to every existing term.
      for (const term of terms) term.push(factor);
    }
  }
  if (terms.length === 1) return null; // no sum factor was actually present
  // Build each term as a product (single arg simplifies on its own; multi-arg
  // product gets a fresh Op('*', ...args)). Then sum them.
  const products = terms.map(t => t.length === 1 ? t[0] : Op('*', ...t));
  return Op('+', ...products);
}
...
/**
 * Prove a simple symbolic comparison under lightweight path assumptions.
 * This is intentionally much smaller than an SMT solver: it handles literal
 * comparisons, Phi branch splits, and one-variable bounds like `fee > 0`.
 */
export function symCompare(check, left, right, assumptions = []) {
  const sl = simplify(left);
  const sr = simplify(right);

  if (sl.kind === 'lit' && sr.kind === 'lit') {
    return compareLiterals(check, sl.value, sr.value);
  }

  if (sl.kind === 'phi') {
    const t = symCompare(check, sl.ifTrue, sr, [...assumptions, sl.cond]);
    const e = symCompare(check, sl.ifFalse, sr, [...assumptions, simplify(Op('!', sl.cond))]);
    if (t === true && e === true) return true;
    if (t === false || e === false) return false;
    return 'unknown';
  }
  ...
  if (sr.kind === 'phi') {
    const t = symCompare(check, sl, sr.ifTrue, [...assumptions, sr.cond]);
    const e = symCompare(check, sl, sr.ifFalse, [...assumptions, simplify(Op('!', sr.cond))]);
    if (t === true && e === true) return true;
    if (t === false || e === false) return false;
    return 'unknown';
  }

  if (comparisonFollowsFromAssumptions(check, sl, sr, assumptions)) return true;
  return 'unknown';
}

function compareLiterals(check, left, right) {
  switch (check) {
...
function comparisonFollowsFromAssumptions(check, left, right, assumptions) {
  if (!['gt', 'gte', 'lt', 'lte'].includes(check)) return false;
  if (right.kind !== 'lit' || typeof right.value !== 'number') return false;

  for (const raw of assumptions) {
    const assumption = simplify(raw);
    if (assumption.kind !== 'op' || assumption.args.length !== 2) continue;
    const [aLeft, aRight] = assumption.args;
    if (!sameSym(aLeft, left)) continue;
    if (aRight.kind !== 'lit' || typeof aRight.value !== 'number') continue;
    if (boundImplies(check, right.value, assumption.op, aRight.value)) return true;
  }
  return false;
}

function boundImplies(wantOp, wantValue, knownOp, knownValue) {
  switch (wantOp) {
    case 'gte':
      return (knownOp === '>'  && knownValue >= wantValue)
          || (knownOp === '>=' && knownValue >= wantValue);
    case 'gt':
      return (knownOp === '>'  && knownValue >= wantValue)
          || (knownOp === '>=' && knownValue >  wantValue);
    case 'lte':
