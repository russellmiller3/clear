...
describe('Symbolic — simplifier', () => {
  ...
  it('distributes 2 * (a + b) into 2*a + 2*b (numeric)', () => {
    const a = Sym('a', 'number'), b = Sym('b', 'number');
  ...
  it('proves 2 * (a + b) equals 2*a + 2*b for numeric a, b', () => {
    const a = Sym('a', 'number'), b = Sym('b', 'number');
  ...
  it('distributes n-ary: k * (x + y + z) into k*x + k*y + k*z (numeric)', () => {
  ...
  it('distributes when the sum is on the left: (a + b) * c into a*c + b*c (numeric)', () => {
    const a = Sym('a', 'number'), b = Sym('b', 'number'), c = Sym('c', 'number');
    const left  = simplify(Op('*', Op('+', a, b), c));
  ...
  it('distributes with a symbolic factor: k * (a + b) === k*a + k*b for any numeric k, a, b', () => {
  ...
  it('does NOT distribute when the sum has untyped operands (soundness gate)', () => {
...
`;
  ...
  it('proves a positive-floor function never returns a negative fee', () => {
    const src = `
