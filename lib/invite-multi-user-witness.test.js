...
describe('multi-user-per-tenant invites — design sanity', () => {
  ...
  // Python parity tests (red-first TDD — 2026-05-11)
  it('compiled Python contains POST invite endpoint when shared scope is on', () => {
    const compiled = compileProgram(SRC);
    expect(compiled.errors.length).toBe(0);
    const py = compiled.python;
    expect(py).toContain('@app.post("/auth/invite")');
    expect(py).toContain('db.create_table("_auth_invites"');
    expect(py).toContain('invite_token');
  });
  it('compiled Python contains GET invite list endpoint when shared scope is on', () => {
    const compiled = compileProgram(SRC);
    const py = compiled.python;
    expect(py).toContain('@app.get("/auth/invite")');
  });
  it('compiled Python does NOT contain invite endpoints without shared scope', () => {
    const noScopeSrc = `target: backend\nallow signup and login\n\ncreate a Deals table:\n  status\n`;
    const compiled = compileProgram(noScopeSrc);
    const py = compiled.python;
    expect(py).not.toContain('@app.post("/auth/invite")');
    expect(py).not.toContain('@app.get("/auth/invite")');
    expect(py).not.toContain('db.create_table("_auth_invites"');
  });
});
