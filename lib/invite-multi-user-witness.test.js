...
async function startCompiledServer(serverJS, port) {
  const tempPath = join(REPO_ROOT, `_invite-witness-${Date.now()}-${process.pid}-${Math.random().toString(36).slice(2, 8)}.cjs`);
  writeFileSync(tempPath, serverJS, 'utf8');
  const proc = spawn(process.execPath, [tempPath], {
    cwd: REPO_ROOT,
    env: { ...process.env, PORT: String(port), NODE_ENV: 'test', JWT_SECRET: 'invite-witness-secret' },
...
describe('multi-user-per-tenant invites — design sanity', () => {
  ...
  it('compiled JS contains the invite endpoint when shared scope is on', () => {
    ...
    expect(js).toContain('const _invites = []');
  ...
  it('compiled JS does NOT contain invite endpoint without shared scope (gated correctly)', () => {
    ...
    expect(js).not.toContain('const _invites = []');
  });
});
