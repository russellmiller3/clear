...
Output shape:
  {
    ...
    controls: [{ id, kind, label, line, sourcePageRoute, selector, action, dataEffect, expected }],
...
function humanizeDataName(value) {
  return String(value || '')
    .replace(/[_-]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function describeControlDataEffect(action, label) {
  const name = humanizeDataName(label) || 'control';
  if (!action || !action.type) return `uses ${name}`;
  if (action.type === 'navigate') return `navigates to ${action.targetRoute || 'another page'}`;
  if (action.type === 'api-call') return `calls ${String(action.method || 'GET').toUpperCase()} ${action.path || ''}`.trim();
  if (action.type === 'state-change') return `updates page state from ${name}`;
  if (action.type === 'input-change') return `updates ${humanizeDataName(action.variable) || name}`;
  if (action.type === 'table-interaction') return `filters and sorts ${name}`;
  if (action.type === 'drilldown') return `selects a ${name} row for detail state`;
  return `${action.type} on ${name}`;
}

function htmlKindForInput(node) {
  const htmlType = node.ui?.htmlType || '';
  if (htmlType === 'select') return 'select';
  if (htmlType === 'checkbox') return 'checkbox';
  if (htmlType === 'textarea') return 'textarea';
  if (htmlType === 'rich-text') return 'rich-text';
  if (htmlType === 'file') return 'file';
  return 'input';
}

/**
 * Strip {var} placeholders from a UAT-collected string and keep only the
 * stable surrounding text. The runtime substitutes {pending_count} into the
 * actual value (e.g. "0"), so the literal "{pending_count} deals waiting on
 * you." would never match the rendered page text. Splitting on {…} and
 * keeping each non-empty fragment lets the walker assert the stable parts
 * ("deals waiting on you.") regardless of the runtime number.
 */
function stripUATPlaceholders(text, out) {
  const s = String(text || '');
  if (!s.includes('{')) {
    if (s.trim()) out.push(s);
    return;
  }
  // Replace {ident} placeholders with a delimiter, split, push each non-empty
  // fragment trimmed. Preserves the stable text around each variable.
  const placeholderSentinel = '\u0000';
  const parts = s.replace(/\{[a-zA-Z_]\w*\}/g, placeholderSentinel).split(placeholderSentinel);
...
export function collectUATVisibleText(nodes, out = []) {
  for (const node of nodes || []) {
    ...
    if (node.type === NodeType.CONTENT) {
      ...
      if (text) stripUATPlaceholders(text, out);
    }
    if (node.type === NodeType.PAGE_HEADER) {
      if (node.title) stripUATPlaceholders(node.title, out);
      if (node.subtitle) stripUATPlaceholders(node.subtitle, out);
    }
    if (node.type === NodeType.STAT_CARD && node.title) stripUATPlaceholders(node.title, out);
    if (node.type === NodeType.CHART && node.title) stripUATPlaceholders(node.title, out);
    if (node.type === NodeType.DISPLAY && node.ui?.label && node.ui.label !== 'Output') stripUATPlaceholders(node.ui.label, out);
...
export function generateUATContract(body) {
  ...
  for (const node of body) {
    ...
    // routeSpecificText is the single string the UAT walker asserts appears on
    // the page. The page title (Clear's `page 'Title'` declaration) is metadata
    // — it sets the <title> tag, not the body. Prefer the first body-visible
    // text fragment (heading, page header, stat card label) so the assertion
    // checks something that's actually on screen. Fall back to the title only
    // when the body has no collected visible text — better than nothing.
    const routeSpecificText = visibleText[0] || node.title || route;
    const page = {
      id: pageId,
  ...
  function addControl(control) {
    ...
    if (!control.dataEffect) control.dataEffect = describeControlDataEffect(control.action, control.label);
  ...
  function collect(nodes, currentPageRoute = null, context = {}) {
    for (const node of nodes || []) {
      ...
      if (node.type === NodeType.ASK_FOR && !node._chatAbsorbed) {
        const variable = sanitizeName(node.variable || '');
        const htmlType = node.ui?.htmlType || htmlKindForInput(node);
        const id = stableUatId('input_control', node.line, node.ui?.label || node.label || variable);
        addControl({
          id,
          kind: 'input-control',
          label: node.ui?.label || node.label || variable || 'Input',
          line: node.line || 0,
          sourcePageRoute: currentPageRoute,
          selector: `#${node.ui?.id || `input_${variable}`}`,
          action: {
            type: 'input-change',
            variable,
            inputType: node.inputType || '',
            htmlType,
            htmlKind: htmlKindForInput(node),
            choices: node.choices || node.ui?.choices || [],
          },
        });
      }
  ...
  return {
    ...
    generatedAt: 'compile-time-static',
    app: {
      hasWebTarget: pages.length > 0,
      hasBackendTarget: body.some(n => n && n.type === NodeType.ENDPOINT),
      hasAuthScaffold: body.some(n => n && n.type === NodeType.AUTH_SCAFFOLD),
    },
    pages,
    routes,
    controls,
...
function emptyContract() {
  return {
    version: 1,
    generatedAt: 'compile-time-static',
    app: { hasWebTarget: false, hasBackendTarget: false, hasAuthScaffold: false },
    pages: [],
    routes: [],
    controls: [],
    apiCalls: [],
...
function isExternalFontUrl(text) {
  const msg = String(text || '');
  return msg.includes('rsms.me/inter/font-files') ||
...
function isIgnorableBrowserError(text) {
  ...
  if (
    msg === 'Failed to load resource: net::ERR_FAILED' ||
    msg === 'Failed to load resource: net::ERR_INTERNET_DISCONNECTED' ||
    msg === 'Failed to load resource: net::ERR_NETWORK_CHANGED'
  ) return true;
  if (msg.endsWith('Failed to fetch') && /^\\[[A-Z]+ \\/api\\//.test(msg)) return true;
  const isExternalFont = isExternalFontUrl(msg);
  if (!isExternalFont) return false;
  return msg.includes('Access to font') ||
...
if (chromium) {
  ...
  page.on('console', msg => {
    ...
    if (msg.type() === 'error' && !isIgnorableBrowserError(text)) consoleErrors.push(text);
  });
  page.on('pageerror', err => {
    if (!isIgnorableBrowserError(err.message)) consoleErrors.push(err.message);
  });
  page.on('requestfailed', req => {
    const url = req.url();
    if (isExternalFontUrl(url) || isIgnorableBrowserError(url)) return;
    if (url.startsWith(BASE)) {
      const failure = req.failure();
      if (failure && failure.errorText && failure.errorText.includes('net::ERR_ABORTED')) return;
      consoleErrors.push('Request failed: ' + req.method() + ' ' + url + (failure && failure.errorText ? ' - ' + failure.errorText : ''));
    }
  });

  // Sign in before walking protected pages.
  // The walker creates a synthetic test user via /auth/signup, captures the
  // returned bearer token, attaches it to every subsequent request via the
  // Authorization header, and mirrors it into localStorage so the front end
  // (which reads token from localStorage on load) sees an authenticated user.
  // Without this step, every creator-scoped table query returns zero rows
  // and the walker mistakes "anonymous user sees nothing" for "table broken".
  if (CONTRACT.app && CONTRACT.app.hasAuthScaffold) {
    await test('Auth: walker signup', async () => {
      const email = 'walker+' + Date.now() + '@test.local';
      const password = 'walker-pw-12345';
      const res = await page.request.post(BASE + '/auth/signup', {
        data: { email, password, name: 'Walker' },
        headers: { 'Content-Type': 'application/json' },
      });
      const status = res.status();
      assert(status === 200 || status === 201, 'Walker signup expected status 200/201, got ' + status);
      const payload = await res.json().catch(() => ({}));
      const token = payload && payload.token;
      assert(typeof token === 'string' && token.length > 0, 'Walker signup did not return a bearer token. Payload: ' + JSON.stringify(payload));
