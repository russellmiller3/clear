...
async function startCompiledServer(serverJS, port) {
  const tempPath = join(REPO_ROOT, `_tenant-witness-${Date.now()}-${process.pid}-${Math.random().toString(36).slice(2, 8)}.cjs`);
  writeFileSync(tempPath, serverJS, 'utf8');
  const proc = spawn(process.execPath, [tempPath], {
    cwd: REPO_ROOT,
    env: { ...process.env, PORT: String(port), NODE_ENV: 'test', JWT_SECRET: 'tenant-witness-secret' },
