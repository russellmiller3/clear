...
page 'Home' at '/':
  ...
  it('records every input-like control with the state variable it changes', () => {
    const src = `build for web
page 'Filters' at '/':
  'Search' is a text input that saves to search_query
  'Stage' as dropdown with ['Open', 'Won'] saves to stage_filter
  checkbox 'Only mine' saves to only_mine`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    const controls = r.uatContract.controls.filter(c => c.kind === 'input-control');
    expect(controls.length).toBe(3);
    const variables = controls.map(c => c.action.variable);
    expect(variables).toContain('search_query');
    expect(variables).toContain('stage_filter');
    expect(variables).toContain('only_mine');
    const stage = controls.find(c => c.action.variable === 'stage_filter');
    expect(stage.action.htmlType).toBe('select');
    expect(stage.dataEffect).toContain('stage filter');
  });

  it('names a data effect for every interactive control in the contract', () => {
    const src = `build for web and javascript backend
database is local memory
create a Deals table:
  customer
page 'Home' at '/':
  ...
  'Search' as text input saves to search_query
  display deals as table showing customer
  button 'Reload':
    get deals from '/api/deals'`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    expect(r.uatContract.controls.length).toBeGreaterThan(0);
    const missing = r.uatContract.controls.filter(c => !c.dataEffect);
...
page 'Home' at '/':
  ...
  it('generates browser steps that change input-like controls', () => {
    const src = `build for web
page 'Filters' at '/':
  'Search' as text input saves to search_query
  'Stage' as dropdown with ['Open', 'Won'] saves to stage_filter
  checkbox 'Only mine' saves to only_mine`;
    ...
    expect(r.errors.length).toBe(0);
    const script = generateBrowserUAT(r.uatContract);
    expect(script).toContain('Use input control:');
    expect(script).toContain('selectOption');
    expect(script).toContain('choices.length > 1 ? choices[1] : choices[0]');
    expect(script).toContain('setChecked');
  });

  it('ignores external font load noise while keeping app errors fatal', () => {
    const src = `build for web
page 'Home' at '/':
  text 'Hello'`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    const script = r.browserUAT;
    expect(script).toContain('function isIgnorableBrowserError');
    expect(script).toContain('rsms.me/inter/font-files');
    expect(script).toContain('fonts.gstatic.com');
    expect(script).toContain("if (msg.type() === 'error' && !isIgnorableBrowserError(text)) consoleErrors.push(text);");
    expect(script).toContain('if (!isIgnorableBrowserError(err.message)) consoleErrors.push(err.message);');
    expect(script).toContain("msg === 'Failed to load resource: net::ERR_FAILED'");
    expect(script).toContain("page.on('requestfailed'");
    expect(script).toContain('if (url.startsWith(BASE))');
    expect(script).toContain("failure.errorText.includes('net::ERR_ABORTED')");
    expect(script).toContain("msg.endsWith('Failed to fetch') && /^\\[[A-Z]+ \\/api\\//.test(msg)");
    expect(script).toContain('if (!isExternalFont) return false;');
  });

  it('keeps generated data-effect labels readable by a 14-year-old', () => {
    const src = `build for web
page 'Snack Finder' at '/':
  'Snack Search' as text input saves to snack_search
  'Snack Type' as dropdown with ['Chips', 'Fruit'] saves to snack_type
  checkbox 'Only cheap snacks' saves to only_cheap_snacks`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    const effects = r.uatContract.controls
      .filter(c => c.kind === 'input-control')
      .map(c => c.dataEffect);
    expect(effects).toContain('updates snack search');
    expect(effects).toContain('updates snack type');
    expect(effects).toContain('updates only cheap snacks');
    for (const effect of effects) {
      expect(effect).not.toContain('_');
      expect(effect).not.toContain('input-change');
    }
  });
});

describe('generateBrowserUAT — auth flow', () => {
  it('flags hasAuthScaffold on the contract envelope when allow signup and login is in source', () => {
    const src = `build for web and javascript backend
database is local memory
...
page 'Home' at '/':
  on page load:
    get deals from '/api/deals'
  display deals as table showing title`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    expect(r.uatContract.app.hasAuthScaffold).toBe(true);
  });

  it('hasAuthScaffold is false when no auth scaffold is declared', () => {
    const src = `build for web
page 'Home' at '/':
  text 'Hello'`;
    const r = compileProgram(src);
    expect(r.errors.length).toBe(0);
    expect(r.uatContract.app.hasAuthScaffold).toBe(false);
  });

  it('emits a signup-then-set-bearer-header step before the walk loop on auth-scaffolded apps', () => {
    const src = `build for web and javascript backend
database is local memory
allow signup and login
create a Deals table:
...
page 'Home' at '/':
  ...
  display deals as table showing title`;
    ...
    const script = r.browserUAT;
    expect(typeof script).toBe('string');
    // The walker must POST to /auth/signup before walking protected pages.
    expect(script).toContain("'/auth/signup'");
    expect(script).toContain('Auth: walker signup');
    // It must capture the bearer token and attach it to subsequent requests.
    expect(script).toContain('payload.token');
    expect(script).toContain('setExtraHTTPHeaders');
    expect(script).toContain("'Bearer '");
    // It must also mirror the token into localStorage so the front end picks it up.
    expect(script).toContain("localStorage.setItem('token'");
    // The signup step must be conditional on the contract's hasAuthScaffold flag.
    expect(script).toContain('CONTRACT.app.hasAuthScaffold');
  });

  it('does NOT emit signup logic when hasAuthScaffold is false', () => {
    const src = `build for web
page 'Home' at '/':
  text 'Hello'`;
    const r = compileProgram(src);
    ...
    const script = r.browserUAT;
    expect(typeof script).toBe('string');
    // Walker still embeds the signup branch (gated by hasAuthScaffold) but the
    // gate evaluates to false at runtime — and the contract reflects that.
    expect(script).toContain('"hasAuthScaffold": false');
  });
});
