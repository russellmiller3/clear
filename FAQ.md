...
**Path:** `lib/prover/`. `evaluator.js` (concrete-value AST walker), `symbolic.js` (symbolic-value algebra + display/path-constraint normalizer), `index.js` (public async `prove(source)` API + bundle formatter), and `z3/` (the SMT decision backend: `session.js` boots Z3, `ast-to-smt.js` translates Clear value trees to Z3 with the type-soundness gate, `solve.js` maps unsat/sat/unknown → PROVED/FAILED+counterexample/PARTIAL). The pre-Z3 term-rewriter is archived verbatim in `legacy-symbolic/` (restorable; used by `z3/parity.test.js` as a differential oracle). Tests: `index.test.js`, `symbolic.test.js`, `business-rules-eval.test.js`, `runtime-witness.test.js`, plus `z3/{z3,encode,parity}.test.js`.
...
**How it works in plain English.** Every `test` block becomes a proof obligation. The prover walks the AST directly — no compilation, no Node spawn — and either (a) verifies the assertion holds for the inputs given (concrete mode), or (b) when a test references a variable that wasn't bound by an assignment, automatically promotes it to a "for any input" placeholder and tries to prove the claim universally (symbolic mode). Symbolic goals are translated to SMT and decided by Z3 — it proves universal arithmetic (commutativity/associativity/distributivity, identities like `x+0 == x`, `x*1 == x`, like-term equalities `x+x == 2*x`), quantified "for all" claims, conditional branch merges, and inequality bounds such as "if fee > 0 return fee, otherwise return 0" proving the result is at least zero. When it can't decide (nonlinear/undecidable, or an untyped value used in arithmetic) it returns PARTIAL, never a false PROVED.
...
**Why Z3 (the SMT solver) backs the symbolic prover (2026-06-26).** The original in-house term-rewriter was a limited decision procedure — it returned PARTIAL the moment a rule left simple algebra, covered only arithmetic/logic, and had a known soundness gap (it could treat `+` as commutative even for strings, a false PROVED). The symbolic *decision* is now an industrial SMT solver ([z3-solver](https://www.npmjs.com/package/z3-solver), the official Z3 WASM build — runs Node-side where threads are free, no browser cross-origin-isolation needed). It decides far more (linear arithmetic, quantified "for all", more theories), gives real counterexamples, and is sound per type: numeric `+` is Real arithmetic, an untyped value is an uninterpreted sort so arithmetic on it returns PARTIAL (never a guessed PROVED), and only provably-string operands use (non-commutative) concat. The verdict taxonomy, the UNVERIFIABLE/impurity boundary, the business-language explanations, and the agent closed-world analysis are unchanged — Z3 only replaced the ~380-LOC decision core behind the same seam. `simplify` stays for display + path-constraint normalization. A `parity.test.js` differential oracle proves Z3 never downgrades a goal the old engine PROVED. And `lib/prover/z3/capability.test.js` is the no-BS proof of the NEW power: it runs real Clear programs through `prove(source)` — `x*x is at least 0` now PROVED, `x*x > 0` now FAILED with the counterexample `x=0`, an uncapped commission rule correctly FAILED instead of falsely PROVED — and runs each identical goal through the archived engine to confirm it returned `unknown`, so every gain is demonstrably new, not marketing. Run it: `node lib/prover/z3/capability.test.js`.

**How to revert to the old (pre-Z3) solver.** The original term-rewriter is preserved verbatim at `lib/prover/legacy-symbolic/symbolic.js` so a rollback never needs reconstruction.

- **Easiest (history intact):** revert the four `feature/z3-prover-backend` commits — `git revert 464feac^..ad884e8` (Phase 0 → Phase 5). That restores the sync term-rewriter prover and removes the Z3 wiring in one shot.
- **Manual restore (if history was squashed):**
  1. Copy the archive back: `cp lib/prover/legacy-symbolic/symbolic.js lib/prover/symbolic.js`.
  2. In `lib/prover/index.js`: drop the `import { proveEquals, proveCompare } from './z3/solve.js'` line, re-import `symEquals, symCompare` from `./symbolic.js`, and revert `symVerdict` to call them synchronously (the pre-Z3 version is in the archived commit). Make `prove` / `proveTest` / `proveTestSymbolic` / `symVerdict` synchronous again and drop the `await`s at their call sites (`cli/clear.js`, `studio/server.js`, `scripts/audit-bundle.mjs`, `lib/proof-business-language.mjs`, and the `await proveSource`/`itAsync` conversions in the spec files).
  3. Optional cleanup: `rm -rf lib/prover/z3` and `npm rm z3-solver`.
- **Why you might:** Z3 adds a WASM dependency and async to the prover. If a deployment can't take either, the archived engine still proves the demo theorems — at the cost of the soundness fix and the broader theory coverage.
