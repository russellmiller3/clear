...
- `call api 'url' with method 'POST' with bearer token sending { ... }` now accepts multi-line body records.
- The fix works in both assigned endpoint calls and standalone scheduled jobs.
- Studio is now the live path in scripts, docs, tests, and Meph guidance; stale Playground paths were replaced with `studio/`.

Verification: new regression test covers endpoint and background-job calls; full compiler suite passed 3037/3037 before the docs/path cleanup.
...
**Update 2026-05-06 evening — Python /auth/login auto-rate-limited (OWASP Piece 4 parity).** When `allow signup and login` is declared, the Python emit now imports `rate_limit` from `clear_runtime.rate_limit`, instantiates `_login_throttle = rate_limit(window_ms=60000, max=10)` at module level, and applies it as a FastAPI dependency on the `/auth/login` route via `dependencies=[Depends(_login_throttle)]`. Mirrors the JS emit's `rateLimit({ windowMs: 60000, max: 10 })` middleware on the same route. 10 attempts per minute per IP, before the handler runs — brute-force throttling is structural, not opt-in. `Depends` added to the FastAPI import line at the top of every Python emit (small cost, keeps the wiring shape simple).

**Update 2026-05-06 evening — Python parity audit slice-detection fix (21 → 5 HIGH gaps).** `scripts/python-parity-audit.mjs` was over-reporting HIGH-severity gaps because the slice approach only counted `NodeType.X` references inside `compileToPythonBackend`. But many handlers live OUTSIDE that slice — in `compileNode` (shared between JS + Python via `ctx.lang === 'python'` branching), in helper functions like `compileAuthScaffoldPython`, and in shared compilers like `compileCrud` / `compileEndpoint` / `compileValidate` (each with internal Python branching). New `countSharedPythonHandlers` walks every `case NodeType.X:` in the JS slice and counts it as Python-handled if the case body shows ANY of: explicit `ctx.lang === 'python'` check, or a call to any `compile<Name>(` helper (which usually has internal Python branches). HIGH-severity gap count drops 21 → 5: **the real remaining gaps are RUN_AGENT (11 JS hits), RUN_PIPELINE (4), SCRIPT (2 — test blocks), RUN_WORKFLOW (1), WITH_OPTIMISTIC_LOCK (1)**. CRUD / AUTH_SCAFFOLD / RATE_LIMIT / POLICY / EMAIL_TRIGGER / QUEUE_DEF were all false positives — handlers exist via shared compilers, audit just couldn't see them. Now Python parity progress is accurately measurable.
...
## 2026-05-06 — Hartl user-guide rewrite, chapters 3-4 + 7-10

Same session as the plan + chapters 1-2 ship below. Continued into chapters 3, 4, 7, 8, 9, and 10 because the cadence felt right and the deal-desk anchor was warm.
...
- **Chapter 8 — Logging In and Owning Your Deals (The Wall Between Users).** Replaces the old 'Multi-Page Apps' stub with the auth + per-row ownership chapter — the headline OWASP Piece 1 primitive in plain English. Teaches what login means (the wall between users), what ownership means (every deal has a creator, only the creator can read/change/delete it), how `allow signup and login` auto-emits signup/login/me endpoints + bcrypt hashing + JWT signing + auto rate-limit on login, why `requires login` goes on the FIRST line of every mutation body, and how `the deal's creator can read, change, or delete` makes the compiler weave a per-user filter into every CRUD operation without the reader writing `user_id` anywhere. End-of-chapter run: signup + create-deal as Alice via `curl`, signup + create-deal as Bob, then GET `/api/deals` as each user — Alice sees only Alice's deal, Bob sees only Bob's, neither can see the other's. Headline pitch beat: a stolen session token cannot read another user's deals.
- **Chapter 9 — The CRO Approval Queue (From CRUD App to Workflow App).** Replaces 'Real-Time Features' (folded into the reference track) with the queue primitive — the chapter where deal-desk earns the name. Teaches what an approval queue is (a list of work items waiting on somebody with authority — code review, expense reports, vacation requests, all the same shape), why workflow primitives exist (a hand-rolled approve/reject/counter pipeline is ~150 lines of boring plumbing every deal-desk-style app reinvents), what an audit table is (a permanent decision trail compliance buyers actually pay for), and what an outbound email queue is (a row-in-a-table outbox so the user's request never blocks on a flaky email server). The five-line `queue for deal: reviewer is 'CRO' / actions: approve, reject, counter / email customer when counter / email rep when approve, reject` block auto-generates the `deal_decisions` audit table, the `deal_notifications` outbox, the filtered `GET /api/deals/queue` handler, login-gated `PUT /api/deals/:id/<action>` URLs per action, and `GET /api/deal-decisions` + `GET /api/deal-notifications` history views. End-of-chapter run: signup as Alice (rep) + Carol (admin/CRO), Alice POSTs a deal, Carol GETs `/api/deals/queue` to see it pending, Carol PUTs `/approve`, then Alice's GET shows status flipped to approved AND `/api/deal-decisions` has one row stamped with Carol's user-id AND `/api/deal-notifications` has one outbox row pointing at Alice's email. Headline pitch beat: five lines that would have been 150 by hand.
- **Chapter 10 — Email the Rep When Approved (From Outbox Row to Real Email).** Replaces the old 'AI-Powered Apps' chapter (AI moves to Chapter 11) with the email delivery chapter — the half of the workflow story that turns Chapter 9's outbox row into a real customer email. Teaches what a provider is (AgentMail, SendGrid, Resend, Postmark, Mailgun — small companies that own the mail-delivery problem so you don't have to), why the email is decoupled from the click (a flaky email server should not be able to make Carol's approve button feel slow or fail outright), what a template is (one rule with `{deal's customer}` placeholders that fills in per-row at send time), and what reply tracking does (`track replies as deal activity` routes the customer's reply back onto the deal record so the conversation and the record are one thing). The `email rep when deal's status changes to 'approved':` block emits a row into the shared `workflow_email_queue` table from every handler that flips the status; the optional `email delivery using agentmail` directive emits a 30-second-poll background worker that delivers pending rows via the provider's API. Inert by default — no real mail leaves the building until the directive is added AND the `AGENTMAIL_API_KEY` env var is set. End-of-chapter run: signup as Alice + Carol, save a deal carrying `rep_email`, have Carol PUT `/approve`, GET `/api/workflow-email-queue` to see one row with placeholders already substituted, set the env var, wait 30 seconds, see the row flip to `'sent'`. Headline pitch beat: the customer's reply lives on the deal — the conversation and the record are one thing.
- **Chapter 11 — The AI Drafter (Claude Writes the Deal Summary).** Replaces the old 'Making It Pretty (Styling and Layout)' reference chapter (folded into Chapter 7 expanded per the plan) with the first AI chapter. Teaches what an AI agent is in code (a named recipe for asking Claude to do one specific job — a function whose body is "ask the AI"), why the AI needs context (Claude only sees what you put into the prompt — `with deal_data` for one record, `knows about: Deals` for ongoing background knowledge), what `ask claude` actually does (HTTPS round-trip to Anthropic's API, ~1-5s, fractions of a cent per call), and the load-bearing distinction between rule-vs-AI: the rule is the gate (deterministic, provable, the same answer every time), the AI is the writer (non-deterministic, great at language, never the verdict). The deal-desk drafter is `define function draft_approval(deal_data):` plus one line of `ask claude '<analyst prompt>' with deal_data returning JSON text:` and a three-field output shape — wired to `/api/deals/draft` with `requires login`. End-of-chapter run: set `ANTHROPIC_API_KEY`, sign Carol up, POST the Acme renewal to `/api/deals/draft`, see Claude come back with a CRO-ready paragraph + recommendation + risk_score where every fact traces to the deal data sent in. Inert by default — missing key logs once and goes quiet. Headline pitch beat: Claude writes the summary, the rule decides the verdict, the audit trail records both. Sets up Chapter 12 (the `rule` keyword) by drawing the AI/rule split sharply.

9 of 12 tutorial chapters done (1-4 + 7-11; 5 and 6 already shipped on previous branches). Remaining: 12 across ~1 session.
