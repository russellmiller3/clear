...
const _clearLineMap = process.env.CLEAR_DEBUG ? {"203":3,"210":7,"211":7,"217":8,"220":10,"221":10,"222":11,"232":13,"233":13,"236":14,"238":14,"247":16,"248":16,"252":17,"254":17,"263":19,"264":19,"269":20,"272":20,"274":21,"284":23,"285":23,"289":24,"290":24,"291":25} : null;
// ┌─────────────────────────────────────────────┐
// │         CLEAR APP — Architecture            │
// └─────────────────────────────────────────────┘
//
// TABLES:
//   Todos: title*, done (line 3)
//
// ENDPOINTS:
//   POST /api/todos (line 7)
//   GET /api/todos (line 13)
//   GET /api/todos/:id (line 16)
//   PUT /api/todos/:id (line 19)
//   DELETE /api/todos/:id (line 23)
//
...
const _clearMap = process.env.CLEAR_DEBUG ? {
  tables: {
    todos: { line: 3, file: "main.clear", fields: { title: { required: true, type: "text" }, done: { type: "boolean" } } },
  },
  endpoints: {
    "POST /api/todos": { line: 7, file: "main.clear", auth: false },
    "GET /api/todos": { line: 13, file: "main.clear", auth: false },
    "GET /api/todos/:id": { line: 16, file: "main.clear", auth: false },
    "PUT /api/todos/:id": { line: 19, file: "main.clear", auth: false },
    "DELETE /api/todos/:id": { line: 23, file: "main.clear", auth: false },
  }
} : null;
...
app.post('/api/todos', async (req, res) => {
  try {
    ...
    // clear:10
    const saved = await _clearTry(() => db.insert('todos', _pick(todo, TodosSchema)), { op: 'insert', table: 'todos', line: 10, file: 'main.clear', source: "" }); // clear:10
    // clear:11
    return res.status(201).json(saved);
  } catch (err) {
    console.error('[POST /api/todos] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'POST /api/todos', line: 7, file: 'main.clear' }, err._clearCtx || {});
...
// clear:13
// clear:13 — GET /api/todos
app.get('/api/todos', async (req, res) => {
  try {
    // clear:14
    const _resp_14 = (await db.findAll('todos', {}, { limit: 50 })).map(_revive);
    // clear:14
    return res.json(_resp_14);
  } catch (err) {
    console.error('[GET /api/todos] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/todos', line: 13, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:16
// clear:16 — GET /api/todos/:id
app.get('/api/todos/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:17
    const _resp_17 = _revive(await db.findOne('todos', { id: incoming?.id }));
    // clear:17
    return res.json(_resp_17);
  } catch (err) {
    console.error('[GET /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'GET /api/todos/:id', line: 16, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
// clear:19
// clear:19 — PUT /api/todos/:id
app.put('/api/todos/:id', async (req, res) => {
  try {
    ...
    await _clearTry(() => db.update('todos', _picked_changes), { op: 'update', table: 'todos', line: 20, file: 'main.clear', source: "" }); // clear:20
    Object.assign(changes, await db.findOne('todos', { id: _picked_changes.id }) || {});
    // clear:21
    return res.json(changes);
  } catch (err) {
    console.error('[PUT /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'PUT /api/todos/:id', line: 19, file: 'main.clear' }, err._clearCtx || {});
    if (typeof process !== 'undefined' && process.env.CLEAR_DEBUG === 'verbose') _ctx.input = req.body;
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
...
// clear:23
// clear:23 — DELETE /api/todos/:id
app.delete('/api/todos/:id', async (req, res) => {
  try {
    const incoming = req.params;
    // clear:24
    await _clearTry(() => db.remove('todos', { id: incoming?.id }), { op: 'remove', table: 'todos', line: 24, file: 'main.clear', source: "" }); // clear:24
    // clear:25
    return res.status(204).json({ message: "ok" });
  } catch (err) {
    console.error('[DELETE /api/todos/:id] Error:', err.message);
    const _ctx = Object.assign({ endpoint: 'DELETE /api/todos/:id', line: 23, file: 'main.clear' }, err._clearCtx || {});
    const _info = _clearError(err, _ctx);
    res.status(_info.status).json(_info.response);
  }
});
