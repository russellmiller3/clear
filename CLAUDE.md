...
**Doc cascade runs at PHASE-END, not commit-end (HARD RULE — added 2026-04-28; tightened 2026-05-01).** When a session ships multiple commits all in service of the same epic / phase / feature, run the relevant documentation sweep at the end of EACH completed phase before continuing. A "phase" is a coherent unit of shipped work (CC-2 close, deal-desk polish, measurement harness cycle, retrieval cycle). One cascade per phase, not per commit. If a phase touched training signal, update `RESEARCH.md`, `FAQ.md`, `CHANGELOG.md`, `FEATURES.md` when relevant, and `learnings.md` before moving on. Do not wait until the end of the whole session; stale phase context is how docs drift.
